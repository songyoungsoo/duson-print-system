...
if (PMA_MYSQL_INT_VERSION >= 32303) {
    ...
    if ($cfg['SkipLockedTables'] == TRUE && PMA_MYSQL_INT_VERSION >= 32330) {
        ...
        if ($db_info_result != FALSE && mysqli_num_rows($db_info_result) > 0) {
            while ($tmp = PMA_mysql_fetch_row($db_info_result)) {
                // if in use memorize tablename
                if (preg_match('in_use=[1-9]+', $tmp[1])) {
                    $sot_cache[$tmp[0]] = TRUE;
                }
            }
            mysqli_free_result($db_info_result);
    ...
    $num_tables     = ($db_info_result) ? @mysqli_numrows($db_info_result) : 0;
