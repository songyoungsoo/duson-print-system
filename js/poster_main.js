/**
 * 포스터 견적안내 시스템 - 고급 갤러리 및 실시간 계산기
 * 포스터 전용 JavaScript 모듈
...
function loadPosterImages() {
    ...
    galleryContainer.innerHTML = '<div class="loading">🎨 포스터 갤러리 로딩 중...</div>';
    ...
    fetch('get_poster_images.php')
        ...
        .then(data => {
            if (data.success && data.data && data.data.length > 0) {
                ...
                galleryContainer.innerHTML = '<div class="error">포스터 갤러리 이미지를 불러올 수 없습니다.</div>';
            }
        })
        .catch(error => {
            console.error('포스터 갤러리 로딩 오류:', error);
