...
class PremiumOptionsGeneric {
    ...
    formatPriceLabel(pc) {
        ...
        if (pc.base_500 !== undefined) {
            const base = Number(pc.base_500);
            const perUnit = Number(pc.per_unit) || 0;
            // per_unit이 0이면 고정가 표시 (약도, 마크로고 등)
            if (perUnit === 0) {
                return base > 0 ? `(${base.toLocaleString()}원)` : '';
            }
            const addFee = pc.additional_fee ? ` +${Number(pc.additional_fee).toLocaleString()}원` : '';
            return `(500매 이하 ${base.toLocaleString()}원, 초과시 매당 ${perUnit.toLocaleString()}원${addFee})`;
        }
