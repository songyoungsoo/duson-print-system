...
## 📋 비즈니스 규칙 참조 (Business Rules Reference)

**Primary Source**: `/home/user/.claude/skills/duson-print-rules/SKILL.md`

두손기획 인쇄 시스템의 비즈니스 규칙은 **duson-print-rules 스킬**에서 관리됩니다.
CLAUDE.md는 기술 구현 기록만 포함하며, 비즈니스 규칙은 스킬을 참조하세요.

### 핵심 규칙 요약

| 규칙 | 내용 | 스킬 섹션 |
|------|------|----------|
| **수량 표기** | 전단지/리플렛만 소수점(0.5연), 나머지는 정수 | [핵심 규칙 1] |
| **규격/옵션 표기** | 규격 2줄 + 옵션 0~2줄 원칙 | [핵심 규칙 2] |
| **이모지 금지** | 모든 출력물에 이모지 사용 금지 | [핵심 규칙 3] |

### 스킬 참조 방법

```bash
# 스킬 파일 직접 읽기
cat /home/user/.claude/skills/duson-print-rules/SKILL.md

# 또는 Claude Code에서
Read /home/user/.claude/skills/duson-print-rules/SKILL.md
```
...
| 스킬명 | 용도 |
|--------|------|
| `duson-print-rules` | 수량/규격/옵션 표기 규칙 |
| `duson-price-calculation` | 9개 품목 가격 계산 로직 |
| `duson-gallery-system` | 갤러리 시스템 아키텍처 |
