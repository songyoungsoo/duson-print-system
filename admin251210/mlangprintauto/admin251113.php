        ...
        <div class="file-section" style="padding: 12px; margin: 10px 0;">
            ...
            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border: 1px solid #e9ecef;">
                    ...
                    if ($row) {
                        ...
                        // ✅ FIX: mlangorder_printauto 테이블에서 이미 조회한 $row에서 ImgFolder 정보 가져오기
                        // session_id를 통한 shop_temp/shop_order 조회 불필요
                        ...
                        // ⚠️ 수정: 주문번호가 정확히 포함된 디렉토리만 검색 (세션 ID 검색 제거)
                        ...
                        if (is_dir($sticker_upload_base)) {
                            ...
                            foreach ($sticker_subdirs as $subdir) {
                                if ($subdir != "." && $subdir != ".." && is_dir($sticker_upload_base . $subdir)) {
                                    // 주문번호가 디렉토리명에 포함된 경우만 추가
                                    if (strpos($subdir, (string)$no) !== false) {
                        ...
                        // uploads/날짜/IP/ 디렉토리 확인 (명함, 전단지 등 날짜별 업로드)
                        // ⚠️ 수정: ImgFolder에 저장된 경로만 사용 (이전 주문 파일 중복 표시 방지)
                        $date_upload_base = "../../uploads/";
                        if (!empty($row['ImgFolder']) && is_dir($date_upload_base)) {
                            // ImgFolder에 저장된 경로가 있으면 해당 경로만 추가
                            $img_folder_path = "../../" . $row['ImgFolder'];
                            if (is_dir($img_folder_path) && !in_array($img_folder_path, $sticker_dirs)) {
                                $sticker_dirs[] = $img_folder_path;
                            }
                        }
                        ...
                        // 🔍 디버그: 검색 중인 디렉토리 목록 표시
                        echo "<div style='background:#fff3cd;padding:8px;margin:10px 0;border-left:3px solid #ffc107;font-size:0.85em;'>";
                        echo "🔍 <strong>검색 중인 디렉토리 (" . count($all_dirs) . "개):</strong><br>";
                        foreach ($all_dirs as $idx => $d) {
                            $exists = is_dir($d) ? "✅" : "❌";
                            echo "$exists " . ($idx + 1) . ". <code>" . str_replace('../../', '', $d) . "</code><br>";
                        }
                        echo "</div>";
                        ...
                        foreach ($all_dirs as $dir) {
                            if (is_dir($dir)) {
                                ...
                                if (strpos($dir, 'sticker_new') !== false) {
                                    echo "<div style='margin-top: 10px; color: #28a745; font-weight: bold;'>🏷️ 스티커 업로드 폴더: <code style='font-size:0.85em;color:#666;'>$display_path</code></div>";
                                } elseif (strpos($dir, 'orders') !== false) {
                                    echo "<div style='margin-top: 10px; color: #007bff; font-weight: bold;'>📦 주문 파일 폴더: <code style='font-size:0.85em;color:#666;'>$display_path</code></div>";
                                } elseif (preg_match('/uploads\/\d{4}\/\d{2}\/\d{2}/', $dir)) {
                                    echo "<div style='margin-top: 10px; color: #9b59b6; font-weight: bold;'>📤 업로드 파일 폴더: <code style='font-size:0.85em;color:#666;'>$display_path</code></div>";
                                } else {
                                    echo "<div style='margin-top: 10px; color: #6c757d; font-weight: bold;'>📁 기타 폴더: <code style='font-size:0.85em;color:#666;'>$display_path</code></div>";
                                }
                        ...
                        // 🔧 날짜 기준 레거시 시스템 처리 (2025-09-20 이전 주문만)
                        $order_date = strtotime($row['date']);
                        $legacy_cutoff_date = strtotime('2025-09-20');

                        if ($order_date < $legacy_cutoff_date) {
                            // 레거시 주문: ThingCate 파일명 체크
                            if (!empty($row['ThingCate']) &&
                                !in_array($row['ThingCate'], $displayed_files) &&
                                preg_match('/\.(jpg|jpeg|png|gif|pdf|ai|zip)$/i', $row['ThingCate'])) {

                                echo "<div style='margin-top: 10px; padding: 8px; background: #e3f2fd; border-left: 3px solid #2196f3;'>";
                                echo "ℹ️ <strong>레거시 파일 참조:</strong> " . htmlspecialchars($row['ThingCate']) . "<br>";
                                echo "<small style='color: #666;'>2025-09-20 이전 주문 - 구 업로드 시스템 파일</small>";
                                echo "</div>";
                            }
                        }
                    } else {
                    ...
                    // 🔧 레거시 업로드 경로 체크 (2025-09-20 이전 주문만)
                    if (isset($order_date) && $order_date < $legacy_cutoff_date) {
                        $dir_path = "../../ImgFolder/$View_ImgFolder";
                        if (!empty($View_ImgFolder) && is_dir($dir_path)) {
                            echo "<div style='margin-top: 10px; padding: 8px; background: #f5f5f5; border-left: 3px solid #9e9e9e;'>";
                            echo "<strong>📁 레거시 파일 디렉토리:</strong> $View_ImgFolder<br>";
