...
// include"../../db.php";
$TIO_CODE="cadaroktwo";
$table="mlangprintauto_cadaroktwo"; // ✅ 소문자 테이블명
...
if($search=="yes"){ //검색모드일때
 ...
 if($myListTreeSelect && $myListTreeSelect != '#') {
     $where_parts[] = "Section='$myListTreeSelect'";
 }
 if($myList && $myList != '#') {
     $where_parts[] = "TreeSelect='$myList'";
 }
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarokTwo' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarokTwo' AND no='{$row['Section']}'");
...
<td align=center><font color=white>
<?php 
$result_FGFree=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarokTwo' AND no='{$row['TreeSelect']}'");
