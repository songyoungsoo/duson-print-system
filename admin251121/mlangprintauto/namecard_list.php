...
<input type='button' onClick="javascript:window.open('<?=$TIO_CODE_ADMIN?>_admin.php?mode=IncForm', '<?=$table?>_Form1','width=820,height=600,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no');" value=' 가격/설명 관리 '>
<input type='button' onClick="javascript:popup=window.open('<?=$TIO_CODE_ADMIN?>_admin.php?mode=form&Ttable=<?=$TIO_CODE?>', '<?=$table?>_Form2','width=300,height=250,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 신 자료 입력 '>
<BR><BR>
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='namecard' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='namecard' AND no='{$row['Section']}'");
$row_FGOne= mysqli_fetch_array($result_FGOne);
if($row_FGOne){ echo(htmlspecialchars($row_FGOne['title'])); }
?>
</font></td>
<td align=center><font color=white>
<?if($row['POtype']=="1"){echo("단면");}?>
<?if($row['POtype']=="2"){echo("양면");}?>
</font></td>
...
<td align=center>
<input type='button' onClick="javascript:popup=window.open('<?=$TIO_CODE_ADMIN?>_admin.php?mode=form&code=Modify&no=<?=$row['no']?>&Ttable=<?=$TIO_CODE?>', '<?=$table?>_Form2Modify','width=300,height=250,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수정 '>
