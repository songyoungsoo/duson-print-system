...
// include"../../db.php";
$TIO_CODE="cadarok";
$table="mlangprintauto_cadarok"; // ✅ 소문자 테이블명
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarok' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarok' AND no='{$row['Section']}'");
...
<td align=center><font color=white>
<?php 
$result_FGFree=mysqli_query($db, "select * from $GGTABLE where Ttable='cadarok' AND no='{$row['TreeSelect']}'");
