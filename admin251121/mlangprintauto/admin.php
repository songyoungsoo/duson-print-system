            ...
            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border: 1px solid #e9ecef;">
                    ...
                    if ($row) {
                        ...
                        if (!empty($row['ImgFolder'])) {
                            // 레거시 형식 경로 처리: _MlangPrintAuto_*_index.php/...
                            if (strpos($row['ImgFolder'], '_MlangPrintAuto_') === 0) {
                                // 새로운 레거시 경로 형식: ImgFolder/{ImgFolder}
                        ...
                        // ❌ uploads/sticker_new/ 스캔 제거
                        // 이유: session_id()는 현재 관리자의 세션이므로 다른 사용자의 파일을 잘못 가져옴
                        // 해결: ImgFolder에 명시된 경로만 사용 (Line 723-732에서 이미 처리됨)

                        // 날짜별 업로드 폴더 스캔 제거 (다른 주문 파일 표시 방지)
                        // 레거시 경로만 사용
                        ...
                        foreach ($all_dirs as $dir) {
                            if (is_dir($dir)) {
                                ...
                                if (strpos($dir, 'sticker_new') !== false) {
                                    ...
                                    echo "<div style='margin-top: 10px; color: #9b59b6; font-weight: bold;'>📤 업로드 파일 폴더 (" . str_replace('../../uploads/', '', $dir) . "):</div>";
                                }
                    ...
                    // 🔧 레거시 업로드 경로 체크 (모든 주문 - 날짜 제한 제거)
                    // ImgFolder가 레거시 형식(_MlangPrintAuto_*_index.php/...)인 경우 표시
                    if (!empty($row['ImgFolder']) && strpos($row['ImgFolder'], '_MlangPrintAuto_') === 0) {
                        ...
                        if (is_dir($dir_path)) {
                            ...
                            echo "<strong>✅ 레거시 경로 파일:</strong> " . htmlspecialchars($row['ImgFolder']) . "<br>";
                            $dir_handle = opendir($dir_path);
                            $file_count = 0;
                            while ($tmp = readdir($dir_handle)) {
                                if ($tmp != "." && $tmp != "..") {
                                    // download.php를 통한 안전한 다운로드
                                    $download_path = urlencode("ImgFolder/" . $row['ImgFolder']);
                                    $file_size = filesize("$dir_path/$tmp");
                                    $file_size_mb = round($file_size / 1024 / 1024, 2);

                                    // 대표 파일 아이콘
                                    $icon = ($tmp == $row['ThingCate']) ? "📌" : "📄";

                                    echo "$icon <a href='download.php?no=$no&downfile=" . urlencode($tmp) . "&path=$download_path' class='file'>";
                            ...
                            closedir($dir_handle);
                            if ($file_count == 0) {
                                echo "<span style='color: #999;'>폴더는 존재하지만 파일이 없습니다.</span>";
                            }
                            echo "</div>";
                        } else {
                            echo "<div style='margin-top: 10px; padding: 8px; background: #fff3cd; border-left: 3px solid #ffc107;'>";
                            echo "<strong>⚠️ 레거시 경로:</strong> " . htmlspecialchars($row['ImgFolder']) . "<br>";
                            echo "<span style='color: #856404;'>폴더가 존재하지 않습니다.</span>";
                            echo "</div>";
                        }
                    }
                    ?>
