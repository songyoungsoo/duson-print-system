...
function get_settings($db) {
    $settings = [];
    $default_settings = [
        'material_art' => 1.5, 'material_yupo' => 2.5, 'material_silver_deadlong' => 2.5,
        'material_clear_deadlong' => 2.5, 'material_gold_paper' => 3.5, 'material_silver_paper' => 3.5,
        'material_kraft' => 2.3, 'material_hologram' => 5.0, 'material_minimum_multiplier' => 10000,
        'design_per_color' => 5000, 'design_minimum' => 10000, 'film_per_sqmm' => 1, 'film_minimum' => 4000,
        'resin_per_sqmm' => 2, 'resin_minimum' => 5000, 'embossing_knife_per_sqmm' => 2,
        'embossing_knife_minimum' => 10000, 'printing_per_sheet_color' => 10, 'printing_minimum' => 10000,
        'white_printing_per_sheet' => 20, 'white_printing_minimum' => 20000, 'white_film_per_sqmm' => 1,
        'white_film_minimum' => 4000, 'white_resin_per_sqmm' => 3, 'white_resin_minimum' => 5000,
        'coating_glossy' => 50000, 'coating_matte' => 40000, 'coating_uv' => 10000,
        'foil_per_sqmm' => 0.016, 'foil_minimum' => 35000, 'plate_per_sqmm' => 5, 'plate_minimum' => 5000,
        'embossing_press_per_1000' => 30000, 'embossing_plate_per_sqmm' => 3,
        'embossing_plate_minimum' => 5000, 'partial_coating_per_1000' => 30000,
    ];
    $check_table = $db->query("SHOW TABLES LIKE 'roll_sticker_settings'");
    if ($check_table && $check_table->num_rows > 0) {
        $result = $db->query("SELECT setting_key, setting_value FROM roll_sticker_settings");
        if ($result) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $width > 0 && $height > 0 && $quantity > 0) {
    ...
    // 1. 재질선택비
    $material_prices = [
        'art' => $settings['material_art'],           // 아트지
        'yupo' => $settings['material_yupo'],          // 유포지
        'silver_deadlong' => $settings['material_silver_deadlong'], // 은데드롱
        'clear_deadlong' => $settings['material_clear_deadlong'],  // 투명데드롱
        'gold_paper' => $settings['material_gold_paper'],    // 금지
        'silver_paper' => $settings['material_silver_paper'],  // 은지
        'kraft' => $settings['material_kraft'],         // 크라프트
        'hologram' => $settings['material_hologram']       // 홀로그램
    ];
    ...
    $material_minimum = $material_price * $settings['material_minimum_multiplier'];
    ...
    $design_cost = 0;
    if ($need_design) {
        $design_cost = max($settings['design_minimum'], $colors * $settings['design_per_color']);
    }
    ...
    $film_cost_per_color = max($settings['film_minimum'], $film_area * $settings['film_per_sqmm']); // 제곱mm당 1원
    ...
    $resin_cost_per_color = max($settings['resin_minimum'], $resin_area * $settings['resin_per_sqmm']); // 제곱mm당 3원
    ...
    $embossing_cost_per_knife = max($settings['embossing_knife_minimum'], $embossing_area * $settings['embossing_knife_per_sqmm']); // 제곱mm당 2원
    ...
    $printing_cost = max($settings['printing_minimum'], $quantity * $colors * $settings['printing_per_sheet_color']);
    ...
    if ($need_white_printing) {
        $white_printing_cost = max($settings['white_printing_minimum'], $quantity * $settings['white_printing_per_sheet']);
        
        // 7-2. 백색인쇄 필름비 (가로 x 세로, 제곱mm당 1원, 최소 4,000원)
        $white_film_area = $width * $height;
        $white_film_cost = max($settings['white_film_minimum'], $white_film_area * $settings['white_film_per_sqmm']);
        
        // 7-3. 백색인쇄 수지판비 (가로 x 세로, 제곱mm당 3원, 최소 5,000원)
        $white_resin_area = $width * $height;
        $white_resin_cost = max($settings['white_resin_minimum'], $white_resin_area * $settings['white_resin_per_sqmm']);
    }
    ...
    if ($coating != 'none') {
        $coating_prices = [
            'glossy' => $settings['coating_glossy'], // 유광
            'matte' => $settings['coating_matte'],  // 무광
            'uv' => $settings['coating_uv']      // UV
        ];
    ...
    if ($foil != 'none') {
        ...
        $foil_cost = max($settings['foil_minimum'], $foil_area * $settings['foil_per_sqmm']);
    }
    ...
    if ($foil != 'none' || $need_plate) {
        ...
        $plate_cost = max($settings['plate_minimum'], $plate_area * $settings['plate_per_sqmm']); // 제곱mm당 5원
    }
    ...
    if ($embossing != 'none') {
        $embossing_press_cost = ($quantity / 1000) * $settings['embossing_press_per_1000'];
    }
    ...
    if ($embossing != 'none') {
        ...
        $embossing_plate_cost_per_plate = max($settings['embossing_plate_minimum'], $embossing_plate_area * $settings['embossing_plate_per_sqmm']); // 제곱mm당 3원
    ...
    if ($partial_coating) {
        $partial_coating_cost = ($quantity / 1000) * $settings['embossing_press_per_1000'];
    }
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: 'Noto Sans KR', 'Malgun Gothic', 'Segoe UI', sans-serif; background: #f0f0f0; padding: 5px; }
        ...
        .dashboard-menu a { padding: 4px 10px; background: white; color: #4472C4; text-decoration: none; border: 1px solid #d0d0d0; font-size: 13px; font-weight: 600; }
        ...
        .form-label { font-weight: 600; color: #444; font-size: 13px; background: #F2F2F2; padding: 2px 4px; margin-bottom: 2px; }
        .form-label-inline { font-weight: 600; color: #444; font-size: 13px; background: #F2F2F2; padding: 2px 4px; }
        .form-input, .form-select { width: 100%; padding: 2px 4px; border: 1px solid #d0d0d0; font-size: 13px; }
        ...
        .result-label { color: #444; font-size: 13px; font-weight: 600; }
        .result-value { font-weight: 600; color: #000; font-size: 13px; text-align: right; }
        ...
        .info-box p { font-size: 12px; color: #000; line-height: 1.3; margin: 0; }
...
<body>
    ...
    <?php if (!empty($result)): ?>
    <!-- 견적서 출력용 (이미지 양식과 동일) -->
    <div class="quote-print" id="quotePrint" style="display: none;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 8px; font-size: 13px;">
            ...
            <tr>
                <td colspan="2" rowspan="2" style="border: 1px solid #000; padding: 6px; text-align: center; font-weight: bold; font-size: 20px; vertical-align: middle;">
                    합계금액 (부가세포함)
                </td>
            </tr>
            <tr></tr>
    ...
    function printQuote() {
        // 견적서 내용 가져오기
        var quotePrint = document.getElementById('quotePrint');
        if (!quotePrint) return;

        // 견적서 내용 복사 (display:none 제거)
        var quoteContent = quotePrint.innerHTML;

        // 팝업 창 열기
        var popupWin = window.open('', '_blank', 'width=800,height=1000,scrollbars=yes');
        if (!popupWin) {
