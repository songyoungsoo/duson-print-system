...
// 인증 (로컬에서는 생략됨)
if (isset($_GET['password']) && $_GET['password'] === 'duson2026!kb') {
    $_SESSION['kb_auth'] = true;
} else if (!isset($_SESSION['kb_auth'])) {
    die("비밀번호 필요: ?password=duson2026!kb");
}
...
$tags = "DNS,도메인,Plesk,Tucows,네임서버,변경";
$category = "서버";

// DB에 삽입
$sql = "INSERT INTO knowledge_base (title, content, tags, category, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())";
$stmt = mysqli_prepare($db, $sql);
mysqli_stmt_bind_param($stmt, "ssss", $title, $content, $tags, $category);

if (mysqli_stmt_execute($stmt)) {
    echo "✅ 성공!\n";
    echo "문서 ID: " . mysqli_insert_id($db) . "\n";
    echo "제목: $title\n";
} else {
    echo "❌ 실패!\n";
