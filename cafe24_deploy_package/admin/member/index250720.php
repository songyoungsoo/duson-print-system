...
if ($mode == "PointModlfy") {
    ...
    $db = new mysqli($host, $user, $password, $dataname);
    if ($db->connect_error) {
        die("데이터베이스 연결 실패: " . htmlspecialchars($db->connect_error));
    }
        ...
        <tr>
            <td align="left">
                <table border="0" align="center" width="100%" cellpadding="2" cellspacing="0">
                    <tr>
                        <form method="post" name="TDsearch" onsubmit="return TDsearchCheckField()" action="<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>">
                            <td align="left">
                                <b>간단 검색 :&nbsp;</b>
            ...
            <td align="right">
                <input type="button" value="방문수 ↑" onClick="window.location.href='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&cate=<?php echo  $cate ?>&title_search=<?php echo  $title_search ?>&CountWW=Logincount&s=desc';">
                <input type="button" value="방문수 ↓" onClick="window.location.href='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&cate=<?php echo  $cate ?>&title_search=<?php echo  $title_search ?>&CountWW=Logincount&s=asc';">
                <input type="button" value="Point ↑" onClick="window.location.href='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&cate=<?php echo  $cate ?>&title_search=<?php echo  $title_search ?>&CountWW=money&s=desc';">
                <input type="button" value="Point ↓" onClick="window.location.href='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&cate=<?php echo  $cate ?>&title_search=<?php echo  $title_search ?>&CountWW=money&s=asc';">
            </td>
        </tr>
    </table>
                ...
                if ($rows) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr bgcolor='#575757'>
                            ...
                            <td><a href="#" onClick="window.open('MemberImail.php?no=<?php echo  $row['no'] ?>&code=1', 'member_iemail','width=600,height=500,top=10,left=10,menubar=no,resizable=yes,statusbar=no,scrollbars=yes,toolbar=no');"><font color="white"><?php echo  htmlspecialchars($row['id']) ?></font></a></td>
                            <td><font color="white"><?php echo  htmlspecialchars($row['name']) ?></font></td>
                            <td align="center"><font color="white"><?php echo  htmlspecialchars($row['Logincount']) ?></font></td>
                            <td align="center"><font color="white"><?php echo  htmlspecialchars($row['EndLogin']) ?></font></td>
                            <td align="center"><font color="white"><?php echo  htmlspecialchars($row['date']) ?></font></td>
                            <td align="center">
                                <table border="0" align="center" width="100%" cellpadding="0" cellspacing="0">
                                    <form method="post" action="<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&TDsearch=<?php echo  $TDsearch ?>&TDsearchValue=<?php echo  $TDsearchValue ?>">
                                        <input type="hidden" name="mode" value="PointModlfy">
                                        <input type="hidden" name="no" value="<?php echo  $row['no'] ?>">
                                        <tr>
                                            <td align="center"><input type="text" name="money" size="7" value="<?php echo  htmlspecialchars($row['money']) ?>"><input type="submit" value="수정"></td>
                                        </tr>
                                    </form>
                                </table>
                            </td>
...
<select onChange="LevelModify(<?php echo  $row['no'] ?>, this)">
    <option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&TDsearch=<?php echo  htmlspecialchars($TDsearch) ?>&TDsearchValue=<?php echo  htmlspecialchars($TDsearchValue) ?>&mode=LevelModify&code=2&no=<?php echo  $row['no'] ?>' <?php echo  $row['level'] == "2" ? "selected" : "" ?>>2 레벨-부운영자</option>
    <option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&TDsearch=<?php echo  htmlspecialchars($TDsearch) ?>&TDsearchValue=<?php echo  htmlspecialchars($TDsearchValue) ?>&mode=LevelModify&code=3&no=<?php echo  $row['no'] ?>' <?php echo  $row['level'] == "3" ? "selected" : "" ?>>3 레벨-골드회원</option>
    <option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&TDsearch=<?php echo  htmlspecialchars($TDsearch) ?>&TDsearchValue=<?php echo  htmlspecialchars($TDsearchValue) ?>&mode=LevelModify&code=4&no=<?php echo  $row['no'] ?>' <?php echo  $row['level'] == "4" ? "selected" : "" ?>>4 레벨-정회원</option>
    <option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?offset=<?php echo  $offset ?>&TDsearch=<?php echo  htmlspecialchars($TDsearch) ?>&TDsearchValue=<?php echo  htmlspecialchars($TDsearchValue) ?>&mode=LevelModify&code=5&no=<?php echo  $row['no'] ?>' <?php echo  $row['level'] == "5" ? "selected" : "" ?>>5 레벨-일반회원</option>
</select>

                            </td>
                            <td align="center">
                                <input type="button" onClick="popup=window.open('admin.php?mode=view&no=<?php echo  $row['no'] ?>', 'MemberModify','width=650,height=600,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes'); popup.focus();" value="회원정보보기">
                                <input type="button" onClick="Member_Admin_Del('<?php echo  $row['no'] ?>');" value=" 탈퇴 ">
                            </td>
                        </tr>
