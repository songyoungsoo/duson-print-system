...
if (!defined('PMA_RELATION_LIB_INCLUDED')){
     ...
     function PMA_query_as_cu($sql, $show_error = TRUE) {
        ...
        if (isset($dbh)) {
            ...
            if (!$result && $show_error == TRUE) {
                PMA_mysqlDie(mysqli_error($dbh), $sql, '', $err_url_0);
            }
    ...
    function PMA_getRelationsParam($verbose = FALSE)
    {
           ...
           || empty($cfg['Server']['pmadb'])) {
            if ($verbose == TRUE) {
                echo 'PMA Database ... '
                     ...
                     . '[ <a href="Documentation.php#pmadb">' . $GLOBALS['strDocu'] . '</a> ]<br />' . "\n"
        ...
        if ($tab_rs) {
            mysqli_free_result($tab_rs);
        } else {
            $cfg['Server']['pmadb'] = FALSE;
        }

        if ($verbose == TRUE) {
            $shit     = '<font color="red"><b>' . $GLOBALS['strNotOK'] . '</b></font> [ <a href="Documentation.php#%s">' . $GLOBALS['strDocu'] . '</a> ]';
