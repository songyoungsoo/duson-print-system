...
/**
 * 스티커 AJAX 가격 계산 API
 * 기존 calculate_price.php와 동일한 로직 사용
...
// 스티커 가격 계산 함수 (기존 calculate_price.php와 동일)
function calculateStickerPrice($jong, $garo, $sero, $mesu, $uhyung, $domusong, $connect) {
    // 재질 코드 추출
    $j1 = substr($jong, 0, 3);
    $j = substr($jong, 4, 10);
