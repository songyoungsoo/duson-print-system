...
<body>
    ...
    <div class="compact-container">
        ...
        <div class="main-content">
            ...
            <div class="gallery-section">
                <div class="gallery-title">🖼️ 카다록/리플렛 샘플 갤러리</div>
                ...
                <div class="gallery-more-button" style="text-align: center; margin-top: 20px;">
                    <button type="button" class="btn-primary" onclick="openUnifiedModal('카탈로그', '📚')">
                        📂 더 많은 샘플 보기
                    </button>
                </div>
            </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            loadCadarokThumbnails();
        });
        
        async function loadCadarokThumbnails() {
            const thumbnailItems = document.querySelectorAll('.thumbnail-item');
            
            try {
                // 카다록과 리플렛 카테고리 모두에서 이미지 가져오기
                const [cadarokResponse, leafletResponse] = await Promise.all([
                    fetch('/api/get_real_orders_portfolio.php?category=cadarok&per_page=2'),
                    fetch('/api/get_real_orders_portfolio.php?category=leaflet&per_page=2')
                ]);
                ...
                if (cadarokData.success && cadarokData.data) {
                    combinedImages = combinedImages.concat(cadarokData.data);
                }
                if (leafletData.success && leafletData.data) {
                    combinedImages = combinedImages.concat(leafletData.data);
                }
                
                // 최대 4개까지만 표시
                combinedImages = combinedImages.slice(0, 4);
                ...
                if (combinedImages.length > 0) {
                    thumbnailItems.forEach((item, index) => {
                        ...
                        if (combinedImages[index]) {
                            const img = document.createElement('img');
                            img.src = combinedImages[index].path;
                            img.alt = combinedImages[index].title || '카다록/리플렛 샘플';
                            img.onclick = () => openUnifiedModal('카탈로그', '📚');
                            item.appendChild(img);
                        }
                    });
                } else {
                    // 샘플 이미지가 없으면 기본 이미지 표시
                    loadDefaultCadarokThumbnails();
                }
            } catch (error) {
        ...
        function loadDefaultCadarokThumbnails() {
            const thumbnailItems = document.querySelectorAll('.thumbnail-item');
            const defaultImages = [
                '/images/samples/cadarok1.jpg',
                '/images/samples/cadarok2.jpg',
                '/images/samples/leaflet1.jpg',
                '/images/samples/leaflet2.jpg'
            ];
            ...
            thumbnailItems.forEach((item, index) => {
                ...
                const img = document.createElement('img');
                img.src = defaultImages[index] || '/images/placeholder.jpg';
                img.alt = '카다록/리플렛 샘플 ' + (index + 1);
                img.onclick = () => openUnifiedModal('카탈로그', '📚');
