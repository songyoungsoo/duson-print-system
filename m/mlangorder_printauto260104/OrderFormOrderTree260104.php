                            ...
                            <tr>
                                ...
                                <td style="border: 0.3pt solid #000; padding: 1.5mm; text-align: center;">
                                    ...
                                    if (isset($is_flyer) && $is_flyer && $mesu_for_display > 0) {
                                        // 전단지: "X연 (Y매)" 형식 (0.5만 소수점, 나머지 정수)
                                        $yeonsu = floatval($quantity_num);
                                        if ($yeonsu == 0.5) {
                                            $yeon_display = '0.5';
                                        } else {
                                            $yeon_display = number_format(intval($yeonsu), 0);
                                        }
                                        echo $yeon_display . '연 (' . number_format(intval($mesu_for_display), 0) . '매)';
                                    } else {
                                        // 기타 제품: 항상 정수 표시
                                        echo $quantity_num ? number_format(intval($quantity_num), 0) : '-';
                                    }
                                    ?>
                                </td>
                                <td style="border: 0.3pt solid #000; padding: 1.5mm; text-align: center;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="border: 0.3pt solid #000; padding: 1.5mm; text-align: center;">
                                    ...
                                    // 전단지 매수 추출
                                    $mesu_value = 0;
                                    if (isset($is_flyer) && $is_flyer && isset($json_data)) {
                                        $mesu_value = intval($json_data['quantityTwo'] ?? $json_data['mesu'] ?? $summary_item['mesu'] ?? 0);
                                    }

                                    if (isset($is_flyer) && $is_flyer && $mesu_value > 0) {
                                        // 전단지: "X연 (Y매)" 형식 (0.5만 소수점, 나머지 정수)
                                        $yeonsu = floatval($quantity_num);
                                        if ($yeonsu == 0.5) {
                                            $yeon_display = '0.5';
                                        ...
                                        echo $yeon_display . '연 (' . number_format($mesu_value, 0) . '매)';
                                    } else {
                                        // 기타 제품: 항상 정수 표시
                                        echo $quantity_num ? number_format(intval($quantity_num), 0) : '-';
                                    }
                                        ...
                                        foreach ($order_rows as $summary_item):
                                            ...
                                            if (!empty($quantity_num)) {
                                                if ($is_flyer && !empty($mesu_for_display) && $mesu_for_display > 0) {
                                                    ...
                                                    $quantity_display = $yeon_display . '연 (' . number_format(intval($mesu_for_display), 0) . '매)';
                                                    $unit = '-'; // 단위 칸에 '-' 표시 (수량에 통합)
                                                } else {
                                                    // 기타 제품: 항상 정수 표시
                                                    $quantity_display = number_format(intval($quantity_num), 0);
                                                }
                                            } else {
                                                $quantity_display = '-';
                                            }
