...
// 🔧 FIX: 16개 파라미터 타입 수정 - additional_options(s=JSON string), additional_options_total(i=integer)
// session_id(s) + product_type(s) + MY_type(s) + PN_type(s) + MY_Fsd(s) + MY_amount(s) + POtype(s) + ordertype(s) + st_price(i) + st_price_vat(i) + additional_options(s) + additional_options_total(i) + mesu(i) + ImgFolder(s) + ThingCate(s) + uploaded_files(s)
mysqli_stmt_bind_param($stmt, "ssssssssiisiisss",
