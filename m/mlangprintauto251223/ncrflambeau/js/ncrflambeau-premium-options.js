/**
 * 양식지(NCR) 추가 옵션 JavaScript 모듈
 *
 * 목적: 넘버링, 미싱 등 추가 옵션의 동적 처리
 * 특징: 실시간 가격 계산 및 UI 업데이트 (전단지 스타일 적용)
 *
 * @version 1.1
 * @date 2025-10-12
...
class NcrFlambeauPremiumOptionsManager {
    constructor() {
        this.basePrices = {
            folding: {
                'numbering': 0         // 넘버링 - 전화 문의 (가격 별도)
            },
            creasing: {
                '1': 30000,            // 1줄 미싱
                '2': 30000,            // 2줄 미싱
        ...
        this.currentQuantity = 500; // 기본 수량 (양식지는 500매 기준)
        this.init();
    }
    ...
    calculateAndUpdatePrice() {
        ...
        const multiplier = Math.max(quantity / 500, 1); // 500매 기준 배수 계산
        ...
        if (foldingEnabled) {
            ...
            if (foldingType && this.basePrices.folding[foldingType]) {
                ...
                console.log(`🔢 넘버링 (${foldingType}): ${price.toLocaleString()}원`);
            } else {
        ...
        if (creasingEnabled) {
            ...
            if (creasingLines && this.basePrices.creasing[creasingLines]) {
                ...
                console.log(`📏 미싱 (${creasingLines}줄): ${price.toLocaleString()}원`);
            } else {
        ...
        // 메인 가격 계산 함수 호출 (양식지 전용 - 자동 계산)
        if (typeof window.autoCalculatePrice === 'function') {
            console.log('🔄 메인 가격 계산 함수 호출');
            window.autoCalculatePrice();
        } else {
            console.warn('⚠️ autoCalculatePrice 함수를 찾을 수 없습니다');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('premiumOptionsSection')) {
        window.ncrPremiumOptions = new NcrFlambeauPremiumOptionsManager();
        console.log('✅ 양식지 추가 옵션 시스템 준비 완료');
    }
});
