<?php
/**
 * 스티커 견적안내 시스템 - 명함 시스템 기반 적용 (깔끔한 디자인 테스트)
 * Features: 포트폴리오 갤러리, 수식 기반 실시간 가격 계산, 드래그 업로드
 * 원본 HTML 구조 + 깔끔한 CSS 적용
 */
...
// CommonGallery 시스템 사용
require_once '../../includes/CommonGallery.php';
...
$page_title = generate_page_title("스티커 견적안내 - 깔끔한 디자인 테스트");
...
<head>
    ...
    <style>
        ...
        /* 디밍된 상태에서의 placeholder 색상 */
        .inline-input.dimmed::placeholder {
        ...
        .inline-input.dimmed::-webkit-input-placeholder {
            color: #f0f0fa !important;
            opacity: 0.9 !important;
            font-weight: 400 !important;
            font-family: 'Noto Sans KR', sans-serif !important;
        ...
        .inline-input.dimmed::-moz-placeholder {
            color: #f0f0fa !important;
            opacity: 0.9 !important;
            font-weight: 400 !important;
            font-family: 'Noto Sans KR', sans-serif !important;
        ...
        .inline-input.dimmed:-ms-input-placeholder {
            color: #f0f0fa !important;
            opacity: 0.9 !important;
            font-weight: 400 !important;
            font-family: 'Noto Sans KR', sans-serif !important;
        ...
        /* 스티커 전용 라벨 정렬 오버라이드 */
        .inline-label {
            text-align: left !important; /* 스티커는 왼쪽 정렬 유지 */
        }
...
<body>
    <div class="page-wrapper">
        ...
        <div class="compact-container test-design-mode">
    ...
    <script>
        ...
        function openProofPopup(category) {
            const popup = window.open('/popup/proof_gallery.php?cate=' + encodeURIComponent(category),
                'proof_popup',
                'width=1200,height=800,scrollbars=yes,resizable=yes,top=' + ((screen.height - 800) / 2) + ',left=' + ((screen.width - 1200) / 2));
    ...
    <!-- CommonGallery JavaScript 포함 -->
    <?php echo CommonGallery::renderScript(); ?>

    <!-- 스티커 전용 5-썸네일 갤러리 오버라이드 -->
    <script>
        // 기존 initCommonGallery를 스티커 전용으로 덮어씀
        document.addEventListener('DOMContentLoaded', function() {
            // 기존 CommonGallery 초기화 대신 스티커 전용 함수 실행
            setTimeout(() => {
                initStickerGalleryWith5Thumbs('stickerGallery_sticker_new', 'sticker_new', '스티커');
            }, 100);
        });

        // 스티커 전용 갤러리 함수 (5개 썸네일, 마지막 하나는 더보기 버튼)
        async function initStickerGalleryWith5Thumbs(containerId, category, categoryLabel) {
            console.log('🏷️ 스티커 5-썸네일 갤러리 초기화:', {containerId, category, categoryLabel});

            try {
                // API에서 5개 이미지 로드
                const response = await fetch(`/api/get_real_orders_portfolio.php?category=${category}&per_page=5`, {
                    method: 'GET',
                    headers: {
                ...
                console.log('📊 스티커 API 응답 데이터:', data);

                if (data.success && data.data && Array.isArray(data.data) && data.data.length > 0) {
                    console.log(`✅ ${data.data.length}개 ${categoryLabel} 이미지 발견!`);
                    renderStickerGalleryWith5Thumbs(containerId, data.data, categoryLabel);
                } else {
                    console.warn('⚠️ API에서 유효한 데이터를 받지 못함:', data);
                    showStickerPlaceholder(containerId, categoryLabel);
                }
            } catch (error) {
                console.error('❌ 스티커 갤러리 API 호출 실패:', error);
                showStickerPlaceholder(containerId, categoryLabel);
            }
        }

        // 5개 썸네일 렌더링 (4개 썸네일 + 1개 더보기 버튼)
        function renderStickerGalleryWith5Thumbs(containerId, images, categoryLabel) {
            console.log('🎨 스티커 5-썸네일 갤러리 렌더링 시작:', {containerId, images: images.length});
            ...
            for (let i = 0; i < Math.min(4, validImages.length); i++) {
                ...
                const title = img.title || `${categoryLabel} 샘플 ${i + 1}`;
                const isActive = i === 0;
                ...
                thumbsHtml.push(`
                    <div class="thumb ${isActive ? 'active' : ''}"
                         ...
                         onclick="selectStickerThumb(this, '${containerId}')">
            ...
            // 5번째 자리에 더보기 버튼
            thumbsHtml.push(`
                <div class="thumb more-button"
                     onclick="openStickerGalleryModal()"
                     role="button"
                     tabindex="0"
                     aria-label="더 많은 스티커 샘플 보기">
            ...
            console.log(`✅ 스티커 5-썸네일 갤러리 렌더링 완료 - ${validImages.length}개 이미지 + 더보기 버튼`);
        }

        // 스티커 썸네일 선택
        function selectStickerThumb(thumbElement, containerId) {
            const imagePath = thumbElement.dataset.img;
            const zoomBox = document.getElementById(containerId + '_zoomBox');

            if (zoomBox && imagePath) {
                zoomBox.style.backgroundImage = `url("${imagePath}")`;

                // 모든 썸네일 비활성화
                document.querySelectorAll(`#${containerId}_thumbs .thumb`).forEach(thumb => {
                    thumb.classList.remove('active');
                });

                // 현재 썸네일 활성화 (더보기 버튼이 아닌 경우에만)
                if (!thumbElement.classList.contains('more-button')) {
                    thumbElement.classList.add('active');
                }

                console.log('🖼️ 스티커 썸네일 선택:', imagePath);
            }
        }

        // 더보기 버튼 클릭 시 실행
        function openStickerGalleryModal() {
            console.log('📸 스티커 전체 갤러리 열기');
            openProofPopup('스티커');
        }

        // 플레이스홀더 표시
        function showStickerPlaceholder(containerId, categoryLabel) {
            const thumbsContainer = document.getElementById(containerId + '_thumbs');
            const zoomBox = document.getElementById(containerId + '_zoomBox');

            if (thumbsContainer) {
                thumbsContainer.innerHTML = '<div class="gallery-error">현재 표시할 스티커 샘플이 없습니다.</div>';
            }
            if (zoomBox) {
