...
<body>
                ...
                <table class="items-table" id="itemsTable">
                    <thead>
                        <tr>
                            ...
                            <th style="width:120px;">총액 *</th>
                            <th style="width:50px;">삭제</th>
                        </tr>
                    </thead>
                    <tbody id="itemsBody">
                        <?php foreach ($items as $idx => $item):
                            $totalPrice = intval($item['total_price'] ?? ($item['quantity'] * $item['unit_price']));
                        ?>
                        <tr class="item-row">
                            ...
                            <td><input type="number" name="items[<?php echo $idx; ?>][total_price]" class="total-price-input" required min="0" value="<?php echo $totalPrice; ?>" onchange="updateGrandTotal()"></td>
    ...
    function addItemRow() {
        ...
        row.innerHTML = `
            ...
            <td><input type="number" name="items[${itemCounter}][total_price]" class="total-price-input" required min="0" value="0"></td>
            <td><button type="button" class="btn-remove" onclick="removeRow(this)">삭제</button></td>
        `;
    ...
    // 총액 입력 시 호출 (필요 시 추가 기능 구현 가능)
    function updateGrandTotal() {
        // 현재는 자동 계산 없이 직접 입력 방식
        // 필요 시 여기에 총 합계 자동 계산 로직 추가 가능
    }
