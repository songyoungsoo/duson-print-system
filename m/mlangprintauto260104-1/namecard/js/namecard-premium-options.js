 ...
 * @version 1.0
 * @date 2025-01-15
 * @author SuperClaude Premium Options System
 */

class NameCardPremiumOptionsManager {
    constructor() {
        this.basePrices = {
            foil: {
     ...
     */
    init() {
        console.log('🔧 명함 프리미엄 옵션 시스템 초기화');
    ...
    calculateAndUpdatePrice() {
        ...
        // 메인 가격 계산 함수 호출 (명함 전용) - 자동 모드로 호출
 ...
 * 프리미엄 옵션 시스템 초기화
 */
function initNameCardPremiumOptions() {
    if (!premiumOptionsManager) {
        premiumOptionsManager = new NameCardPremiumOptionsManager();
    }
...
/**
 * calculatePremiumOptions 별칭 함수 (namecard-compact.js 호환성)
 * 명함 전용 프리미엄 옵션 총액을 반환
 */
function calculatePremiumOptions() {
    return getPremiumOptionsTotal();
}

/**
 * initializePremiumOptionsListeners 별칭 함수 (namecard-compact.js 호환성)
 * 프리미엄 옵션 이벤트 리스너 초기화
 */
function initializePremiumOptionsListeners() {
    console.log('🔧 initializePremiumOptionsListeners 호출됨');
    if (!premiumOptionsManager) {
        premiumOptionsManager = initNameCardPremiumOptions();
    }
    return premiumOptionsManager;
}

// 전역 함수로 내보내기 (다른 스크립트에서 사용 가능)
window.NameCardPremiumOptionsManager = NameCardPremiumOptionsManager;
window.initNameCardPremiumOptions = initNameCardPremiumOptions;
window.getPremiumOptionsTotal = getPremiumOptionsTotal;
window.calculatePremiumOptions = calculatePremiumOptions;
window.initializePremiumOptionsListeners = initializePremiumOptionsListeners;
