                ...
                ?>
                    <tr>
                        ...
                        <td>
                            <?php
                            // 두손기획 비즈니스 규칙: 수량 표기
                            $is_flyer_summary = in_array($item['product_type'], ['inserted', 'leaflet']);

                            if ($is_flyer_summary) {
                                // 전단지/리플렛: "X연 (Y,YYY매)"
                                $yeonsu = !empty($item['MY_amount']) ? floatval($item['MY_amount']) : 1;
                                $mesu = $item['flyer_mesu'] ?? $item['mesu'] ?? null;
                                ...
                                if (!empty($mesu)) {
                                    echo $yeon_display . '연 (' . number_format(intval($mesu), 0) . '매)';
                                } else {
                                    echo $yeon_display . '연';
                                }
                            } else {
                                ...
                                echo number_format($qty_val, 0) . $unit;
                            }
                            ?>
                        </td>
                        <td class="text-right">
