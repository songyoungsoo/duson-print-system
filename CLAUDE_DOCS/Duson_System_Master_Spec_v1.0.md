...
| 날짜 | 수정자 | 수정항목 | 관련 파일 |
|------|--------|----------|-----------|
| 2026-01-14 | Claude | [공표] 품목 매핑 사전 통합 | 전체 |
...
> **이 섹션은 시스템 전체를 관통하는 최상위 규정입니다.**

### 상위 원칙
| 원칙 | 설명 |
|------|------|
| **SSOT** | 모든 계산과 출력은 중앙 집중형 공통 함수에서만 처리 |
| **레거시 격리** | 과거 땜질식 코드/문서/수동 계산 로직 완전 도려냄 |
| **데이터 원자화** | 수량(숫자)과 단위(코드)를 DB에 분리 저장 |

### [필독] 전사 표준 품목 매핑 사전 (9개 제품)
