# 레거시 업로드/다운로드 시스템 구현 가이드

**작성일**: 2025년 11월 18일
**대상 제품**: inserted (전단지), ncrflambeau (양식지)
**작업 완료**: ✅ 로컬 개발 완료 및 실서버 배포 완료

---

## 📋 목차

1. [개요](#개요)
2. [업로드 경로 형식](#업로드-경로-형식)
3. [구현된 파일 목록](#구현된-파일-목록)
4. [상세 구현 내용](#상세-구현-내용)
5. [데이터베이스 구조](#데이터베이스-구조)
6. [관리자 페이지 연동](#관리자-페이지-연동)
7. [FTP 배포 과정](#ftp-배포-과정)
8. [테스트 방법](#테스트-방법)
9. [문제 해결](#문제-해결)

---

## 개요

### 목적
기존 주문 시스템에서 사용하던 레거시 업로드 경로 형식을 `inserted`(전단지)와 `ncrflambeau`(양식지) 제품에 적용하여, 일관된 파일 관리 및 관리자 다운로드 기능 제공

### 주요 기능
- **계층적 디렉토리 구조**: 제품명/연도/월일/IP/타임스탬프
- **원본 파일명 추적**: JSON 매핑으로 타임스탬프 파일명 ↔ 원본 파일명 관리
- **관리자 다운로드**: 원본 파일명으로 복원하여 제공
- **통합 시스템**: 기존 admin.php 및 download.php와 완벽 호환

---

## 업로드 경로 형식

### 경로 구조
```
ImgFolder/_MlangPrintAuto_{product}_index.php/{YYYY}/{MMDD}/{IP}/{timestamp}/
```

### 실제 예시

#### 전단지 (inserted)
```
ImgFolder/_MlangPrintAuto_inserted_index.php/2025/1117/222.108.84.120/1763351602/
├── 1763351602.pdf      ← 고객원본파일.pdf
├── 1763351603.jpg      ← 디자인시안.jpg
└── 1763351604.ai       ← 최종디자인.ai
```

#### 양식지 (ncrflambeau)
```
ImgFolder/_MlangPrintAuto_ncrflambeau_index.php/2025/1117/222.108.84.120/1763351800/
├── 1763351800.pdf      ← 양식견본.pdf
└── 1763351801.jpg      ← 참고이미지.jpg
```

### 경로 요소 설명
- **ImgFolder**: 루트 업로드 디렉토리 (고정)
- **_MlangPrintAuto_{product}_index.php**: 제품별 구분자
- **YYYY**: 4자리 연도 (예: 2025)
- **MMDD**: 월일 4자리 (예: 1117)
- **IP**: 업로드한 사용자 IP 주소
- **timestamp**: Unix 타임스탬프 (time() 함수 결과)

---

## 구현된 파일 목록

### 1. 공통 설정 파일
#### `/includes/upload_config.php`
**새로 추가된 함수**:
```php
// 레거시 업로드 경로 생성
function generateLegacyUploadPath($product_type)

// 레거시 디렉토리 생성 및 권한 설정
function createLegacyUploadDirectory($physical_path)
```

**파일 크기**: 5,252 bytes

### 2. 전단지 시스템
#### `/mlangprintauto/inserted/add_to_basket.php`
**주요 변경사항**:
- SimpleUploadHandler 제거
- 레거시 경로 생성 로직 추가
- 타임스탬프 기반 파일명 저장
- INSERT 쿼리에 ImgFolder, ThingCate, original_filename 필드 추가

**파일 크기**: 4,755 bytes

### 3. 양식지 시스템
#### `/mlangprintauto/ncrflambeau/add_to_basket.php`
**주요 변경사항**:
- UploadPathHelper 제거
- 레거시 경로 생성 로직 추가
- INSERT 쿼리 15개 → 16개 파라미터로 확장

**파일 크기**: 8,239 bytes

### 4. 관리자 페이지 (기존 파일 수정)
#### `/admin/mlangprintauto/admin.php`
**수정 위치**:
- Lines 721-735: ImgFolder 경로 감지 로직 개선
- Lines 832-876: 레거시 경로 파일 표시 및 다운로드 링크 생성

#### `/admin/mlangprintauto/download.php`
**기능**:
- Lines 26-53: 원본 파일명 조회 (original_filename JSON)
- Line 133: 원본 파일명으로 다운로드 제공

---

## 상세 구현 내용

### 1. 레거시 경로 생성 함수

#### `generateLegacyUploadPath($product_type)`
```php
/**
 * 레거시 형식 업로드 경로 생성
 *
 * @param string $product_type 제품 타입 (inserted, ncrflambeau 등)
 * @return array ['img_folder' => 경로, 'physical_path' => 실제 경로]
 */
function generateLegacyUploadPath($product_type) {
    $ip = $_SERVER['REMOTE_ADDR'] ?? '127.0.0.1';
    $timestamp = time();
    $year = date('Y');
    $month_day = date('md'); // MMDD 형식

    // IPv6 localhost 처리
    if ($ip === '::1') {
        $ip = '127.0.0.1';
    }

    // 경로 구조 생성
    $folder_name = "_MlangPrintAuto_{$product_type}_index.php";
    $img_folder = "ImgFolder/{$folder_name}/{$year}/{$month_day}/{$ip}/{$timestamp}/";

    // 실제 파일시스템 경로
    $physical_path = __DIR__ . '/../' . $img_folder;

    return [
        'img_folder' => $img_folder,
        'physical_path' => $physical_path
    ];
}
```

**특징**:
- 자동으로 연도/월일 계산
- IP 주소 정규화 (IPv6 localhost 처리)
- 상대 경로와 절대 경로 모두 반환

---

### 2. 디렉토리 생성 함수

#### `createLegacyUploadDirectory($physical_path)`
```php
/**
 * 레거시 업로드 디렉토리 생성
 *
 * @param string $physical_path 실제 파일시스템 경로
 * @return bool 성공 여부
 */
function createLegacyUploadDirectory($physical_path) {
    if (!file_exists($physical_path)) {
        if (!mkdir($physical_path, 0755, true)) {
            error_log("Failed to create directory: {$physical_path}");
            return false;
        }
        chmod($physical_path, 0777); // 웹서버 쓰기 권한
    }
    return true;
}
```

**특징**:
- 중첩 디렉토리 자동 생성 (`mkdir` 3번째 파라미터 `true`)
- 권한 0777 설정으로 웹서버 쓰기 보장
- 실패 시 error_log 기록

---

### 3. 파일 업로드 처리 로직

#### 전단지 (inserted) 예시
```php
// 레거시 경로 생성
$upload_path_info = generateLegacyUploadPath('inserted');
$img_folder = $upload_path_info['img_folder'];
$physical_path = $upload_path_info['physical_path'];

// 디렉토리 생성
if (!createLegacyUploadDirectory($physical_path)) {
    safe_json_response(false, null, '업로드 디렉토리 생성 실패');
}

// 파일 업로드 처리
$uploaded_files = [];
$original_filename_map = [];

if (!empty($_FILES['uploaded_files'])) {
    $files = $_FILES['uploaded_files'];
    $file_count = is_array($files['name']) ? count($files['name']) : 1;

    for ($i = 0; $i < $file_count; $i++) {
        $file_name = is_array($files['name']) ? $files['name'][$i] : $files['name'];
        $file_tmp = is_array($files['tmp_name']) ? $files['tmp_name'][$i] : $files['tmp_name'];
        $file_error = is_array($files['error']) ? $files['error'][$i] : $files['error'];

        if ($file_error === UPLOAD_ERR_OK) {
            // 타임스탬프 기반 파일명
            $timestamp = time() + $i;
            $ext = pathinfo($file_name, PATHINFO_EXTENSION);
            $saved_name = $timestamp . '.' . $ext;
            $destination = $physical_path . $saved_name;

            if (move_uploaded_file($file_tmp, $destination)) {
                $uploaded_files[] = [
                    'original' => $file_name,
                    'saved' => $saved_name,
                    'path' => $img_folder . $saved_name
                ];
                $original_filename_map[$saved_name] = $file_name;
            }
        }
    }
}
```

**핵심 포인트**:
- `time() + $i`: 여러 파일 동시 업로드 시 타임스탬프 중복 방지
- `$original_filename_map`: 타임스탬프 파일명 → 원본 파일명 매핑
- 단일/다중 파일 모두 처리 가능 (`is_array()` 체크)

---

### 4. 데이터베이스 저장

#### INSERT 쿼리 (전단지)
```php
$original_filename_json = json_encode($original_filename_map, JSON_UNESCAPED_UNICODE);
$thing_cate = !empty($uploaded_files) ? $uploaded_files[0]['saved'] : '';

$sql = "INSERT INTO shop_temp (
    session_id, product_type, MY_type, PN_type, MY_Fsd, MY_amount,
    POtype, ordertype, st_price, st_price_vat, additional_options,
    additional_options_total, ImgFolder, ThingCate, original_filename
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

mysqli_stmt_bind_param($stmt, "ssssssssiisisss",
    $session_id, $product_type, $MY_type, $PN_type, $MY_Fsd, $MY_amount,
    $POtype, $ordertype, $price, $vat_price, $additional_options_json,
    $additional_options_total, $img_folder, $thing_cate, $original_filename_json
);
```

#### INSERT 쿼리 (양식지)
```php
$insert_query = "INSERT INTO shop_temp (
    session_id, product_type, MY_type, MY_Fsd, PN_type, MY_amount,
    ordertype, st_price, st_price_vat, premium_options, premium_options_total,
    work_memo, upload_method, ImgFolder, ThingCate, original_filename
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

mysqli_stmt_bind_param($stmt, "sssssssisissssss",
    $session_id, $product_type, $MY_type, $MY_Fsd, $PN_type, $MY_amount,
    $ordertype, $price, $vat_price, $additional_options_json, $additional_total,
    $work_memo, $upload_method, $img_folder, $thing_cate, $original_filename_json
);
```

**파라미터 수 차이**:
- 전단지: 15개 (POtype 포함)
- 양식지: 16개 (work_memo, upload_method 포함)

---

## 데이터베이스 구조

### shop_temp 테이블 주요 필드

| 필드명 | 타입 | 설명 | 예시 |
|--------|------|------|------|
| `ImgFolder` | VARCHAR(255) | 업로드 경로 | `ImgFolder/_MlangPrintAuto_inserted_index.php/2025/1117/222.108.84.120/1763351602/` |
| `ThingCate` | VARCHAR(255) | 대표 파일명 (첫 번째 파일) | `1763351602.pdf` |
| `original_filename` | TEXT | 원본 파일명 매핑 (JSON) | `{"1763351602.pdf":"고객원본.pdf","1763351603.jpg":"시안.jpg"}` |

### mlangorder_printauto 테이블
주문 처리 시 `shop_temp`에서 `mlangorder_printauto`로 데이터 복사:
- `ImgFolder`: 그대로 복사
- `ThingCate`: 대표 파일명 복사
- `original_filename`: JSON 매핑 복사

---

## 관리자 페이지 연동

### 1. ImgFolder 경로 감지 (admin.php:721-735)

```php
// ✅ FIX: mlangorder_printauto 테이블에서 ImgFolder 정보 가져오기
if (!empty($row['ImgFolder'])) {
    // 레거시 형식 경로 처리: ImgFolder/_MlangPrintAuto_*_index.php/...
    if (strpos($row['ImgFolder'], 'ImgFolder/') === 0) {
        // 새로운 레거시 경로 형식: ImgFolder로 시작하는 전체 경로
        $sticker_dirs[] = "../../" . $row['ImgFolder'];
    } elseif (strpos($row['ImgFolder'], '_MlangPrintAuto_') === 0) {
        // ImgFolder/ 없이 _MlangPrintAuto_로 시작하는 경우
        $sticker_dirs[] = "../../ImgFolder/" . $row['ImgFolder'];
    } else {
        // 기존 방식 (상대 경로)
        $sticker_dirs[] = "../../" . $row['ImgFolder'];
    }
}
```

**지원하는 경로 형식**:
1. `ImgFolder/_MlangPrintAuto_...` (현재 사용)
2. `_MlangPrintAuto_...` (ImgFolder/ 생략 형식)
3. 기타 상대 경로 (기존 방식)

---

### 2. 파일 목록 표시 및 다운로드 (admin.php:832-876)

```php
// 🔧 레거시 업로드 경로 체크
if (!empty($row['ImgFolder']) &&
    (strpos($row['ImgFolder'], 'ImgFolder/') === 0 ||
     strpos($row['ImgFolder'], '_MlangPrintAuto_') === 0)) {

    // 경로 결정
    if (strpos($row['ImgFolder'], 'ImgFolder/') === 0) {
        $dir_path = "../../" . $row['ImgFolder'];
        $download_base_path = $row['ImgFolder'];
    } else {
        $dir_path = "../../ImgFolder/" . $row['ImgFolder'];
        $download_base_path = "ImgFolder/" . $row['ImgFolder'];
    }

    if (is_dir($dir_path)) {
        echo "<div style='margin-top: 10px; padding: 8px; background: #e8f5e9; border-left: 3px solid #4caf50;'>";
        echo "<strong>✅ 레거시 경로 파일:</strong> " . htmlspecialchars($row['ImgFolder']) . "<br>";

        $dir_handle = opendir($dir_path);
        while ($tmp = readdir($dir_handle)) {
            if ($tmp != "." && $tmp != "..") {
                $download_path = urlencode($download_base_path);
                $file_size = filesize("$dir_path/$tmp");
                $file_size_mb = round($file_size / 1024 / 1024, 2);

                // 대표 파일 아이콘
                $icon = ($tmp == $row['ThingCate']) ? "📌" : "📄";

                echo "$icon <a href='download.php?no=$no&downfile=" . urlencode($tmp) .
                     "&path=$download_path' class='file'>";
                echo htmlspecialchars($tmp) . "</a> ({$file_size_mb}MB)";

                if ($tmp == $row['ThingCate']) {
                    echo " <span style='color: #28a745; font-weight: bold;'>(대표 파일)</span>";
                }
                echo "<br>";
            }
        }
        closedir($dir_handle);
        echo "</div>";
    }
}
```

**화면 표시 예시**:
```
✅ 레거시 경로 파일: ImgFolder/_MlangPrintAuto_inserted_index.php/2025/1117/222.108.84.120/1763351602/
📌 1763351602.pdf (2.5MB) (대표 파일)
📄 1763351603.jpg (1.2MB)
📄 1763351604.ai (5.8MB)
```

---

### 3. 원본 파일명 복원 다운로드 (download.php:26-53)

```php
// 🆕 원본 파일명 조회 (주문번호가 있는 경우)
$original_filename = null;
if (!empty($no) && $connect) {
    $query = "SELECT original_filename FROM mlangorder_printauto WHERE no = ?";
    $stmt = mysqli_prepare($connect, $query);

    if ($stmt) {
        mysqli_stmt_bind_param($stmt, 'i', $no);
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);

        if ($row = mysqli_fetch_assoc($result)) {
            $original_filename_json = $row['original_filename'];

            if (!empty($original_filename_json)) {
                $filename_map = json_decode($original_filename_json, true);

                // $downfile(timestamp 파일명)에 해당하는 원본 파일명 찾기
                if (isset($filename_map[$downfile])) {
                    $original_filename = $filename_map[$downfile];
                    error_log("원본 파일명 발견: {$downfile} → {$original_filename}");
                }
            }
        }
        mysqli_stmt_close($stmt);
    }
}

// 🆕 다운로드 파일명 결정: 원본 파일명이 있으면 사용, 없으면 timestamp 파일명 사용
$download_filename = !empty($original_filename) ? $original_filename : $downfile;
```

**동작 원리**:
1. 주문번호로 `original_filename` JSON 조회
2. JSON 파싱하여 타임스탬프 파일명에 매핑된 원본 파일명 찾기
3. 원본 파일명이 있으면 사용, 없으면 타임스탬프 파일명 그대로 사용
4. HTTP 헤더에 원본 파일명 설정하여 다운로드

---

## FTP 배포 과정

### 업로드 명령어

```bash
# 1. upload_config.php 업로드
curl -T /var/www/html/includes/upload_config.php \
     ftp://dsp1830.shop/includes/ \
     --user dsp1830:ds701018

# 2. inserted/add_to_basket.php 업로드
curl -T /var/www/html/mlangprintauto/inserted/add_to_basket.php \
     ftp://dsp1830.shop/mlangprintauto/inserted/ \
     --user dsp1830:ds701018

# 3. ncrflambeau/add_to_basket.php 업로드
curl -T /var/www/html/mlangprintauto/ncrflambeau/add_to_basket.php \
     ftp://dsp1830.shop/mlangprintauto/ncrflambeau/ \
     --user dsp1830:ds701018
```

### 업로드 결과

| 파일 | 크기 | 상태 |
|------|------|------|
| includes/upload_config.php | 5,252 bytes | ✅ 성공 |
| mlangprintauto/inserted/add_to_basket.php | 4,755 bytes | ✅ 성공 |
| mlangprintauto/ncrflambeau/add_to_basket.php | 8,239 bytes | ✅ 성공 |

---

## 테스트 방법

### 1. 전단지 업로드 테스트

#### 테스트 URL
```
http://dsp1830.shop/mlangprintauto/inserted/index.php
```

#### 테스트 시나리오
1. 제품 옵션 선택 (용지, 사이즈, 수량 등)
2. 파일 업로드 (PDF, JPG 등 여러 개)
3. 장바구니 담기 버튼 클릭
4. 성공 메시지 확인: `"장바구니에 추가되었습니다. (파일 N개 업로드 완료)"`

#### 예상 결과
- 파일 저장 위치: `ImgFolder/_MlangPrintAuto_inserted_index.php/2025/1118/[IP]/[timestamp]/`
- 파일명: `1731900000.pdf`, `1731900001.jpg` (타임스탬프 기반)
- 데이터베이스 `shop_temp` 테이블:
  - `ImgFolder`: 전체 경로 저장
  - `ThingCate`: `1731900000.pdf` (첫 번째 파일)
  - `original_filename`: `{"1731900000.pdf":"고객원본.pdf","1731900001.jpg":"시안.jpg"}`

---

### 2. 양식지 업로드 테스트

#### 테스트 URL
```
http://dsp1830.shop/mlangprintauto/ncrflambeau/index.php
```

#### 테스트 시나리오
1. 양식지 옵션 선택
2. 파일 업로드
3. 장바구니 담기
4. 성공 메시지 확인

#### 예상 결과
- 파일 저장 위치: `ImgFolder/_MlangPrintAuto_ncrflambeau_index.php/2025/1118/[IP]/[timestamp]/`
- 동일한 타임스탬프 파일명 및 JSON 매핑

---

### 3. 관리자 다운로드 테스트

#### 테스트 URL
```
http://dsp1830.shop/admin/mlangprintauto/admin.php?mode=OrderView&no=[주문번호]
```

#### 테스트 시나리오
1. 주문 상세보기 페이지 접속
2. "✅ 레거시 경로 파일" 섹션 확인
3. 파일 목록 표시 확인:
   - 📌 아이콘: 대표 파일
   - 📄 아이콘: 추가 파일
   - 파일 크기 표시
4. 파일 클릭하여 다운로드
5. 다운로드된 파일명이 원본 파일명인지 확인

#### 예상 결과
- 화면에 타임스탬프 파일명 표시: `1731900000.pdf`
- 다운로드 시 원본 파일명으로 저장: `고객원본.pdf`

---

### 4. 디렉토리 구조 확인

#### SSH/FTP 접속 후 확인
```bash
cd /var/www/html/ImgFolder/
ls -la _MlangPrintAuto_inserted_index.php/2025/1118/

# 예상 출력:
# drwxrwxrwx 3 www-data www-data 4096 Nov 18 10:30 222.108.84.120
# cd 222.108.84.120/
# ls -la
# drwxrwxrwx 2 www-data www-data 4096 Nov 18 10:30 1731900000
# cd 1731900000/
# ls -la
# -rw-r--r-- 1 www-data www-data 524288 Nov 18 10:30 1731900000.pdf
# -rw-r--r-- 1 www-data www-data 102400 Nov 18 10:30 1731900001.jpg
```

---

## 문제 해결

### 문제 1: 업로드 디렉토리 생성 실패

**증상**:
```
장바구니 추가 중 오류가 발생했습니다: 업로드 디렉토리 생성 실패
```

**원인**:
- 상위 디렉토리 권한 부족
- 디스크 공간 부족

**해결 방법**:
```bash
# 권한 확인
ls -la /var/www/html/ImgFolder/

# 권한 수정
sudo chmod 777 /var/www/html/ImgFolder/
sudo chown -R www-data:www-data /var/www/html/ImgFolder/
```

---

### 문제 2: 파일이 관리자 페이지에 표시되지 않음

**증상**:
- 업로드는 성공했으나 관리자 페이지에서 파일 목록이 보이지 않음

**원인**:
- `admin.php`가 구버전 (ImgFolder 경로 처리 로직 없음)
- 데이터베이스에 `ImgFolder` 값이 저장되지 않음

**해결 방법**:
```bash
# 1. admin.php 최신 버전 확인
grep "레거시 경로 파일" /var/www/html/admin/mlangprintauto/admin.php

# 2. 있으면 OK, 없으면 admin.php 재배포 필요

# 3. 데이터베이스 확인
mysql -u dsp1830 -p dsp1830
SELECT ImgFolder, ThingCate, original_filename
FROM mlangorder_printauto
WHERE no = [주문번호];
```

---

### 문제 3: 다운로드 시 원본 파일명이 아닌 타임스탬프 파일명으로 저장됨

**증상**:
- 다운로드 클릭 시 `1731900000.pdf` 같은 타임스탬프 파일명으로 저장

**원인**:
- `download.php`가 구버전 (원본 파일명 복원 로직 없음)
- 데이터베이스 `original_filename` 필드가 NULL

**해결 방법**:
```bash
# 1. download.php 최신 버전 확인
grep "원본 파일명 조회" /var/www/html/admin/mlangprintauto/download.php

# 2. 데이터베이스 확인
SELECT original_filename
FROM mlangorder_printauto
WHERE no = [주문번호];

# 3. JSON 형식 확인
# 예상: {"1731900000.pdf":"고객원본.pdf"}
# 실제: NULL 또는 빈 문자열이면 문제
```

---

### 문제 4: INSERT 쿼리 실패 - 파라미터 수 불일치

**증상**:
```
장바구니 추가 실패: Number of elements in type definition string doesn't match number of bind variables
```

**원인**:
- `bind_param` 타입 문자열과 실제 변수 개수 불일치
- 전단지: 15개 파라미터
- 양식지: 16개 파라미터

**해결 방법**:
```php
// 전단지 확인
mysqli_stmt_bind_param($stmt, "ssssssssiisisss", ...);
// "ssssssssiisisss" = 15글자 → 15개 변수 필요

// 양식지 확인
mysqli_stmt_bind_param($stmt, "sssssssisissssss", ...);
// "sssssssisissssss" = 16글자 → 16개 변수 필요

// 타입 문자열:
// s = string
// i = integer
```

---

### 문제 5: 파일 업로드 후 빈 배열 반환

**증상**:
```json
{
  "success": true,
  "data": {
    "basket_id": 123,
    "uploaded_files_count": 0,
    "upload_path": "ImgFolder/..."
  }
}
```

**원인**:
- `$_FILES['uploaded_files']` 비어있음
- 프론트엔드에서 `name="uploaded_files"` 속성 누락
- PHP `upload_max_filesize` 설정 초과

**해결 방법**:
```html
<!-- 프론트엔드 HTML 확인 -->
<input type="file" name="uploaded_files[]" multiple>
<!-- ✅ name 속성이 "uploaded_files[]"인지 확인 -->

<!-- PHP 설정 확인 -->
<?php
echo "upload_max_filesize: " . ini_get('upload_max_filesize') . "<br>";
echo "post_max_size: " . ini_get('post_max_size');
?>
```

---

## 추가 개선 사항 (선택)

### 1. 파일 타입 검증 강화
```php
$allowed_extensions = ['pdf', 'jpg', 'jpeg', 'png', 'gif', 'ai', 'psd'];
$file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));

if (!in_array($file_ext, $allowed_extensions)) {
    safe_json_response(false, null, '허용되지 않은 파일 형식입니다.');
}
```

### 2. 파일 크기 제한
```php
$max_file_size = 50 * 1024 * 1024; // 50MB

if ($_FILES['uploaded_files']['size'][$i] > $max_file_size) {
    safe_json_response(false, null, '파일 크기는 50MB를 초과할 수 없습니다.');
}
```

### 3. 이미지 파일 검증 (위조 방지)
```php
if (in_array($file_ext, ['jpg', 'jpeg', 'png', 'gif'])) {
    $image_info = getimagesize($file_tmp);
    if ($image_info === false) {
        safe_json_response(false, null, '유효하지 않은 이미지 파일입니다.');
    }
}
```

### 4. 로그 기록 강화
```php
error_log(sprintf(
    "[UPLOAD] Product: %s, Session: %s, IP: %s, Files: %d, Total Size: %.2fMB",
    $product_type,
    $session_id,
    $_SERVER['REMOTE_ADDR'],
    count($uploaded_files),
    $total_size / 1024 / 1024
));
```

---

## 참고 자료

### 관련 문서
- [CLAUDE.md](CLAUDE.md) - 전체 프로젝트 구조 문서
- [FTP_SERVER_CLEANUP_GUIDE.md](FTP_SERVER_CLEANUP_GUIDE.md) - FTP 서버 정리 가이드

### PHP 함수 레퍼런스
- [`move_uploaded_file()`](https://www.php.net/manual/en/function.move-uploaded-file.php)
- [`json_encode()`](https://www.php.net/manual/en/function.json-encode.php)
- [`mysqli_stmt_bind_param()`](https://www.php.net/manual/en/mysqli-stmt.bind-param.php)

### 디버깅 팁
```php
// 업로드 디렉토리 확인
var_dump($physical_path);
var_dump(file_exists($physical_path));
var_dump(is_writable($physical_path));

// 파일 업로드 에러 확인
var_dump($_FILES['uploaded_files']['error']);

// JSON 인코딩 확인
var_dump($original_filename_map);
var_dump($original_filename_json);
```

---

## 작업 이력

| 날짜 | 작업 내용 | 담당자 |
|------|----------|--------|
| 2025-11-18 | 레거시 업로드 시스템 설계 및 구현 | Claude |
| 2025-11-18 | 전단지(inserted) 시스템 적용 | Claude |
| 2025-11-18 | 양식지(ncrflambeau) 시스템 적용 | Claude |
| 2025-11-18 | 관리자 페이지 연동 및 다운로드 기능 구현 | Claude |
| 2025-11-18 | 실서버 FTP 배포 완료 | Claude |

---

## 라이선스 및 저작권

**Copyright © 2025 Duson Planning Print (두손기획인쇄)**
All rights reserved.

이 문서는 내부 개발 참고용으로 작성되었습니다.

완료! 이제 테스트해주세요:
http://dsp1830.shop/mlangprintauto/inserted/index.php 접속
옵션 선택
파일 업로드 (PDF, JPG 등)
"장바구니 담기" 클릭
수정 내용:
// ❌ 이전 (잘못된 방식)
formData.append("uploaded_files[" + index + "]", file);

// ✅ 수정 (올바른 방식)  
formData.append("uploaded_files[]", file);
변경 이유:
uploaded_files[0], uploaded_files[1] 방식은 PHP에서 개별 키로 인식됨
uploaded_files[] 방식은 PHP가 자동으로 배열로 인식함
테스트 폼에서 확인한 것처럼 uploaded_files[]가 정상 작동함
이제 파일이 정상적으로 업로드될 것입니다! 결과를 알려주세요.
/ 가 더 들어가야하나요?
테스트 파일 내용 - 2025-11-18 15:40:52
완벽합니다! 🎉 파일 업로드가 성공했습니다! 확인된 내용:
파일명: 테스트 파일 내용
업로드 시간: 2025-11-18 15:40:52
이제 전단지(inserted) 시스템의 파일 업로드가 정상 작동합니다!
다음 단계: 양식지(ncrflambeau)도 동일하게 수정
양식지도 같은 문제가 있을 것이므로 수정하겠습니다:
Grep "formData\.append\("uploaded_files\[" (in /var/www/html/mlangprintauto/ncrflambeau, type: js)
Session limit reached ∙ resets 7pm
