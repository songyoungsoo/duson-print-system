...
// 🔧 FIX: 출력 버퍼링 시작 (header() redirect를 위해 필수!)
ob_start();
...
try {
    ...
    foreach ($cart_items as $item) {
        ...
        switch ($item['product_type']) {
            ...
            case 'cadarok':
                ...
                $product_info = json_encode($cadarok_data, JSON_UNESCAPED_UNICODE);
                break;
            ...
            case 'msticker':
                ...
                $product_info = json_encode($msticker_data, JSON_UNESCAPED_UNICODE);
                break;
            ...
            case 'ncrflambeau':
                ...
                $ncrflambeau_data = [
                    ...
                    'MY_type_name' => $type_name,
                    'MY_Fsd' => $item['MY_Fsd'],
                    'Section_name' => $size_name,
                    'PN_type' => $item['PN_type'],
                    'PN_type_name' => $color_name,
                    'MY_amount' => $qty,
                    'ordertype' => $item['ordertype'],
                    'created_at' => date('Y-m-d H:i:s')
                ];

                $product_info = json_encode($ncrflambeau_data, JSON_UNESCAPED_UNICODE);
                break;
            ...
            case 'merchandisebond':
                ...
                $merchandisebond_data = [
                    ...
                    'POtype_name' => $sides,
                    'PN_type' => $item['PN_type'],
                    'PN_type_name' => $after_name,
                    'MY_amount' => $qty,
                    'ordertype' => $item['ordertype'],
                    'created_at' => date('Y-m-d H:i:s')
                ];

                $product_info = json_encode($merchandisebond_data, JSON_UNESCAPED_UNICODE);
                break;
        ...
        // mlangorder_printauto 테이블에 삽입 (ImgFolder 필드 포함) - Phase B: product_type, is_custom_product 추가
        $insert_query = "INSERT INTO mlangorder_printauto (
            ...
            phone, Hendphone, cont, date, OrderStyle, ThingCate, product_type, is_custom_product,
        ...
        // 🔧 FIX: 변수 초기화 (bind_param "No data supplied" 에러 방지)
        $img_folder_path = '';
        $thing_cate = '';
        ...
        $uploaded_files_json = $item['uploaded_files'] ?? '';

        // 🆕 ImgFolder 경로는 위에서 이미 설정됨 (레거시 경로 또는 기본 경로)
        // $img_folder_path 변수는 이미 lines 403 또는 451에서 설정되었음
        ...
        // 🔧 FIX: 변수를 먼저 정의한 후 문자열로 변환
        $st_price_value = isset($item['st_price']) ? $item['st_price'] : 0;
        $st_price_vat_value = isset($item['st_price_vat']) ? $item['st_price_vat'] : 0;
        $st_price = (string)$st_price_value;
        $st_price_vat = (string)$st_price_vat_value;

        // 37개 파라미터 타입 문자열 (Phase B: product_type, is_custom_product 추가)
        ...
        // 15:cont(s) 16:date(s) 17:OrderStyle(s) 18:ThingCate(s) 19:product_type(s) 20:is_custom_product(i)
        // 21:coating_enabled(i) 22:coating_type(s) 23:coating_price(i)
        // 24:folding_enabled(i) 25:folding_type(s) 26:folding_price(i)
        // 27:creasing_enabled(i) 28:creasing_lines(i) 29:creasing_price(i)
        // 30:additional_options_total(i)
        // 31:premium_options(s) 32:premium_options_total(i)
        // 33:envelope_tape_enabled(i) 34:envelope_tape_quantity(i) 35:envelope_tape_price(i) 36:envelope_additional_options_total(i)
        // 37:unit(s) - 🆕 단위 필드
        // 타입: i(1)+s(18)+ii+si+isi+iii+i+si+iiii+s = 1+18+2+2+3+3+1+2+4+1 = 37
        $type_string = 'issssssssssssssssssiisiisiiiiisiiiiis';
        ...
        mysqli_stmt_bind_param($stmt, $type_string,
            ...
            $phone, $hendphone, $final_cont, $date, $order_style, $thing_cate, $product_type, $is_custom_product,
            $coating_enabled, $coating_type, $coating_price,
            $folding_enabled, $folding_type, $folding_price,
            $creasing_enabled, $creasing_lines, $creasing_price,
            $additional_options_total,
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            // ✅ Phase 4: 교정 요청 자동 생성
            try {
                $proofWorkflow = new ProofWorkflow($connect);
                $proof_created = $proofWorkflow->createProofRequest($new_no, $item['product_type']);
                if ($proof_created) {
                    error_log("교정 요청 생성 성공: order_no={$new_no}, product_type={$item['product_type']}");

                    // 이메일 알림 전송
                    if (!empty($email)) {
                        try {
                            $emailNotification = new EmailNotification();
                            $email_sent = $emailNotification->sendProofRequestNotification(
                                $email,
                                $username,
                                $new_no,
                                $item['product_type']
                            ...
                            if ($email_sent) {
                                error_log("교정 요청 이메일 전송 성공: {$email}");
                            }
                        } catch (Exception $e) {
                            error_log("교정 요청 이메일 전송 실패: " . $e->getMessage());
                        }
                    }
                } else {
                    error_log("교정 요청 생성 실패: order_no={$new_no}");
                }
            } catch (Exception $e) {
                error_log("교정 요청 생성 오류: " . $e->getMessage());
                // 교정 요청 생성 실패는 주문을 중단하지 않음
            }
    ...
    error_log("주문 번호 목록: " . $order_list);
    error_log("이메일: " . $email);
    error_log("사용자명: " . $username);
    error_log("리다이렉트 URL: " . $redirect_url);
    error_log("=====================================");

    // 🔧 FIX: 출력 버퍼 비우기 (header 오류 방지)
    if (ob_get_length()) {
        ob_end_clean();
    }
