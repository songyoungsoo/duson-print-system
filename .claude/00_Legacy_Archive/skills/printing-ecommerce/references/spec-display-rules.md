...
### 양식지(NCR) 단위 분석 (2026-01-12)

**결론: 양식지는 항상 '권' 단위만 사용**

- 양식지는 제본 형태로 판매 (100매철, 50매철)
- 10권 = 100매철 제본 10묶음
- '매'는 철 내부의 낱장 수를 의미

**DB 데이터 주의점**:
- `unit` 컬럼: 기본값 '매'가 잘못 저장됨 (무시해야 함)
- `quantity_unit` 컬럼: 최신 주문은 '권' 올바르게 저장
- `SpecDisplayService::getUnit()`이 PRODUCT_UNITS를 우선하여 해결
