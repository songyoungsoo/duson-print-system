...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-url-safety` | URL에 쉼표 사용 탐지 (Plesk 500 에러 방지) |
| 2 | `verify-sticker-logic` | 스티커 가격 DB lookup 방지 (formula 강제) |
| 3 | `verify-product-units` | 제품별 수량 단위 규정 검증 (QuantityFormatter SSOT 강제) |
| 4 | `verify-bind-param` | bind_param 3요소(플레이스홀더/타입문자/변수) 카운트 불일치 탐지 |
