...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-url-safety` | URL에 쉼표 사용 탐지 (Plesk 500 에러 방지) | `**/*.php` |
| `verify-sticker-logic` | 스티커 가격 DB lookup 방지 (formula 강제) | `**/*.php` |
| `verify-product-units` | 제품별 수량 단위 규정 검증 (QuantityFormatter SSOT 강제) | `**/*.php` |
...
| `verify-option-pricing` | 옵션 가격 시스템 SSOT 위반/파이프라인 단절/가격 불일치 탐지 | `admin/mlangprintauto/quote/widgets/*.php`, `mlangprintauto/**/*.php`, `includes/AdditionalOptions*.php` |
| `verify-quote-widget` | 실시간 견적받기 위젯(quote_gauge.php) 전체 9개 제품 페이지 존재/CSS/JS/순서 검증 | `mlangprintauto/*/index.php` |
| `verify-order-grouping` | 건수(order_count)/그룹(order_group_id) 파이프라인 정합성 검증 | `**/*.php`, `**/*.js`, `dashboard/**/*.php` |
