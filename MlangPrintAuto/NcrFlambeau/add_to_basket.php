...
$MY_Fsd = $_POST['MY_Fsd'] ?? '';  // Section 역할
$PN_type = $_POST['PN_type'] ?? ''; // TreeSelect 역할
...
// 장바구니에 추가 (littleprint와 동일한 구조 사용)
...
if ($stmt) {
    // NcrFlambeau는 POtype이 없으므로 빈 문자열로 설정
    $POtype = '';
    mysqli_stmt_bind_param($stmt, "sssssssii", $session_id, $MY_type, $MY_Fsd, $PN_type, $MY_amount, $POtype, $ordertype, $price, $vat_price);
