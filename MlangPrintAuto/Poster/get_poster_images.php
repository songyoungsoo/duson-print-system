...
try {
    ...
    // 포스터 전용 카테고리 포트폴리오에서 최신 이미지 4개 가져오기
    $query = "SELECT Mlang_bbs_no, Mlang_bbs_title, Mlang_bbs_connent, Mlang_bbs_link 
              FROM Mlang_portfolio_bbs 
              WHERE Mlang_bbs_reply='0' AND (CATEGORY='포스터' OR CATEGORY='poster' OR CATEGORY='대형포스터' OR CATEGORY='현수막')
              ORDER BY CASE WHEN CATEGORY='포스터' THEN 1 WHEN CATEGORY='poster' THEN 2 WHEN CATEGORY='대형포스터' THEN 3 WHEN CATEGORY='현수막' THEN 4 ELSE 5 END, Mlang_bbs_no DESC 
              LIMIT 4";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // 이미지 경로 결정 (sub/poster.php와 동일한 방식)
