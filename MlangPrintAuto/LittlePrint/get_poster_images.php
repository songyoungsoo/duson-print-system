...
try {
    ...
    // 포스터 카테고리 포트폴리오에서 최신 이미지 4개 가져오기 (sub/poster.php 참조)
    $query = "SELECT Mlang_bbs_no, Mlang_bbs_title, Mlang_bbs_connent, Mlang_bbs_link 
              FROM Mlang_portfolio_bbs 
              WHERE Mlang_bbs_reply='0' AND (CATEGORY='포스터' OR CATEGORY='poster' OR CATEGORY='리플렛')
              ORDER BY CASE WHEN CATEGORY='포스터' THEN 1 WHEN CATEGORY='poster' THEN 2 WHEN CATEGORY='리플렛' THEN 3 ELSE 4 END, Mlang_bbs_no DESC 
              LIMIT 4";
