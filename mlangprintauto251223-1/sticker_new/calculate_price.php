...
try {
    ...
    $response = [
        'success' => true,
        'data' => [
            // 필수 필드 (quotation-modal-common.js 요구사항)
            'Order_PriceForm' => (int) $result['st_price'],
            'Total_PriceForm' => (int) $result['st_price_vat'],

            // 부가세 계산
            'VAT_PriceForm' => (int) ($result['st_price_vat'] - $result['st_price']),

            // 레거시 호환 (기존 코드 유지)
            'price' => number_format($result['st_price']),
            'price_vat' => number_format($result['st_price_vat']),
            'raw_price' => (int) $result['st_price'],
            'raw_price_vat' => (int) $result['st_price_vat']
        ]
    ];
