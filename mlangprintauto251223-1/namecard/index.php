...
<body class="namecard-page<?php echo $isQuotationMode ? ' quotation-modal-mode' : ''; ?>">
    ...
    <script>
        ...
        function resetPriceDisplay() {
            ...
            // 🔧 견적서 모달 모드에서는 일부 요소가 없을 수 있음 - 방어적 체크
            ...
            if (uploadButton && uploadButton.style) {
        ...
        function showPriceError(message) {
            ...
            // 🔧 견적서 모달 모드에서도 안전하게 처리
            if (priceAmount) priceAmount.textContent = '계산 오류';
            if (priceDetails) priceDetails.textContent = message;
        }

        // 업로드 버튼 표시
        function showUploadButton() {
            const uploadButton = document.getElementById('uploadOrderButton');
            // 🔧 견적서 모달 모드에서는 uploadButton이 없을 수 있음
            if (uploadButton && uploadButton.style) {
