...
/**
 * 공통 인증 처리 파일
 * 경로: MlangPrintAuto/includes/auth.php
 * 메인 auth.php와 동일한 세션 처리
 */
...
// 공통 인증 함수 로드
include_once $_SERVER['DOCUMENT_ROOT'] . '/includes/auth_functions.php';

// 데이터베이스 연결 (각 페이지에서 이미 연결되어 있다고 가정)
// $connect 변수 초기화
$connect = null;

// $db 변수가 있다면 $connect로 할당
if (isset($db) && $db) {
    $connect = $db; // db.php에서 $db로 연결하는 경우
}

// 연결이 없다면 직접 연결 시도
if (!$connect) {
    $connect = @mysqli_connect('localhost', 'root', '', 'duson1830');
    if (!$connect) {
