...
function resetPriceDisplay() {
    ...
    // 업로드 버튼 숨기기 (hidden 클래스 추가)
    if (uploadOrderButton) {
        uploadOrderButton.classList.add('hidden');
    }
...
function calculatePrice(isAuto = false) {
    ...
    // 추가 옵션 총액 추가
    const additionalTotal = parseInt(document.getElementById('additional_options_total')?.value || 0);
    formData.set('additional_options_total', additionalTotal);
    ...
    .then(response => {
        ...
        if (response.success && response.data) {
            window.currentPriceData = response.data;
...
function updatePriceDisplay(priceData) {
    ...
    if (uploadOrderButton) {
        uploadOrderButton.classList.remove('hidden');
    }
...
// 견적서 모달 호환성: autoCalculatePrice alias
window.autoCalculatePrice = calculatePrice;
