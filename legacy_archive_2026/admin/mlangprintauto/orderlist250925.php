...
$YearOne = $_GET['YearOne'] ?? $_POST['YearOne'] ?? '';
$YearTwo = $_GET['YearTwo'] ?? $_POST['YearTwo'] ?? '';
$JK = $_GET['JK'] ?? $_POST['JK'] ?? '';
$offset = $_GET['offset'] ?? $_POST['offset'] ?? 0;
$CountWW = $_GET['CountWW'] ?? $_POST['CountWW'] ?? '';
$s = $_GET['s'] ?? $_POST['s'] ?? '';
$cont = $_GET['cont'] ?? $_POST['cont'] ?? '';
$check = $_POST['check'] ?? [];
$PHP_SELF = $_SERVER['PHP_SELF'] ?? '';
$mlang_pagego = '';
$OrderStyleYU9OK = $_GET['OrderStyleYU9OK'] ?? $_POST['OrderStyleYU9OK'] ?? '';
$OrderCate = $_GET['OrderCate'] ?? $_POST['OrderCate'] ?? '';
...
<?php
// 데이터베이스에서 상위 사용 분야 동적 로드
$typeQuery = "SELECT Type, COUNT(*) as cnt
...
if ($typeResult) {
    while($typeRow = mysqli_fetch_array($typeResult)) {
        ...
        // 표시명 결정 (매핑된 이름이 있으면 사용, 없으면 원본)
        $displayName = isset($typeDisplayNames[$typeValue]) ? $typeDisplayNames[$typeValue] : $typeValue;

        // 너무 긴 이름은 잘라내기
        if(mb_strlen($displayName) > 10) {
            $displayName = mb_substr($displayName, 0, 10) . '...';
        }

        // 선택 상태 확인
        $selected = ($Type == $typeValue) ? $CateFF : '';

        echo "<option value='{$typeValue}' {$selected}>{$displayName} ({$typeCnt})</option>";
    }
}
?>
</select>
        ...
        <option value='OrderStyle' <?if($Cate=="OrderStyle"){echo("$CateFF");}?>>진행상태</option>
		</select>
&nbsp;날짜검색 :&nbsp;
...
if($Type || $TDsearchValue || $YearOne || $YearTwo){ // 검색어 값 또는 필터링
...
// 분야 필터 처리 - 한글/영어 혼재 데이터 대응
if($Type && $Type != "total"){
    // 영어와 한글 매핑 테이블
    $typeMapping = [
        'inserted' => ['inserted', '전단지'],
        'sticker' => ['sticker', '스티카', '스티커'],
        'NameCard' => ['NameCard', '명함'],
        'MerchandiseBond' => ['MerchandiseBond', '상품권'],
        'envelope' => ['envelope', '봉투'],
        'NcrFlambeau' => ['NcrFlambeau', '양식지'],
        'cadarok' => ['cadarok', '리플렛'],
        'cadarokTwo' => ['cadarokTwo', '카다로그'],
        'LittlePrint' => ['LittlePrint', '소량인쇄']
    ];

    if(isset($typeMapping[$Type])) {
        $typeValues = $typeMapping[$Type];
        $typeConditions = [];
        foreach($typeValues as $value) {
            $typeConditions[] = "Type='$value'";
        }
        $TypeOk = "(" . implode(" OR ", $typeConditions) . ")";
    } else {
        $TypeOk = "Type='$Type'";
    }
}else{
...
// 검색어 필터 처리
if($TDsearchValue && $Cate){
    $SearchOk = "$Cate like '%$TDsearchValue%'";
}else{
    $SearchOk = "";
}
...
if($YearOne && $YearTwo){
    ...
    $DateOk = "date > '$YearOneOk' and date < '$YearTwoOk'";
}else{
    $DateOk = "";
}
...
// WHERE 절 조합
$whereParts = array();
if($TypeOk) $whereParts[] = $TypeOk;
if($SearchOk) $whereParts[] = $SearchOk;
if($DateOk) $whereParts[] = $DateOk;

if(count($whereParts) > 0){
    $Mlang_query = "select * from $table where " . implode(" and ", $whereParts);
}else{
...
<?php echo isset($typeMapping[$row['Type']]) ? htmlspecialchars($typeMapping[$row['Type']]) : htmlspecialchars($row['Type']); ?>
</font></td>
