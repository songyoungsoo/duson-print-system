...
// URL 파라미터로 종류/후가공 사전 선택 (네비게이션 드롭다운에서 진입 시)
...
if ($url_type) {
    ...
    if ($url_section) {
        ...
        $sec_q = "SELECT no FROM mlangprintauto_transactioncate 
                  WHERE Ttable='MerchandiseBond' AND BigNo='" . intval($url_type) . "' 
...
<body class="merchandisebond-page<?php echo ($isQuotationMode || $isAdminQuoteMode) ? ' quotation-modal-mode' : ''; ?>" <?php ThemeLoader::renderBodyAttributes(); ?>>
    ...
    <?php $detail_page_product = 'merchandisebond'; include __DIR__ . "/../../_detail_page/detail_page_loader.php"; ?>
