...
class LeafletPremiumOptionsManager {
    ...
    applyDBPrices(dbOptions) {
        ...
        console.log('✅ DB 가격 데이터 적용 완료 (전단지)');
        this.calculateAndUpdatePrice();
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    if (document.getElementById('premiumOptionsSection')) {
        ...
        if (typeof loadPremiumOptionsFromDB === 'function') {
            const dbData = await loadPremiumOptionsFromDB('inserted');
            if (dbData && window.leafletPremiumOptions) {
                window.leafletPremiumOptions.applyDBPrices(dbData);
            }
        }
