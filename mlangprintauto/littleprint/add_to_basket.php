...
// ✅ 소량인쇄물: quantity = MY_amount, unit = '매'
$quantity = floatval($MY_amount);
$unit = '매';

// 장바구니에 추가 (추가 옵션 JSON 방식 + 파일 업로드)
$insert_query = "INSERT INTO shop_temp (session_id, product_type, MY_type, Section, PN_type, MY_amount, quantity, unit, POtype, ordertype, st_price, st_price_vat,
                additional_options, additional_options_total, ImgFolder, ThingCate, uploaded_files)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

$stmt = mysqli_prepare($db, $insert_query);
if ($stmt) {
    mysqli_stmt_bind_param($stmt, "ssssssdsssiisisss",
        $session_id, $product_type, $MY_type, $Section, $PN_type, $MY_amount, $quantity, $unit, $POtype, $ordertype, $price, $vat_price,
