...
function calculatePrice(isAuto = true) {
    ...
    .then(response => {
        if (response.success) {
            ...
            window.currentPriceData = priceData;  // index.php에서 접근 가능하도록 전역 설정
...
function addToBasketFromModal() {
    ...
    formData.set('calculated_price', Math.round(currentPriceData.total_price));      // 공급가액 (VAT 미포함)
    formData.set('calculated_vat_price', Math.round(currentPriceData.total_with_vat));  // 합계 (VAT 포함)
