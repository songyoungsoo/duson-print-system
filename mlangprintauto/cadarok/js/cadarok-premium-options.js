...
class CadarokPremiumOptionsManager {
    ...
    applyDBPrices(dbOptions) {
        ...
        console.log('✅ DB 가격 데이터 적용 완료 (카다록)');
        this.calculateAndUpdatePrice();
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    if (document.getElementById('premiumOptionsSection')) {
        ...
        if (typeof loadPremiumOptionsFromDB === 'function') {
            const dbData = await loadPremiumOptionsFromDB('cadarok');
            if (dbData && window.cadarokPremiumOptions) {
                window.cadarokPremiumOptions.applyDBPrices(dbData);
            }
        }
        console.log('✅ 카다록 추가 옵션 시스템 준비 완료');
    }
});
