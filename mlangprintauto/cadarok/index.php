...
<body class="cadarok-page<?php echo $body_class; ?>">
            ...
            <div class="product-calculator">
                ...
                <form id="cadarokForm">
                    ...
                    <div class="upload-order-button" id="uploadOrderButton">
                        <?php if ($is_quotation_mode): ?>
                            <!-- 견적서 모달 모드: 단일 버튼 (계산+적용) -->
                            <button type="button" class="btn-upload-order" id="applyQuotationBtn" onclick="calculateAndApply()" style="background: #217346;">
                                ✅ 견적서에 적용
                            </button>
                        <?php else: ?>
                            <!-- 일반 모드: 파일 업로드 및 주문 -->
...
<?php endif; ?>
    ...
    <script>
        ...
        // 견적서 모달 모드: 가격 계산 후 자동 적용 함수
        window.calculateAndApply = function() {
            console.log('📤 [TUNNEL 1/5] "✅ 견적서에 적용" 버튼 클릭됨 - 계산 시작');

            const btn = document.getElementById('applyQuotationBtn');
            const originalText = btn.innerHTML;
            btn.disabled = true;
            btn.innerHTML = '💰 계산 중...';

            // 가격 계산 실행 (cadarok.js의 calculatePrice 함수 호출)
            if (typeof calculatePrice === 'function') {
                calculatePrice();

                // 계산 완료 대기 후 견적서에 적용
                let checkCount = 0;
                const waitForPrice = setInterval(function() {
                    checkCount++;
                    ...
                    if (priceData && priceData.total_price) {
                        ...
                        console.log('✅ 가격 계산 완료, 견적서에 적용 시작');
                        btn.innerHTML = '📝 견적서에 입력 중...';

                        // sendToQuotation 내부 로직 직접 실행
                        sendToQuotationDirect(btn, originalText, priceData);
                    } else if (checkCount > 30) { // 15초 타임아웃
                        clearInterval(waitForPrice);
                        console.error('❌ 가격 계산 타임아웃');
                        alert('가격 계산에 실패했습니다. 옵션을 확인해주세요.');
                        btn.innerHTML = originalText;
                        btn.disabled = false;
                    }
                }, 500);
            } else {
                console.error('❌ calculatePrice 함수를 찾을 수 없음');
                alert('시스템 오류: 가격 계산 함수를 찾을 수 없습니다.');
                btn.innerHTML = originalText;
                btn.disabled = false;
            }
        };
        ...
        // 견적서 데이터 전송 내부 함수
        function sendToQuotationDirect(btn, originalText, priceData) {
            console.log('📤 [TUNNEL 2/5] 견적서 데이터 전송 시작');

            try {
                // 폼에서 제품 스펙 정보 수집 (카다록 전용)
                ...
                const quotationData = {
                    ...
                    unit: '부',
                    supply_price: Math.round(priceData.total_price),
                    vat_price: Math.round(priceData.vat_price)
                };
