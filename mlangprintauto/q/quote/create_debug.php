...
// OPcache 클리어 (캐시 문제 해결)
if (function_exists('opcache_invalidate')) {
    opcache_invalidate(__FILE__, true);
    opcache_invalidate(__DIR__ . '/../../includes/ProductSpecFormatter.php', true);
}
...
// DEBUG: 파일 버전 확인
echo "\n<!-- FILE VERSION: create_debug.php - " . date('Y-m-d H:i:s', filemtime(__FILE__)) . " -->\n";
echo "<!-- OPCACHE STATUS: " . (function_exists('opcache_get_status') ? 'AVAILABLE' : 'NOT AVAILABLE') . " -->\n\n";
                        ...
                        <tbody id="itemsBody">
                            ...
                            if (count($quoteTempItems) > 0):
                                foreach ($quoteTempItems as $item):
                                    ...
                                    // DEBUG: 스티커 수량 문제 조사 (VISIBLE OUTPUT)
                                    echo "\n<!-- DEBUG START -->\n";
                                    ...
                                    echo "<!-- mesu value: " . ($item['mesu'] ?? 'NULL') . " -->\n";
                                    echo "<!-- data_version: " . ($item['data_version'] ?? 'NULL') . " -->\n";
                                    echo "<!-- ProductSpecFormatter file: " . (class_exists('ProductSpecFormatter') ? 'LOADED' : 'NOT LOADED') . " -->\n";

                                    // ProductSpecFormatter 사용 (MY_amount에서 quantity 추출)
                                    $qty = ProductSpecFormatter::getQuantity($item);
                                    $qtyDisplay = ProductSpecFormatter::getQuantityDisplay($item);  // 장바구니 형식

                                    echo "<!-- getQuantityDisplay result: '" . $qtyDisplay . "' -->\n";
                                    echo "<!-- qty value: " . $qty . " -->\n";
                                    echo "<!-- DEBUG END -->\n\n";
