...
class QuoteManager {
    ...
    public function generateQuoteNo($type = 'quotation') {
        ...
        // 오늘 생성된 문서 수 조회
        $query = "SELECT COUNT(*) as cnt FROM quotations WHERE quote_no LIKE ?";
    ...
    private function quoteNoExists($quoteNo) {
        $query = "SELECT id FROM quotations WHERE quote_no = ? LIMIT 1";
        $stmt = mysqli_prepare($this->db, $query);
    ...
    public function createFromCart($sessionId, $data) {
        ...
        try {
            ...
            $quotationsource = isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in'] === true
                ? 'admin_auto'
                : 'customer';
