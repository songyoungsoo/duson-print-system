...
require_once $_SERVER['DOCUMENT_ROOT'] . '/includes/SpecDisplayService.php';  // Phase 2.3: 통합 출력 서비스

class QuoteManager {
    private $db;
    private $formatter;
    private $specDisplayService;  // Phase 2.3: 통합 출력 서비스
     ...
     * ========================================
     *
     * quotation_temp / quote_items 테이블의 가격 필드:
     *
     * price_supply      = 공급가액 (부가세 제외)
     * price_vat         = 합계금액 (공급가 + 부가세) ⚠️ 필드명 오해 유발!
     * price_vat_amount  = 부가세액
     *
     * ✅ 올바른 계산:
     * $vat = $priceVat - $priceSupply;        // 부가세액 = 합계 - 공급가
     * $vat = $priceVatAmount;                 // 또는 price_vat_amount 필드 직접 사용
     *
     * ❌ 잘못된 계산 (절대 금지):
     * $total = $priceSupply + $priceVat;      // price_vat는 부가세가 아님!
     * $unitPrice = $priceSupply / $quantity;  // 역계산 금지!
     *
     * 📌 핵심 원칙:
     * - 품목에서 계산된 가격을 그대로 사용
     * - 절대 역계산하지 않음 (공급가 ÷ 수량 = 단가 계산 금지)
     * - price_vat는 "합계금액"이지 "부가세액"이 아님
     *
     * @see Phase 3 fields for data integrity
     * @see duson-price-calculation 스킬
     */

    public function __construct($db) {
        $this->db = $db;
        $this->formatter = new ProductSpecFormatter($db);
        $this->specDisplayService = new SpecDisplayService($db);  // Phase 2.3
    }

    /**
     * formatted_display 생성 헬퍼 함수
     * Phase 2.3: SpecDisplayService로 위임하여 통합 출력
     *
    ...
    private function generateFormattedDisplay($productType, $data) {
        // ✅ Phase 2.3: SpecDisplayService로 위임
        $data['product_type'] = $productType;
        $displayData = $this->specDisplayService->getDisplayData($data);

        $formatted = '';

        // line1: 용지/규격 정보
        if (!empty($displayData['line1'])) {
            $formatted .= $displayData['line1'] . "\n";
        }

        // line2: 인쇄/수량/디자인 정보
        if (!empty($displayData['line2'])) {
            $formatted .= $displayData['line2'] . "\n";
        }

        // 수량 표시 (단위 포함 보장)
        if (!empty($displayData['quantity_display'])) {
            $formatted .= "수량: " . $displayData['quantity_display'] . "\n";
        }
    ...
    private function addItemFromCart($quoteId, $itemNo, $cartItem) {
        ...
        // ✅ Phase 2.3: SpecDisplayService로 통합 출력 데이터 생성
        $cartItem['product_type'] = $productType;
        $specDisplayData = $this->specDisplayService->getDisplayData($cartItem);
        $quantity_display = $specDisplayData['quantity_display'] ?? '';

        // Phase 2: DataAdapter로 표준 데이터 추출 (다른 필드용)
        ...
        // ✅ 사용자 요구: DB 값 그대로 사용, 계산 금지
        // Phase 3 필드 우선 사용, 없으면 st_price 사용
        $supplyPrice = !empty($cartItem['price_supply'])
            ? intval($cartItem['price_supply'])  // Phase 3 필드 (우선)
            : intval($cartItem['st_price'] ?? 0);  // 레거시 필드 (fallback)

        $totalPrice = !empty($cartItem['price_vat'])
            ? intval($cartItem['price_vat'])  // Phase 3 필드 (우선)
            : intval($cartItem['st_price_vat'] ?? 0);  // 레거시 필드 (fallback)

        // ✅ 부가세 계산 (공급가액 이후만 계산 허용)
        // Phase 3 필드가 있으면 사용, 없으면 계산
        $vatAmount = !empty($cartItem['price_vat_amount'])
            ? intval($cartItem['price_vat_amount'])  // Phase 3 필드 (우선)
            : ($totalPrice - $supplyPrice);  // 계산 (fallback)
    ...
    private function addItemFromQuoteTemp($quoteId, $itemNo, $tempItem) {
        ...
        $isManualEntry = !empty($tempItem['is_manual_entry']) ? 1 : 0;

        // 🆕 수기 입력 품목 처리: jong 컬럼에 품목명, spec_size에 규격 저장
        if ($isManualEntry && !empty($tempItem['jong'])) {
            $productName = $tempItem['jong'];
            // 규격: spec_size 우선, domusong fallback
            $specification = $tempItem['spec_size'] ?? $tempItem['domusong'] ?? '';
        } else {
            $productName = ProductSpecFormatter::getProductTypeName($productType);
        ...
        // 2. 수량 및 단위 추출
        if ($isManualEntry && !empty($tempItem['quantity_display'])) {
            // 수기 품목: quantity_display에서 숫자 추출 (예: "250부" → 250)
            $quantityDisplay = $tempItem['quantity_display'];
            $quantity = floatval(preg_replace('/[^0-9.]/', '', $quantityDisplay));
            // 단위 추출 (마지막 한글 문자)
            if (preg_match('/[매연부권개장]$/u', $quantityDisplay, $matches)) {
                $unit = $matches[0];
            } else {
                $unit = '개';
            }
        } else {
            // 자동 품목: ProductSpecFormatter 사용
        ...
        // ✅ 사용자 요구: DB 값 그대로 사용, 계산 금지
        // Phase 3 필드 우선 사용, 없으면 st_price 사용
        $supplyPrice = !empty($tempItem['price_supply'])
            ? intval($tempItem['price_supply'])  // Phase 3 필드 (우선)
            : intval($tempItem['st_price'] ?? 0);  // 레거시 필드 (fallback)

        $totalPrice = !empty($tempItem['price_vat'])
            ? intval($tempItem['price_vat'])  // Phase 3 필드 (우선)
            : intval($tempItem['st_price_vat'] ?? 0);  // 레거시 필드 (fallback)

        // ✅ 부가세 계산 (공급가액 이후만 계산 허용)
        $vatAmount = !empty($tempItem['price_vat_amount'])
            ? intval($tempItem['price_vat_amount'])  // Phase 3 필드 (우선)
            : ($totalPrice - $supplyPrice);  // 계산 (fallback)

        // 4. 단가 계산 - ✅ 역계산 제거 (2026-01-08)
        // 품목마다 단가 개념이 다르므로 단가는 표시하지 않음
        // (전단지: 연당 가격, 명함: 매당 가격 등)
        // 견적서에서는 총 공급가만 표시하면 충분
        $unitPrice = 0;  // 역계산 금지: 공급가 ÷ 수량 절대 금지

        // 5. 메모 추출
        $notes = $tempItem['MY_comment'] ?? $tempItem['work_memo'] ?? '';

        // 🆕 $isManualEntry는 함수 상단(line 675)에서 이미 설정됨 - 여기서 덮어쓰지 않음
        ...
        // source_type: ENUM('cart','manual','custom') - 수기 입력이면 'manual', 아니면 'cart'
        $sourceType = $isManualEntry ? 'manual' : 'cart';
        ...
        // 28개 파라미터 (Phase 3: 15 original + 13 Phase 3)
        // Original 15: i i s s s d s d i i i s i s i
        // Phase 3 13:  s s s s s d s i s i i i i
        mysqli_stmt_bind_param($stmt, "iisssdsdiiisisi" . "sssssdsissiii",
            $quoteId, $itemNo, $productType, $productName, $specification,
            $quantity, $unit, $unitPrice, $supplyPrice, $vatAmount, $totalPrice,
            $sourceType, $sourceId, $notes, $isManualEntry,
            $specType, $specMaterial, $specSize, $specSides, $specDesign,
            $quantityValue, $quantityUnitPhase3, $quantitySheets, $quantityDisplayPhase3,
            $priceSupplyPhase3, $priceVat, $priceVatAmountPhase3, $dataVersion
        );
