...
try {
    ...
    foreach ($cart_items as $item) {
        ...
        $insert_query = "INSERT INTO mlangorder_printauto (
            ...
            phone, Hendphone, cont, date, OrderStyle, ThingCate, is_member,
        ...
        error_log("주문번호: $new_no");
        error_log("username 변수: [$username]");
        error_log("email 변수: [$email]");
        error_log("product_type_name: [$product_type_name]");
        error_log("============================");

        // 35 parameters: 1(i) + 17(s) + is_member(i) + coating(isi) + folding(isi) + creasing(iii) + additional(i) + premium(si) + envelope(iiii)
        mysqli_stmt_bind_param($stmt, 'isssssssssssssssssiisiisiiisiiiiii',
            $new_no, $product_type_name, $img_folder_path, $uploaded_files_json, $product_info, $item['st_price'], $item['st_price_vat'],
            $username, $email, $postcode, $address, $full_address,
            $phone, $hendphone, $final_cont, $date, $order_style, $thing_cate, $is_member_flag,
            $coating_enabled, $coating_type, $coating_price,
            $folding_enabled, $folding_type, $folding_price,
            $creasing_enabled, $creasing_lines, $creasing_price,
            $additional_options_total,
