...
<td class="order-table-td order-table-td--number">
    ...
    <?php if (!empty($row["quote_no"])) { ?>
    <br><span class="badge badge--success" style="font-size: 0.65rem; padding: 2px 4px;" title="견적서 <?php echo htmlspecialchars($row["quote_no"]) ?>">📋<?php echo !empty($row["order_group_seq"]) ? ' #' . $row["order_group_seq"] : ''; ?></span>
    <?php } ?>
</td>
<td class="order-table-td">
    <span class="badge badge--outline"><?php echo $productLabel ?></span>
    <?php if (!empty($row["custom_product_name"])) { ?>
    <br><span style="font-size: 0.7rem; color: #666;"><?php echo htmlspecialchars($row["custom_product_name"]) ?></span>
    <?php } ?>
</td>
<td class="order-table-td">
    <?php
