...
$table="mlangprintauto_ncrflambeau"; // ✅ 소문자 테이블명
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='NcrFlambeau' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='NcrFlambeau' AND no='{$row['Section']}'");
...
<td align=center><font color=white>
<?php 
$result_FGFree=mysqli_query($db, "select * from $GGTABLE where Ttable='NcrFlambeau' AND no='{$row['TreeSelect']}'");
