        ...
        <div class="file-section" style="padding: 12px; margin: 10px 0;">
            ...
            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border: 1px solid #e9ecef;">
                    ...
                    if ($row) {
                        ...
                        if (is_dir($date_upload_base)) {
                            // 최근 30일 폴더 확인
                            for ($i = 0; $i < 30; $i++) {
                                $check_date = date('Y/m/d', strtotime("-$i days"));
                                $date_dir = $date_upload_base . $check_date;
                                if (is_dir($date_dir)) {
                                    $ip_dirs = scandir($date_dir);
                                    foreach ($ip_dirs as $ip_dir) {
                                        if ($ip_dir != "." && $ip_dir != ".." && is_dir($date_dir . "/" . $ip_dir)) {
                                            $sticker_dirs[] = $date_dir . "/" . $ip_dir;
                                        }
                                    }
                                }
                            }
