...
<tr>
<td align=left>
<font color=red>*</font> 가상회원의 정보는 최대 50명까지 입력할수 있습니다.<BR>
<font color=red>*</font> 회원의 진짜 정보가 50명이 넘으면 가상회원의 정보는 호출되지 않습니다.<BR>
<font color=red>*</font> 진짜회원이 49명일경우 까지는 진짜회원(49명)+가상회원(50명) 으로 호출됩니다.<BR>
</td>
<td align=right>

현재 가상회원수-<font style='color:blue;'><b><?=$total?></b></font>&nbsp;명
<?if($total < 50){?>
<input type='button' onClick="javascript:popup=window.open('admin.php?mode=form', 'Member_X','width=300,height=220,top=50,left=50,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 가상회원정보 입력하기 '>
<?}else{?><BR><font color=#FF0099>50명이상 입력불가</font><?}?>
</td>
</tr>
</table>
...
<tr bgcolor='#575757'>
<td align=center><font color=white><?=$row[id]?></font></td>
<td align=center><?if($row[sex]=="1"){echo("<font color=gold>남성");}else{echo("<font color=pink>여성");}?></font></td>
...
<?
if($row[school]=="1"){echo("초등(국민)학교 졸업");}
if($row[school]=="2"){echo("초등(국민)학교 중퇴");}
if($row[school]=="3"){echo("중학교 졸업");}
if($row[school]=="4"){echo("중학교 중퇴");}
if($row[school]=="5"){echo("고등학교 졸업");}
if($row[school]=="6"){echo("고등학교 중퇴");}
if($row[school]=="7"){echo("대학교 졸업");}
if($row[school]=="8"){echo("대학교 중퇴");}
if($row[school]=="9"){echo("대학원 졸업");}
if($row[school]=="10"){echo("대학원 중퇴");}
if($row[school]=="11"){echo("대학원 이상");}
if($row[school]=="11"){echo("학력 무");}	
?>
</font></td>
<td align=center>
