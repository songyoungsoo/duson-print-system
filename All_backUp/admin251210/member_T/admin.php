...
include"../../db.php";
$result= mysqli_query($db, "select * from $admin_table where no='1'");
...
if($code=="modify"){
include"../../db.php";
$result= mysqli_query($db, "select * from member_T where no='$no'");
$row= mysqli_fetch_array($result);
$Viewname="$row[name]";
$Viewyear="$row[year]";
$Viewmap="$row[map]";
...
if($mode=="form_ok"){
	...
	$result = mysqli_query($db, "SELECT max(no) FROM member_T");
	if (!$result) {
		echo "
			<script>
...
$result = mysqli_query($db, "DELETE FROM member_T WHERE no='$no'");
mysqli_close($db);
