...
class CommonGallery {
    ...
    public static function renderScript() {
        ...
        /**
         * 공통 갤러리 렌더링 (포스터 방식) - 5-썸네일 시스템 (4개 이미지 + 1개 더보기 버튼)
         */
        function renderCommonGallery(containerId, images, categoryLabel) {
            console.log('🎨 공통 갤러리 렌더링 시작 (5-썸네일):', {containerId, images: images.length});
            ...
            thumbsHtml.push('<div class=\"thumb more-button\"' +
                ...
                ' role=\"button\"' +
                ' tabindex=\"0\"' +
                ' aria-label=\"더 많은 ' + categoryLabel + ' 샘플 보기\">' +
            ...
            console.log(`✅ 공통 갤러리 렌더링 완료 - \${validImages.length}개 이미지 + 더보기 버튼`);
        ...
        /**
         * 더보기 버튼 클릭 핸들러 (공통 갤러리 모달 열기)
         */
        function openCommonGalleryModal() {
            console.log('📂 더보기 버튼 클릭 - 갤러리 모달 열기');
            // 공통 갤러리 팝업 함수 호출 (common-gallery-popup.js)
            if (typeof openGalleryPopup === 'function') {
                openGalleryPopup();
            } else {
                console.warn('⚠️ openGalleryPopup 함수를 찾을 수 없습니다');
                alert('갤러리를 준비 중입니다.');
            }
        }
        </script>
    ...
    public static function renderCSS() {
        ...
        /* 썸네일 그리드 - 5개 (4개 이미지 + 1개 더보기 버튼) */
        .proof-thumbs {
            display: grid;
            grid-template-columns: repeat(5, 1fr);
            gap: 10px;
            width: 100%;
            min-width: 0; /* flex 컨테이너 내에서 축소 방지 */
        }
