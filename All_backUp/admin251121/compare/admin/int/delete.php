...
if($AdminCode21=="form"){
include"../title.php";
$result= mysql_query("select * from $table where Mlang_bbs_no='$no'",$db);
...
if($bbs=="del"){$result = mysql_query("DELETE FROM $table WHERE Mlang_bbs_no='$no'");
}else{$result = mysql_query("DELETE FROM $table WHERE no='$no'");}


////////////// 새로 추가한 기능 BBSTOP /////////////////////////
$BBS_TOPresult= mysql_query("select * from BBS_TOP where BBS_Table='$BBS_Table' and BBS_No='$no'",$db);
   $BBS_TOProw= mysql_fetch_array($BBS_TOPresult);
     if($BBS_TOProw){
$result = mysql_query("DELETE FROM BBS_TOP WHERE no='$BBS_TOProw[no]'");
         }
mysql_close();
////////////// 새로 추가한 기능 BBSTOP /////////////////////////
