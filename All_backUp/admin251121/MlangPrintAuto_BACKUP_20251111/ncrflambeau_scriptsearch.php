...
<?php
// ✅ db.php는 이미 NcrFlambeau_List.php에서 include되었으므로 제거
// include"../../db.php";
...
?>
acts[<?=$g?>] = new Activity('<?=$row['no']?>', [<?php
...
// ✅ mysqli_close는 NcrFlambeau_List.php에서 처리하므로 제거
// mysqli_close($db); 
?>
...
while($row= mysqli_fetch_array($result)) { 
?>
actsmyListTreeSelect[<?=$g?>] = new Activity('<?=$row['no']?>', [<?php
...
while($row= mysqli_fetch_array($result)) { 
?>
VLmyListTreeSelect[<?=$g?>] = new Activity('<?=$row['no']?>', [<?php
              $result_Two= mysqli_query($db, "select * from $GGTABLE where TreeNo='{$row['no']}' order by no asc");
                $rows_Two=mysqli_num_rows($result_Two);
                  if($rows_Two){
                     while($row_Two= mysqli_fetch_array($result_Two)) {
...
{ 
?>
<option value='<?=$row['no']?>' <?if($RadOne==$row['no']){echo("selected style='font-size:10pt; background-color:#429EB2; color:#FFFFFF;'");}?>><?=htmlspecialchars($row['title'])?></option>
<?php 	$r++;
}
...
<select name=myList>
<option value='#'>:::::: 선택하세요 ::::::</option>
<?if($myList){echo("<option value='$myList' selected style='font-size:10pt; background-color:#429EB2; color:#FFFFFF;'>");?><?php
// ✅ db.php는 이미 NcrFlambeau_List.php에서 include되었으므로 제거
// include"../../db.php";
$result=mysqli_query($db, "select * from $GGTABLE where no='$myList'");
$row= mysqli_fetch_array($result);
if($row){ echo(htmlspecialchars($row['title'])); } // ✅ mysqli_close는 NcrFlambeau_List.php에서 처리하므로 제거
// mysqli_close($db);
?><?echo("</option>");}?>
</select>
...
<option value='#'>:::::: 선택하세요 ::::::</option>
<?if($myListTreeSelect){echo("<option value='$myListTreeSelect' selected style='font-size:10pt; background-color:#429EB2; color:#FFFFFF;'>");?><?php
// ✅ db.php는 이미 NcrFlambeau_List.php에서 include되었으므로 제거
// include"../../db.php";
