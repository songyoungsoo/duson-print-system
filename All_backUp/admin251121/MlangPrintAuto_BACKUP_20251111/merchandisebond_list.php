...
$table="mlangprintauto_merchandisebond"; // ✅ 소문자 테이블명
...
<?php
// ✅ listsearchbox.php에서 필요한 $db 연결을 먼저 확보
if (!isset($db)) {
    include"../../db.php";
}
?>
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='MerchandiseBond' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='MerchandiseBond' AND no='{$row['Section']}'");
