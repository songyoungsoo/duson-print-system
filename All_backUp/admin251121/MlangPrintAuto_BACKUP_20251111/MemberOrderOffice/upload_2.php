...
if(is_uploaded_file($photofile_2)) {
	...
	if (preg_match('/^' . $file_extention . '$/i', "html|php3|phtml|inc|asp")) {
 	...
 	if( $MAXFSIZE * 1024 < $photofile_2_size) {
		...
		$msg = "\\n사진자료를 업로드중에 문제가 발생하였습니다.\\n\\n회원정보수정에서 사진자료를 재입력하시면 됩니다.\\n\\n문제요인: 업로드하신 파일의 크기가 $photofile_2_kfsize KB입니다. 관리자가 제한한 용량은 $MAXFSIZE KB입니다.";
		echo ("<script language=javascript>
                     window.alert('$msg');
                  </script>");
	}
...
if(preg_match('/^' . "(([^/a-zA-Z]){1 . '$/i', })(\.jpg|\.jpeg|\.bmp|\.png|\.gif)", $MlangHanGulChange ,$MlangHanGulregs)){
