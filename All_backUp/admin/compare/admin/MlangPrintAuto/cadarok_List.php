...
if($search=="yes"){ //검색모드일때
 $Mlang_query="select * from $table where style='$RadOne' and Section='$myListTreeSelect' and TreeSelect='$myList'";
}else{ // 일반모드 일때
...
<? 
$result_FGTwo=mysql_query("select * from $GGTABLE where no='$row[style]'",$db);
$row_FGTwo= mysql_fetch_array($result_FGTwo);
if($row_FGTwo){ echo("$row_FGTwo[title]"); }
?>
</font></td>
<td align=center><font color=white>
<? 
$result_FGOne=mysql_query("select * from $GGTABLE where no='$row[Section]'",$db);
$row_FGOne= mysql_fetch_array($result_FGOne);
if($row_FGOne){ echo("$row_FGOne[title]"); }
?>
</font></td> 
<td align=center><font color=white>
<? 
$result_FGFree=mysql_query("select * from $GGTABLE where no='$row[TreeSelect]'",$db);
$row_FGFree= mysql_fetch_array($result_FGFree);
if($row_FGFree){ echo("$row_FGFree[title]"); }
?>
</font></td> 
<td align=center><font color=white><?if($row[quantity]=="9999"){echo("기타");}else{echo("$row[quantity]부");}?></font></td>
<td align=center><font color=white>
