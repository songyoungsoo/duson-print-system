...
<?
$Cate_result= mysql_query("select * from $GGTABLE where Ttable='$Ttable' and BigNo='0'",$db);
...
<option value='<?=$PHP_SELF?>?ACate=<?=$Cate_row[no]?>&Ttable=<?=$Ttable?>' <?if($ACate=="$Cate_row[no]"){echo("style='background-color:#429EB2; color:#FFFFFF;' selected");}?>><?=$Cate_row[title]?>-(<?=$DF_Tatle_2?>)</option>

<?
$Sub_result= mysql_query("select * from $GGTABLE where TreeNo='$Cate_row[no]'",$db);
$Sub_row= mysql_fetch_array($Sub_result);
?>

<?if($Sub_row[TreeNo]){?>
<option value='<?=$PHP_SELF?>?ATreeNo=<?=$Sub_row[TreeNo]?>&Ttable=<?=$Ttable?>' <?if($ATreeNo=="$Cate_row[no]"){echo("style='background-color:#429EB2; color:#FFFFFF;' selected");}?>><?=$Cate_row[title]?>-(<?=$DF_Tatle_3?>)</option>
<?}?>
...
<?
if($row[TreeNo]){
 $BigNo_result= mysql_query("select * from $GGTABLE where Ttable='$View_TtableB' and no='$row[TreeNo]'",$db);
   }else{   
    $BigNo_result= mysql_query("select * from $GGTABLE where Ttable='$View_TtableB' and no='$row[BigNo]'",$db);
    }
   $BigNo_row= mysql_fetch_array($BigNo_result);
     if($BigNo_row){
...
<td align=center>
<input type='button' onClick="javascript:popup=window.open('./CateAdmin.php?mode=form&code=modify&no=<?=$row[no]?>&Ttable=<?=$Ttable?><?if($row[TreeNo]){?>&TreeSelect=2<?}else if($row[BigNo]=="0"){}else{?>&TreeSelect=1<?}?><?if($Cate){echo("&Cate=$Cate");}?><?if($ATreeNo){echo("&ATreeNo=$ATreeNo");}?><?if($ACate){echo("&ACate=$ACate");}?>', 'WebOffice_<?=$PageCode?>Modify','width=500,height=120,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수정 '>
