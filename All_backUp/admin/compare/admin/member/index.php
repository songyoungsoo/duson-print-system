...
<td align=center>
  ...
  <tr><td align=center><INPUT TYPE="text" NAME="money" size='7' value='<?=$row[money]?>'><input type='submit' value='수정'></td></tr></form>
  </table>
...
<option value='<?=$PHP_SELF?>?offset=<?=$offset?>&TDsearch=<?=$TDsearch?>&TDsearchValue=<?=$TDsearchValue?>&mode=LevelModify&code=2&no=<?=$row[no]?>' <? if($row[level]=="2"){echo("selected");} ?>>2 레벨-부운영자</option>
<option value='<?=$PHP_SELF?>?offset=<?=$offset?>&TDsearch=<?=$TDsearch?>&TDsearchValue=<?=$TDsearchValue?>&mode=LevelModify&code=3&no=<?=$row[no]?>' <? if($row[level]=="3"){echo("selected");} ?>>3 레벨-골드회원</option>
<option value='<?=$PHP_SELF?>?offset=<?=$offset?>&TDsearch=<?=$TDsearch?>&TDsearchValue=<?=$TDsearchValue?>&mode=LevelModify&code=4&no=<?=$row[no]?>' <? if($row[level]=="4"){echo("selected");} ?>>4 레벨-정회원</option>
<option value='<?=$PHP_SELF?>?offset=<?=$offset?>&TDsearch=<?=$TDsearch?>&TDsearchValue=<?=$TDsearchValue?>&mode=LevelModify&code=5&no=<?=$row[no]?>' <? if($row[level]=="5"){echo("selected");} ?>>5 레벨-일반회원</option>
</select>
