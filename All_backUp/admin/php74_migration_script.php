<?php
/**
 * PHP 5.2 → 7.4 마이그레이션 자동화 스크립트
 *
 * 안전하고 체계적인 마이그레이션을 위한 3단계 접근법
 * - Phase 1: 기본 구문 호환성
 * - Phase 2: 데이터베이스 레이어 현대화
 * - Phase 3: 보안 강화
 */

declare(strict_types=1);

// 로그 파일 설정
$logFile = __DIR__ . '/php74_migration_log_' . date('Y-m-d_H-i-s') . '.txt';

class PHP74Migrator {
    private string $logFile;
    private array $stats = [
        'total_files' => 0,
        'migrated_files' => 0,
        'errors' => 0,
        'skipped_files' => 0
    ];

    public function __construct(string $logFile) {
        $this->logFile = $logFile;
    }

    public function writeLog(string $message): void {
        $timestamp = date('Y-m-d H:i:s');
        $logMessage = "[$timestamp] $message" . PHP_EOL;
        file_put_contents($this->logFile, $logMessage, FILE_APPEND | LOCK_EX);
        echo $logMessage;
    }

    public function migrateFile(string $filePath, bool $dryRun = false): bool {
        if (!file_exists($filePath)) {
            $this->writeLog("❌ 파일이 존재하지 않습니다: $filePath");
            $this->stats['errors']++;
            return false;
        }

        $originalContent = file_get_contents($filePath);
        if ($originalContent === false) {
            $this->writeLog("❌ 파일 읽기 실패: $filePath");
            $this->stats['errors']++;
            return false;
        }

        // Phase 1: 기본 구문 호환성
        $migratedContent = $this->applyPhase1($originalContent, $filePath);

        // Phase 2: 데이터베이스 레이어 (기본 변환)
        $migratedContent = $this->applyPhase2($migratedContent, $filePath);

        // Phase 3: 기본 보안 강화
        $migratedContent = $this->applyPhase3($migratedContent, $filePath);

        // 변경사항이 있는지 확인
        if ($originalContent === $migratedContent) {
            $this->writeLog("⏭️  변경사항 없음: $filePath");
            $this->stats['skipped_files']++;
            return true;
        }

        if (!$dryRun) {
            // 백업 생성
            $backupPath = $filePath . '.backup_php52_' . date('YmdHis');
            if (!copy($filePath, $backupPath)) {
                $this->writeLog("❌ 백업 생성 실패: $filePath");
                $this->stats['errors']++;
                return false;
            }
            $this->writeLog("💾 백업 생성: $backupPath");

            // 마이그레이션된 코드 저장
            if (file_put_contents($filePath, $migratedContent) === false) {
                $this->writeLog("❌ 파일 저장 실패: $filePath");
                $this->stats['errors']++;
                return false;
            }
            $this->writeLog("✅ 마이그레이션 완료: $filePath");
        } else {
            $this->writeLog("🔍 [DRY RUN] 마이그레이션 예정: $filePath");
        }

        $this->stats['migrated_files']++;
        return true;
    }

    /**
     * Phase 1: 기본 구문 호환성
     */
    private function applyPhase1(string $content, string $filePath): string {
        $changes = [];

        // 1.1 짧은 PHP 태그 → 완전한 PHP 태그
        if (preg_match('/^<\?[^p]/', $content)) {
            $content = preg_replace('/^<\?/', '<?php' . PHP_EOL . 'declare(strict_types=1);', $content);
            $changes[] = "짧은 PHP 태그 변환";
        }

        // 1.2 전역 변수 현대화
        if (strpos($content, '$_SERVER['PHP_AUTH_USER'] ?? ''') !== false) {
            $content = str_replace(
                ['empty($_SERVER['PHP_AUTH_USER'])', '$_SERVER['PHP_AUTH_USER'] ?? '''],
                ['empty($_SERVER[\'PHP_AUTH_USER\'])', '$_SERVER[\'PHP_AUTH_USER\'] ?? \'\''],
                $content
            );
            $changes[] = '$_SERVER['PHP_AUTH_USER'] ?? '' → $_SERVER 변환';
        }

        if (strpos($content, '$_SERVER['PHP_AUTH_PW'] ?? ''') !== false) {
            $content = str_replace(
                ['empty($_SERVER['PHP_AUTH_PW'])', '$_SERVER['PHP_AUTH_PW'] ?? '''],
                ['empty($_SERVER[\'PHP_AUTH_PW\'])', '$_SERVER[\'PHP_AUTH_PW\'] ?? \'\''],
                $content
            );
            $changes[] = '$_SERVER['PHP_AUTH_PW'] ?? '' → $_SERVER 변환';
        }

        // 1.3 변수 초기화 (기본적인 경우)
        $commonVars = ['$mode', '$no', '$search', '$id', '$name'];
        foreach ($commonVars as $var) {
            if (strpos($content, $var) !== false && !preg_match("/\{$var}\s*=/", $content)) {
                // GET/POST 변수는 null 병합 연산자로 초기화
                $content = preg_replace(
                    '/(\$' . substr($var, 1) . '\s*=\s*\$_[GP])/i',
                    '$1OST[\'' . substr($var, 1) . '\'] ?? \'\'',
                    $content
                );
            }
        }

        if (!empty($changes)) {
            $this->writeLog("📝 Phase 1 적용 ($filePath): " . implode(', ', $changes));
        }

        return $content;
    }

    /**
     * Phase 2: 데이터베이스 레이어 현대화
     */
    private function applyPhase2(string $content, string $filePath): string {
        $changes = [];

        // 2.1 MySQL 함수 → MySQLi 변환 (기본적인 경우)
        $mysqlReplacements = [
            'mysqli_query($db, ' => 'mysqli_query($db, ',
            'mysqli_fetch_array(' => 'mysqli_fetch_array(',
            'mysqli_fetch_row(' => 'mysqli_fetch_row(',
            'mysqli_close($db)' => 'mysqli_close($db)',
            'mysqli_close(' => 'mysqli_close('
        ];
// ⚠️  에러 처리 권장: mysqli_error() 사용을 고려하세요


        foreach ($mysqlReplacements as $old => $new) {
            if (strpos($content, $old) !== false) {
                $content = str_replace($old, $new, $content);
                $changes[] = "$old → $new";
            }
        }

        if (!empty($changes)) {
            $this->writeLog("🗄️ Phase 2 적용 ($filePath): " . implode(', ', $changes));
        }

        return $content;
    }

    /**
     * Phase 3: 기본 보안 강화
     */
    private function applyPhase3(string $content, string $filePath): string {
        $changes = [];

        // 3.1 strcmp → hash_equals (비밀번호 비교)
        if (strpos($content, 'strcmp(') !== false && strpos($content, 'password') !== false) {
            $content = preg_replace(
                '/strcmp\(\$([^,]+),\s*\$([^)]+)\)\s*==\s*0/',
                'hash_equals($1, $2)',
                $content
            );
            $changes[] = 'strcmp → hash_equals (보안 강화)';
        }

        if (!empty($changes)) {
            $this->writeLog("🛡️ Phase 3 적용 ($filePath): " . implode(', ', $changes));
        }

        return $content;
    }

    public function scanAndMigrate(string $directory, bool $dryRun = false): array {
        $this->writeLog("🚀 마이그레이션 시작: $directory");

        $iterator = new RecursiveIteratorIterator(
            new RecursiveDirectoryIterator($directory, RecursiveDirectoryIterator::SKIP_DOTS),
            RecursiveIteratorIterator::LEAVES_ONLY
        );

        foreach ($iterator as $file) {
            if ($file->isFile() && $file->getExtension() === 'php') {
                $this->stats['total_files']++;
                $filePath = $file->getRealPath();
                $this->migrateFile($filePath, $dryRun);
            }
        }

        $this->printStats();
        return $this->stats;
    }

    private function printStats(): void {
        $this->writeLog("=== 마이그레이션 완료 요약 ===");
        $this->writeLog("총 파일 수: {$this->stats['total_files']}");
        $this->writeLog("마이그레이션된 파일: {$this->stats['migrated_files']}");
        $this->writeLog("변경사항 없는 파일: {$this->stats['skipped_files']}");
        $this->writeLog("에러 발생: {$this->stats['errors']}");
    }
}

// 메인 실행
$migrator = new PHP74Migrator($logFile);

echo "=== PHP 5.2 → 7.4 마이그레이션 도구 ===" . PHP_EOL;
echo "대상 디렉토리: " . __DIR__ . PHP_EOL;

$dryRun = isset($argv[1]) && $argv[1] === '--dry-run';

if ($dryRun) {
    echo "🔍 DRY RUN 모드: 실제 변경하지 않고 미리보기만 수행합니다." . PHP_EOL;
} else {
    echo "⚠️  실제 마이그레이션을 시작합니다. 백업이 자동으로 생성됩니다." . PHP_EOL;
    echo "계속하려면 Enter를 누르세요...";
    fgets(STDIN);
}

$result = $migrator->scanAndMigrate(__DIR__, $dryRun);

echo PHP_EOL . "로그 파일: $logFile" . PHP_EOL;
if (!$dryRun) {
    echo "백업 파일들은 *.backup_php52_YYYYMMDDHHMMSS 형식으로 저장되었습니다." . PHP_EOL;
}
?>