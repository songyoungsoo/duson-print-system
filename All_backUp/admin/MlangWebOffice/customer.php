...
function WebOffice_customer_Del(no){
	if (confirm(+no+'번 자료를 삭제 하시겠습니까..?\n\n채용직원정보의 모든 자료까지 동시에 다 같이 삭제가 됩니다.\n\n한번 삭제한 자료는 복구 되지 않으니 신중을 기해주세요.............!!')) {
		str='./customer/admin.php?no='+no+'&mode=delete';
...
<input type='button' onClick="javascript:popup=window.open('./customer/admin.php?mode=form', 'WebOffice_customerForm','width=600,height=400,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 정보 입력하기 '>
</td>
</tr>
</table>
...
<?php 
$result_staff=mysqli_query($db, "select * from WebOffice_customer_staff where customer_no=$row[no]");
$total_staff = mysqli_affected_rows($db);
if($result_staff){echo("$total_staff");}else{echo("자료무");}
?>
명</font></td>
<td align=center>
<input type='button' onClick="javascript:popup=window.open('./customer/admin.php?mode=view&no=<?=$row[no]?>', 'WebOffice_customerView','width=650,height=600,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value='정보자세히보기'>
<input type='button' onClick="javascript:popup=window.open('./customer/admin.php?mode=form&code=modify&no=<?=$row[no]?>', 'WebOffice_customerModify','width=600,height=400,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수정 '>
