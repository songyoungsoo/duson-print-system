...
<BR>
<p align=center>
<form name='AdminPassKleInfo' method='post' OnSubmit='javascript:return AdminPassKleCheckField()' action='<?=$PHP_SELF?>'>
...
<tr><td align=center>
관리자ID <input type='text' name='id' maxLength='10' size='20' value='<?=$adminid?>'>&nbsp;&nbsp;
관리자PASS <input type='text' name='pass' maxLength='10' size='20' value='<?=$adminpasswd?>'>
</td></tr>

<tr><td bgcolor='#6699CC' class='td11'>카테고리명 관리</td></tr>
<tr><td align=center>
<input type='text' name='cate' maxLength='10' size='80' value='<?=$AdCate?>'>
<BR>
...
<tr><td align=center>
TEXT <INPUT TYPE="radio" NAME="style" <?if($AdStyle=="text"){echo("checked");}?> value='text'>
PHOTO <INPUT TYPE="radio" NAME="style" <?if($AdStyle=="photo"){echo("checked");}?> value='photo'>
width <input type='text' name='width' maxLength='10' size='20' value='<?=$AdWidth?>'>&nbsp;&nbsp;
height <input type='text' name='height' maxLength='10' size='20' value='<?=$AdHeight?>'>
<BR>
...
include"db.php";
	...
	fwrite($fp, "\$db=mysqli_connect(\$host,\$user,\$password);\n");
	fwrite($fp, "mysqli_select_db(\$db, \"\$dataname\");\n");
...
$result= mysqli_query($db, "select * from MlangOrder where no='$no'");
...
<tr>
<td bgcolor='<?=$Bgcolor_2?>' width=100 class='td1' align='<?=$align_td1?>'>&nbsp;사이트 제목&nbsp;</td>
<td bgcolor='<?=$Bgcolor_3?>'>
<input type='text' name='title' maxLength='50' size='50' <?=$InputStyle?> <?if($ModifyCode){echo("value='$GF_title'");}?>>
</td>
</tr>

<tr>
<td bgcolor='<?=$Bgcolor_2?>' width=100 class='td1' align='<?=$align_td1?>'>&nbsp;사이트 URL&nbsp;</td>
<td bgcolor='<?=$Bgcolor_3?>'>
<input type='text' name='url' maxLength='300' size='50' <?=$InputStyle?> <?if($ModifyCode){echo("value='$GF_url'");}?>>
</td>
</tr>
...
<td bgcolor='<?=$Bgcolor_3?>'>
<TEXTAREA NAME="cont" ROWS="10" COLS="50" <?=$InputStyle?>><?if($ModifyCode){echo("$GF_cont");}?></TEXTAREA>
</td>
</tr>

<tr>
<td bgcolor='<?=$Bgcolor_2?>' width=100 class='td1' align='<?=$align_td1?>'>&nbsp;사이트 배너&nbsp;</td>
<td bgcolor='<?=$Bgcolor_3?>'>
<?if($ModifyCode){if($GF_upfile){?>
<INPUT TYPE="checkbox" NAME="photofileModify">파일을 수정하시려면 체크를 해주세요<BR>
<img src='./upload/<?=$GF_upfile?>' width=100 height=40>
<?} }?>
...
<td bgcolor='<?=$Bgcolor_3?>'>
<?php 	echo "<select name='cate' $InputStyle><OPTION VALUE='0' selected>▒선택하세요▒</OPTION>";
...
if($photofile){$upload_dir="./upload"; include"upload.php";}

include"db.php";
	$result = mysqli_query($db, "SELECT max(no) FROM $table");
	if (!$result) {
		echo "
			<script>
				window.alert(\"DB 접속 에러입니다!\")
...
$upload_dir="./upload"; include"upload.php";
unlink("./upload/$GF_upfile");
}}else{$photofileNAME="$GF_upfile";}
}else{if($photofile){$upload_dir="./upload"; include"upload.php";}}

$query ="UPDATE $table SET title='$title', cont='$cont', url='$url', cate='$cate', upfile='$photofileNAME' WHERE no='$no'";
$result= mysqli_query($db, $query);
	if(!$result) {
		echo "
			<script language=javascript>
...
<td align=right>
<input type='button'  onClick="javascript:popup=window.open('<?=$PHP_SELF?>?mode=AdminSiteSubmit', 'MlangFriendSiteSubmit','width=600,height=430,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value='웹사이트등록하기'>
<input type='button'  onClick="javascript:popup=window.open('<?=$PHP_SELF?>?mode=AdminModifyPass', 'MlangFriendSiteAdMinModifdy','width=600,height=300,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value='프로그램환경설정'>
</td>
</tr>
</table>
...
<?php if($cate){$Mlang_query="select * from $table  where cate='$cate'";}else{$Mlang_query="select * from $table";}
...
<td align=center><a href='<?=$row[url]?>' target='_blank'><?=$row[title]?></a></td>
<td align=center>
<?if($row[upfile]){?><a href='<?=$row[url]?>' target='_blank'><img src='./upload/<?=$row[upfile]?>' border=0 width=100 height=40></a><?}else{?>배너 없음<?}?>
</td>
<td align=center><?=$row[date]?></td>
<td align=center>
<input type='button' onClick="javascript:popup=window.open('<?=$PHP_SELF?>?mode=AdminSiteSubmit&ModifyCode=<?=$row[no]?>', 'MlangFFFiteModify','width=650,height=500,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수정 '>
