...
if (isset($_SESSION['user_id'])) {
    ...
    $user_name = isset($_SESSION['id_login_ok']['id']) ? $_SESSION['id_login_ok']['id'] : '';
    ...
    function downloadTemplateSize(width, height, productName) {
        ...
        window.sendToQuotation = function() {
            ...
            try {
                ...
                // 규격 문자열 생성
                const specification = `${colorText} / ${paperText} / ${sizeText} / ${quantityText} / ${ordertypeText}`.trim();

                // 전단지 수량 계산: 매수를 연 단위로 변환 (1000매 = 1연)
                const mesuValue = parseInt(myAmountSelect.value) || 1000;
                const yeonQuantity = (mesuValue / 1000).toFixed(1);  // 소수점 1자리 (예: 0.5연, 1.5연)

                // 견적서 폼에 전달할 데이터 구조
                const quotationData = {
                    product_name: '전단지',
                    specification: specification,
                    mesu: mesuValue,  // 매수 (원본값)
                    quantity: parseFloat(yeonQuantity),  // 연 단위 수량 (소수점 1자리)
                    unit: '연',  // 전단지 단위는 "연"
                    supply_price: parseInt(window.currentPriceData.Order_PriceForm) || 0,  // 공급가 (VAT 제외)
                    vat_price: parseInt(window.currentPriceData.Total_PriceForm) || 0,  // VAT 포함 가격
