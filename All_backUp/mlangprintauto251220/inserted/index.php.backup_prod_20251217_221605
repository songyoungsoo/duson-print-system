            ...
            <aside class="product-calculator" aria-label="실시간 견적 계산기">
                <form id="orderForm" method="post">
                    ...
                    <div class="upload-order-button" id="uploadOrderButton">
                        <?php if ($is_quotation_mode): ?>
                            <!-- 견적서 모달 모드: 견적서에 적용 버튼만 표시 (실시간 계산) -->
    ...
    function downloadTemplateSize(width, height, productName) {
        ...
        window.sendToQuotation = function() {
            ...
            if (!priceAmountElement) {
                alert('가격 표시 영역을 찾을 수 없습니다.');
                return;
            }

            const priceText = priceAmountElement.textContent || priceAmountElement.innerText;
            console.log('💰 가격 표시 영역 내용:', priceText);

            // 가격에 숫자가 포함되어 있는지 확인
            const hasPrice = /\d/.test(priceText) && !priceText.includes('견적 계산 필요') && !priceText.includes('계산 중');
            if (!hasPrice) {
                alert('가격이 계산되지 않았습니다. 모든 옵션을 선택하면 자동으로 계산됩니다.');
                return;
            }

            console.log('✅ 가격 확인 완료:', priceText);
            ...
            try {
                ...
                // 수량 값 추출 - select option에서 value와 data-mesu 가져오기
                const selectedOption = myAmountSelect.options[myAmountSelect.selectedIndex];
                const quantityValue = parseFloat(selectedOption.value) || 1;  // value는 연수
                const mesuValue = parseInt(selectedOption.dataset.mesu) || 0;  // data-mesu는 매수
                ...
                if (coatingEnabled && coatingEnabled.checked) {
                    ...
                    if (coatingType && coatingType.value) {
                        optionsList.push(coatingType.options[coatingType.selectedIndex].text);
                    }
                }
                if (foldingEnabled && foldingEnabled.checked) {
                    const foldingType = document.getElementById('folding_type');
                    if (foldingType && foldingType.value) {
                        optionsList.push(foldingType.options[foldingType.selectedIndex].text);
                    }
                }
                if (creasingEnabled && creasingEnabled.checked) {
                    const creasingLines = document.getElementById('creasing_lines');
                    if (creasingLines && creasingLines.value) {
                        optionsList.push('오시 ' + creasingLines.options[creasingLines.selectedIndex].text);
                    }
                }
                ...
                // 1줄: 색상 / 용지 / 규격
                // 2줄: 인쇄면 / 타입 + 추가옵션
                const line1 = `${myTypeText} / ${pnTypeText} / ${myFsdText}`;
                const line2Parts = [potypeText, ordertypeText, ...optionsList];
                const line2 = line2Parts.join(' / ');
                const specification = `${line1}\n${line2}`.trim();

                // 수량 표시 문자열 (별도 컬럼용)
                const mesuDisplay = mesuValue > 0 ? `\n(${mesuValue.toLocaleString()}매)` : '';
                const quantityDisplay = `${quantityValue}연${mesuDisplay}`;

                console.log('📊 수량 추출:', { 
                    text: myAmountText, 
                    quantity: quantityValue + '연', 
                    mesu: mesuValue + '매' 
                });

                // 가격 표시 영역에서 가격 추출
                // "123,456원" 형식에서 숫자만 추출
                const priceMatch = priceText.match(/[\d,]+/);
                const totalPrice = priceMatch ? parseInt(priceMatch[0].replace(/,/g, '')) : 0;

                // 부가세 포함 가격 계산 (공급가액 * 1.1)
                const vatPrice = Math.round(totalPrice * 1.1);

                console.log('💰 추출된 가격:', { priceText, totalPrice, vatPrice });

                // 견적서 폼에 전달할 데이터 구조
                const quotationData = {
                    product_name: '전단지',
                    product_type: 'inserted',  // 전단지 타입 명시
                    specification: specification,
                    quantity: quantityValue,   // 연수 (0.5, 1, 1.5 등)
                    flyer_mesu: mesuValue,     // 전단지/리플렛 전용 매수 (스티커용 mesu와 분리)
                    mesu: mesuValue,           // 레거시 호환용
                    quantity_display: quantityDisplay,  // 표시용: "0.5연\n(2,000매)"
                    unit: '연',                // 단위는 "연"
