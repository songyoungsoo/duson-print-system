...
// 전역 변수 (window 스코프로 노출)
window.currentPriceData = null;
let currentPriceData = null; // 로컬 참조용
let galleryImages = [];
...
function handleModalBasketAdd(uploadedFiles, onSuccess, onError) {
    ...
    // 추가 옵션 가격 포함한 총 가격 계산
    const additionalOptionsTotalElement = document.getElementById('additional_options_total');
    const additionalOptionsTotal = additionalOptionsTotalElement ? parseInt(additionalOptionsTotalElement.value) || 0 : 0;

    const basePriceWithoutVat = Math.round(currentPriceData.Order_PriceForm) + additionalOptionsTotal;
    const totalPriceWithVat = Math.round(basePriceWithoutVat * 1.1);

    console.log('💰 가격 계산:', {
        basePrice: Math.round(currentPriceData.Order_PriceForm),
        additionalOptions: additionalOptionsTotal,
        totalWithoutVat: basePriceWithoutVat,
    ...
    // 추가 옵션 데이터 수집 (🔧 FIX: checked 대신 ID로 직접 접근)
    console.log('🔧 추가 옵션 데이터 수집 시작');

    // 코팅 옵션 - ID로 직접 접근
    const coatingCheckbox = document.getElementById('coating_enabled');
    const coatingEnabledValue = (coatingCheckbox && coatingCheckbox.checked) ? '1' : '0';
    formData.set('coating_enabled', coatingEnabledValue);
    console.log('코팅 체크박스 상태:', coatingCheckbox ? coatingCheckbox.checked : 'not found', '→', coatingEnabledValue);

    if (coatingEnabledValue === '1') {
        const coatingType = document.getElementById('coating_type')?.value || '';
        const coatingPrice = document.getElementById('coating_price')?.value || '0';
    ...
    // 접지 옵션 - ID로 직접 접근
    const foldingCheckbox = document.getElementById('folding_enabled');
    const foldingEnabledValue = (foldingCheckbox && foldingCheckbox.checked) ? '1' : '0';
    formData.set('folding_enabled', foldingEnabledValue);
    console.log('접지 체크박스 상태:', foldingCheckbox ? foldingCheckbox.checked : 'not found', '→', foldingEnabledValue);

    if (foldingEnabledValue === '1') {
        const foldingType = document.getElementById('folding_type')?.value || '';
        const foldingPrice = document.getElementById('folding_price')?.value || '0';
    ...
    // 오시 옵션 - ID로 직접 접근
    const creasingCheckbox = document.getElementById('creasing_enabled');
    const creasingEnabledValue = (creasingCheckbox && creasingCheckbox.checked) ? '1' : '0';
    formData.set('creasing_enabled', creasingEnabledValue);
    console.log('오시 체크박스 상태:', creasingCheckbox ? creasingCheckbox.checked : 'not found', '→', creasingEnabledValue);

    if (creasingEnabledValue === '1') {
        const creasingLines = document.getElementById('creasing_lines')?.value || '';
        const creasingPrice = document.getElementById('creasing_price')?.value || '0';
    ...
    console.log('📊 추가 옵션 총합:', additionalOptionsTotal, '원');
    ...
    if (fileInput && fileInput.files && fileInput.files.length > 0) {
        console.log('✅ 파일 발견:', fileInput.files.length, '개');
        for (let i = 0; i < fileInput.files.length; i++) {
            formData.append('uploaded_files[]', fileInput.files[i]);
            console.log(`✅ 파일 추가: ${fileInput.files[i].name}`);
        }
    } else {
        console.log('⚠️ 파일 없음');
    }
    ...
    console.log('- 기본가격 (VAT제외):', basePriceWithoutVat);
    console.log('- 총 가격 (VAT포함):', totalPriceWithVat);
    console.log('- 추가옵션:', additionalOptionsTotal);
    ...
    for (let pair of formData.entries()) {
        if (pair[0].includes('uploaded_files')) {
            console.log(pair[0] + ':', pair[1]);
        }
    }
...
function initDropdownEvents() {
    ...
    if (paperTypeSelect) {
        paperTypeSelect.addEventListener('change', function() {
            resetDownstreamSelects(['MY_amount']);
            resetPriceDisplay();
            updateQuantities(); // 수량 업데이트 후 자동 계산됨
        });
    }
    ...
    if (sidesSelect) {
        sidesSelect.addEventListener('change', function() {
            ...
            resetDownstreamSelects(['MY_amount']);
            resetPriceDisplay();
            updateQuantities(); // 단면/양면에 따라 수량 다시 로드 후 자동 계산됨
        });
    }
...
function resetPriceDisplay() {
    ...
    window.currentPriceData = null; // 전역도 초기화
}
        ...
        .then(data => {
            ...
            if (quantitySelect) {
                ...
                data.forEach((option, index) => {
                    ...
                    optionElement.dataset.mesu = option.mesu || '';  // 매수를 data 속성으로 저장
...
// 전역 스코프로 노출 (index.php에서 호출 가능하도록)
...
function calculatePrice(isAuto = false) {
    ...
    // 🆕 추가 옵션 가격 포함 (additional_options_total)
    ...
    if (additionalOptionsTotalInput) {
        additionalOptionsTotal = parseInt(additionalOptionsTotalInput.value) || 0;
    }
    ...
    const requestUrl = 'calculate_price_ajax.php?' + params.toString();
    console.log('📡 가격 계산 요청 URL:', requestUrl);
    console.log('📋 요청 파라미터:', {
        MY_type: formData.get('MY_type'),
        PN_type: formData.get('PN_type'),
        MY_Fsd: formData.get('MY_Fsd'),
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            window.currentPriceData = data.data; // 전역으로도 설정
...
function updatePriceDisplay(priceData) {
    ...
    // 🆕 추가 옵션 가격 가져오기 (additional_options_total)
...
function initGalleryZoom() {
    ...
    zoomBox.addEventListener('mousemove', function(e) {
        ...
        if (currentImageType === 'small') {
            targetSize = 200; // 작은 이미지는 2배
        } else {
            targetSize = 200; // 큰 이미지는 2배
        }
    });
