...
class UploadPathHelper {
    ...
    private static function getBaseRoot() {
        if (self::$baseRoot === null) {
            // CLI 환경에서는 DOCUMENT_ROOT가 빈 문자열일 수 있음
            $docRoot = (!empty($_SERVER['DOCUMENT_ROOT'])) ? $_SERVER['DOCUMENT_ROOT'] : '/var/www/html';
            self::$baseRoot = $docRoot . self::$webRoot;
        }
    ...
    public static function generateUploadPath($product, $ip = null, $timestamp = null) {
        ...
        // IP 주소 정규화 (Windows 호환)
        // IPv6 주소 (::1) → ipv6_1
        // IPv4 주소 (192.168.1.1) → 192_168_1_1
        $safe_ip = str_replace([':', '.'], ['_', '_'], $ip);
        if (strpos($ip, ':') !== false) {
            // IPv6 주소인 경우 ipv6_ 접두어 추가
            $safe_ip = 'ipv6_' . trim($safe_ip, '_');
        }
        $ip = $safe_ip;
