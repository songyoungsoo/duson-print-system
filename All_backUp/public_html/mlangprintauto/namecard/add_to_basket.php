...
// 디버그: 받은 데이터 로그
error_log("=== 명함 장바구니 추가 시작 [" . date('Y-m-d H:i:s') . "] ===");
error_log("받은 POST 데이터: " . print_r($_POST, true));
error_log("받은 FILES 데이터: " . print_r($_FILES, true));
...
// 🔍 디버깅: $_FILES 상태 확인
error_log("🔍 $_FILES 체크:");
error_log("$_FILES 존재 여부: " . (isset($_FILES) ? "있음" : "없음"));
error_log("$_FILES 내용: " . print_r($_FILES, true));
error_log("uploaded_files 키 존재: " . (isset($_FILES['uploaded_files']) ? "있음" : "없음"));
error_log("uploaded_files 비어있음: " . (empty($_FILES['uploaded_files']) ? "예" : "아니오"));

if (!empty($_FILES['uploaded_files'])) {
    error_log("✅ 파일 업로드 로직 진입!");
    // 디렉토리 생성
...
if ($stmt) {
    ...
    error_log("bind_param 전 - 파라미터 수: 14개");
    error_log("=== 명함 옵션명 디버그 ===");
    error_log("MY_type_name: " . $MY_type_name);
    error_log("Section_name: " . $Section_name);
    error_log("POtype_name: " . $POtype_name);

    $bind_result = mysqli_stmt_bind_param($stmt, "sssssssiisisss",
    ...
    error_log("명함 장바구니 prepare 실패: " . $error_msg);
    error_log("SQL: " . $insert_query);
