...
// 전역 변수들 - currentPriceData가 이미 window 객체에 있을 수 있으므로 체크
if (typeof window.currentPriceData === 'undefined') {
...
function loadPaperTypes(style) {
    ...
    fetch(`/mlangprintauto/namecard/get_paper_types.php?style=${style}`)
...
function loadQuantities() {
    ...
    fetch(`/mlangprintauto/namecard/get_quantities.php?style=${style}&section=${section}&potype=${potype}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function calculatePrice(isAuto = true) {
    ...
    fetch('/mlangprintauto/namecard/calculate_price_ajax.php?' + params.toString())
    .then(response => {
        if (!response.ok) {
...
function addToBasketFromModal() {
    ...
    fetch('/mlangprintauto/namecard/add_to_basket.php', {
        method: 'POST',
        body: formData
    })
    .then(response => {
