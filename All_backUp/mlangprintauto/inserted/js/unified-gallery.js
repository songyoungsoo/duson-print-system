...
class UnifiedGallery {
    ...
    async loadSampleImages() {
        try {
            const response = await fetch('/api/get_leaflet_samples.php');
            const data = await response.json();
            
            if (data.success && data.data) {
                this.currentImages = data.data.slice(0, this.options.thumbnailLimit);
                this.renderThumbnails();
                this.setMainImage(0);
            } else {
            ...
            console.error('샘플 이미지 로딩 실패:', error);
            this.loadDefaultImages();
        }
    }
    ...
    openLightbox(index) {
        ...
        if (lightboxOverlay && !lightboxOverlay.hasAttribute('data-click-listener')) {
            lightboxOverlay.setAttribute('data-click-listener', 'true');
        ...
        if (lightboxImage && !lightboxImage.hasAttribute('data-click-listener')) {
            lightboxImage.setAttribute('data-click-listener', 'true');
        ...
        // 라이트박스 컨텐츠 영역 클릭 시에도 닫기 (이전/다음 버튼 제외)
        const lightboxContent = this.lightbox.querySelector('.lightbox-content');
        if (lightboxContent && !lightboxContent.hasAttribute('data-click-listener')) {
            lightboxContent.setAttribute('data-click-listener', 'true');
            lightboxContent.addEventListener('click', (e) => {
                // 이전/다음 버튼이나 닫기 버튼을 클릭한 경우는 제외
