...
<?
include"../../db.php";
$Cate_result= mysqli_query("select * from $GGTABLE where Ttable='$page' and BigNo='0'",$db);
...
<?
$result= mysqli_query("select * from $GGTABLE where Ttable='$page' and TreeNo='$Cate_row[no]' order by no asc",$db);
...
<?
$result= mysqli_query("select * from $GGTABLE where Ttable='$page' and BigNo='$Cate_row[no]' order by no asc",$db);
$rows=mysqli_num_rows($result);
if($rows){
	$g=0;
while($row= mysqli_fetch_array($result)) { 
