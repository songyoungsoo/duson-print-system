...
$types = [];
$r = mysqli_query($db, "SELECT no, title FROM mlangprintauto_transactioncate WHERE Ttable='NameCard' AND BigNo='0' ORDER BY TreeNo, no");
if ($r) { while ($row = mysqli_fetch_assoc($r)) { $types[] = $row; } }

// 프리미엄 옵션 가격을 DB에서 조회 (premium_options SSOT)
$premOpts = [];
$product_type_for_opts = 'namecard';
$r = mysqli_query($db, "
...
function loadChildren(parentVal, childId) {
    ...
    fetch(OPT_URL + '?table=namecard&parent=' + parentVal, {credentials: 'same-origin'})
...
function loadQuantities() {
    ...
    if (!style || !section) {
        qty.innerHTML = '<option value="">상위 항목을 선택</option>';
        resetPrice();
        return;
    }

    var url = OPT_URL + '?table=namecard&source=price&field=quantity&filter_style=' + style + '&filter_Section=' + section + '&filter_POtype=' + document.getElementById('POtype').value;
    fetch(url, {credentials: 'same-origin'})
        ...
        .then(function(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var o = document.createElement('option');
                o.value = data[i].no;
                var n = parseInt(data[i].title);
                o.textContent = n ? n.toLocaleString() + '매' : data[i].title;
                qty.appendChild(o);
            }
            // Auto-select first quantity and trigger price calculation
            if (data.length >= 1) {
                qty.value = data[0].no;
        ...
        .catch(function() {
            qty.innerHTML = '<option value="">로딩 실패</option>';
        });
...
function calculatePrice() {
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({product: 'namecard', params: params})
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
        ...
        if (data.success && data.payload) {
            ...
            var premNames = [];
            var premMap = {foil:'박', numbering:'넘버링', perforation:'타공', rounding:'귀돌이', creasing:'오시'};
            var opts2 = ['foil', 'numbering', 'perforation', 'rounding', 'creasing'];
            for (var j = 0; j < opts2.length; j++) {
                if (document.getElementById('opt_' + opts2[j]).checked) {
                    premNames.push(premMap[opts2[j]]);
                }
            }
            if (premNames.length > 0) line2Parts.push(premNames.join('+'));
