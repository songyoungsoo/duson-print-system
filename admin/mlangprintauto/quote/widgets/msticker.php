...
$types = [];
$r = mysqli_query($db, "SELECT no, title FROM mlangprintauto_transactioncate WHERE Ttable='Msticker' AND BigNo='0' ORDER BY TreeNo, no");
...
function loadChildren(parentVal, childId) {
    ...
    fetch(OPT_URL + '?table=msticker&parent=' + parentVal, {credentials: 'same-origin'})
...
function loadQuantities() {
    ...
    var url = OPT_URL + '?table=msticker&source=price&field=quantity&filter_style=' + style + '&filter_Section=' + section + '&filter_POtype=' + document.getElementById('POtype').value;
...
function calculatePrice() {
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({product: 'msticker', params: params})
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
