...
$types = [];
$r = mysqli_query($db, "SELECT no, title FROM mlangprintauto_transactioncate WHERE Ttable='Cadarok' AND BigNo='0' ORDER BY TreeNo, no");
if ($r) { while ($row = mysqli_fetch_assoc($r)) { $types[] = $row; } }

// 추가옵션 가격을 DB에서 조회 (premium_options SSOT)
$addOpts = ['coating' => [], 'folding' => [], 'creasing' => []];
$product_type_for_opts = 'cadarok';
$r = mysqli_query($db, "
...
function loadChildren(parentVal, childId) {
    ...
    fetch(OPT_URL + '?table=cadarok&parent=' + parentVal, {credentials: 'same-origin'})
...
function calculatePrice() {
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({product: 'cadarok', params: params})
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
        ...
        if (data.success && data.payload) {
            ...
            if (Object.keys(optionDetails).length > 0) {
                currentPayload.premium_options = JSON.stringify(optionDetails);
            }
        } else {
