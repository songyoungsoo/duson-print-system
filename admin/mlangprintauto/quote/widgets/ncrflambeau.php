...
$types = [];
$r = mysqli_query($db, "SELECT no, title FROM mlangprintauto_transactioncate WHERE Ttable='NcrFlambeau' AND BigNo='0' ORDER BY TreeNo, no");
...
<div class="form-grid">
    <label>종류</label>
    <select id="MY_type" onchange="loadCascade(this.value,'MY_Fsd',['PN_type','MY_amount'])">
    ...
    <label>규격</label>
    <select id="MY_Fsd" onchange="loadCascade(this.value,'PN_type',['MY_amount'])">
    ...
    <label>수량</label>
    <select id="MY_amount" onchange="calculatePrice()">
...
function calculatePrice() {
    ...
    var myFsd = document.getElementById('MY_Fsd').value;
    var pnType = document.getElementById('PN_type').value;
    var myAmount = document.getElementById('MY_amount').value;

    if (!myType || !myFsd || !pnType || !myAmount) {
        resetPrice();
        return;
    }
    ...
    fetch(API_URL, {
        ...
        body: JSON.stringify({product: 'ncrflambeau', params: params})
    })
    .then(function(r) { return r.json(); })
    .then(function(data) {
