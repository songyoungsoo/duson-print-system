...
// 6. 📋 ImagePathResolver 통합 경로 해석 사용
require_once $base_dir . "includes/ImagePathResolver.php";
...
if (!$json_path_found && !file_exists($full_path)) {
    ...
    // ImagePathResolver 통합 해석 시도 (마지막 폴백)
    if (!file_exists($full_path) && !empty($no)) {
        // DB에서 전체 row 조회하여 ImagePathResolver 사용
        if (!isset($db)) {
            include $base_dir . "db.php";
        }
        if (isset($db) && $db) {
            $query = "SELECT no, uploaded_files, ImgFolder, ThingCate FROM mlangorder_printauto WHERE no = ? LIMIT 1";
            $stmt = mysqli_prepare($db, $query);
            if ($stmt) {
                mysqli_stmt_bind_param($stmt, "i", $no);
                mysqli_stmt_execute($stmt);
                ...
                if ($row) {
                    $resolved_path = ImagePathResolver::resolve($no, $downfile, $row);
                    if ($resolved_path && file_exists($resolved_path)) {
                        $full_path = $resolved_path;
                        error_log("Download: ImagePathResolver 사용 - $full_path");
                    }
                }
            }
        }
