...
include"../../db.php";
$result= mysqli_query($db, "select * from mlangorder_printauto_OfferOrder where no='$no'");
...
include"../../db.php";
$query ="UPDATE mlangorder_printauto_OfferOrder SET AdminSelect='yes' WHERE no='$no'";
...
include"../../db.php";
$result = mysqli_query($db, "DELETE FROM mlangorder_printauto_OfferOrder WHERE no='$no'");
mysqli_close($db);
...
<td align=center>
<?php if($row[AdminSelect]=="no"){echo("<b><font color=red>미확인</font></b>");}
if($row[AdminSelect]=="yes"){echo("<font color=white>확인</font>");}
?>
</td>
<td align=center><input type='button' onClick="javascript:popup=window.open('<?=$PHP_SELF?>?mode=view&no=<?=$row[no]?>', 'mlangorder_printauto_OfferOrder','width=600,height=430,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 자세한정보보기 '></font></td>
<td align=center>
