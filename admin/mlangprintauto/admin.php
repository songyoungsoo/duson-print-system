...
include_once __DIR__ . "/../includes/theme_loader_admin.php"; // 관리자 테마 로더
...
?>
<head>
    <?php AdminThemeLoader::renderInlineStyles(); ?>
    <style>
...
<body class='coolBar' <?php AdminThemeLoader::renderBodyAttributes(); ?>>
...
?>
    ...
    <style>
        ...
        /* ===== OrderFormOrderTree.php 스타일 오버라이드 (Admin 색감 적용) ===== */
        ...
        .excel-group-header td {
            background: linear-gradient(180deg, #6c5ce7 0%, #a29bfe 100%) !important;
            color: #fff !important;
        }
    ...
    <?php if (!empty($no)) { ?>
        <div class="file-section" style="padding: 12px; margin: 10px 0;">
            ...
            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border: 1px solid #e9ecef;">
                    ...
                    <?php
                    if ($row) {
                        // ✅ ImagePathResolver 사용 - 날짜 기반 필터링 적용 (2025-12-06 추가)
                        // - 교정용 이미지: 2018년 이전 제외
                        // - 고객 원고 파일: 2024년 이전 제외
                        require_once __DIR__ . '/../../includes/ImagePathResolver.php';

                        echo "<strong>📎 업로드된 파일:</strong><br>";

                        // ImagePathResolver로 파일 목록 가져오기 (날짜 필터 적용)
                        $file_result = ImagePathResolver::getFilesFromRow($row, true);
                        $files = $file_result['files'];
                        $total_file_count = count($files);

                        // 필터 제외 메시지 표시
                        $filter_message = ImagePathResolver::getFilterMessage($file_result);
                        ...
                        if (count($proof_files) > 0) {
                            echo "<div style='margin-top: 10px; color: #28a745; font-weight: bold;'>📌 교정/시안 이미지:</div>";
                            foreach ($proof_files as $file_info) {
                                $name = $file_info['name'] ?? $file_info['saved_name'] ?? '';
                                $saved_name = $file_info['saved_name'] ?? $name;
                                $file_size_mb = 0;
                                ...
                                echo "📌 <a href='download.php?no=$no&downfile=" . urlencode($saved_name) . "' class='file'>";
                                echo htmlspecialchars($name) . "</a> ({$file_size_mb}MB) <span style='color: #28a745;'>(교정)</span><br>";
                            }
                        }

                        // ✅ 고객 원고 파일 표시
                        if (count($customer_files) > 0) {
                            echo "<div style='margin-top: 10px; color: #2196f3; font-weight: bold;'>📄 고객 원고 파일:</div>";
                            foreach ($customer_files as $file_info) {
                                $name = $file_info['original_name'] ?? $file_info['name'] ?? $file_info['saved_name'] ?? '';
                                $saved_name = $file_info['saved_name'] ?? $name;
                                $file_size_mb = 0;
                                if (isset($file_info['path']) && file_exists($file_info['path'])) {
                                    $file_size_mb = round(filesize($file_info['path']) / 1024 / 1024, 2);
                                ...
                                echo "📄 <a href='download.php?no=$no&downfile=" . urlencode($saved_name) . "' class='file'>";
                                echo htmlspecialchars($name) . "</a> ({$file_size_mb}MB)<br>";
                            }
                        }

                        // ✅ 레거시/기타 파일 표시
                        if (count($legacy_files) > 0) {
                            echo "<div style='margin-top: 10px; color: #ff9800; font-weight: bold;'>📁 기타 파일:</div>";
                            foreach ($legacy_files as $file_info) {
                                $name = $file_info['name'] ?? $file_info['saved_name'] ?? '';
                                $saved_name = $file_info['saved_name'] ?? $name;
                                $file_size_mb = 0;
                                if (isset($file_info['path']) && file_exists($file_info['path'])) {
                                ...
                                $is_thing_cate = ($saved_name == $row['ThingCate']);
                                $icon = $is_thing_cate ? "📌" : "📄";
                                echo "$icon <a href='download.php?no=$no&downfile=" . urlencode($saved_name) . "' class='file'>";
                                echo htmlspecialchars($name) . "</a> ({$file_size_mb}MB)";
                                if ($is_thing_cate) {
