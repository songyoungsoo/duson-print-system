...
$no = $_REQUEST['no'] ?? '';
$YearOne = $_REQUEST['YearOne'] ?? '';
$YearTwo = $_REQUEST['YearTwo'] ?? '';
$Type = $_REQUEST['Type'] ?? '';
$Cate = $_REQUEST['Cate'] ?? '';
$TDsearchValue = $_REQUEST['TDsearchValue'] ?? '';
$offset = isset($_REQUEST['offset']) ? (int)$_REQUEST['offset'] : 0;  // 숫자일 경우 형변환
$CountWW = $_REQUEST['CountWW'] ?? '';
$ModifyCode = $_REQUEST['ModifyCode'] ?? '';  // ModifyCode 변수 추가
$OrderCate = $_POST['OrderCate'] ?? '';
$Orderstyle = $_POST['Orderstyle'] ?? '';

$JK = $_POST['JK'] ?? $_GET['JK'] ?? null;
$check = $_POST['check'] ?? [];  // 배열 형태로 초기화
...
// $Orderstyle = isset($_POST['Orderstyle']) ? $_POST['Orderstyle'] : '';
...
<input type='button' onClick="javascript:popup=window.open('admin.php?mode=SinForm&coe&no=<?php echo  $row['no'] ?><?php if($row['ThingCate']){?>&ModifyCode=ok<?php } ?>', 'SinHH','width=600,height=300,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value='교정/시안 <?php if($row['ThingCate']){ ?>수정<?php } else { ?>등록<?php } ?>'>
