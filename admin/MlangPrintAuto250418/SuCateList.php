...
function WebOffice_customer_Del(no) {
  if (confirm(no + '번 자료를 삭제 하시겠습니까..??\n\n최상위 일경우 하위항목까지 삭제가 됩니다.\n\n한번 삭제한 자료는 복구 되지 않으니 신중을 기해주세요.............!!')) {
    const str = './SuCateAdmin.php?no=' + no + '&mode=delete';
...
(<b><?php echo $View_TtableC?></b>) 수량 LIST
</td>
</tr>
<tr>
<td>
...
?>
<option value='<?php echo $PHP_SELF?>?Cate=<?php echo $Cate_row['no']?>&Ttable=<?php echo $Ttable?>' <?php if ($Cate == $Cate_row['no']){?>style='background-color:#429EB2; color:#FFFFFF;' selected<?php } ?>><?php echo $Cate_row['title']?></option>
<?php
  }
}
?>
<option value='<?php echo $PHP_SELF?>?Ttable=<?php echo $Ttable?>'>→ 전체자료보기</option>
</SELECT>
</td>
</tr>
</table>
...
<input type='button' onClick="javascript:popup=window.open('./SuCateAdmin.php?mode=form&Ttable=<?php echo $Ttable?>', 'WebOffice_<?php echo $PageCode?>Form','width=500,height=120,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수량 입력하기 '>
</td>
</tr>
</table>
...
<td align=center>
<input type='button' onClick="javascript:popup=window.open('./SuCateAdmin.php?mode=form&code=modify&no=<?php echo $row['no']?>&Ttable=<?php echo $Ttable?>', 'WebOffice_<?php echo $PageCode?>Modify','width=500,height=120,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value=' 수정 '>
