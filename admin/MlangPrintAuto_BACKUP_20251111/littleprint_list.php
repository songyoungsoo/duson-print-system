...
$table="mlangprintauto_littleprint"; // ✅ 소문자 테이블명
...
<?php 
$result_FGTwo=mysqli_query($db, "select * from $GGTABLE where Ttable='LittlePrint' AND no='{$row['style']}'");
...
<td align=center><font color=white>
<?php 
$result_FGFree=mysqli_query($db, "select * from $GGTABLE where Ttable='LittlePrint' AND no='{$row['TreeSelect']}'");
...
<td align=center><font color=white>
<?php 
$result_FGOne=mysqli_query($db, "select * from $GGTABLE where Ttable='LittlePrint' AND no='{$row['Section']}'");
...
<td align=center><font color=white><?=$row['quantity']?></font></td>
<td align=center><font color=white>
...
<input type='button' onClick="javascript:WomanMember_Admin_Del('<?=$row['no']?>');" value=' 생제 '>
</td>
<tr>
