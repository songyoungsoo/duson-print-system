...
$CountWW = $_GET['CountWW'] ?? $_POST['CountWW'] ?? '';  // 정렬 컬럼 (예: no, date 등)

// 폼 데이터 초기화 (검색 키워드 관련)
$One_1 = $_POST['One_1'] ?? $_GET['One_1'] ?? '';
$One_2 = $_POST['One_2'] ?? $_GET['One_2'] ?? '';
$One_3 = $_POST['One_3'] ?? $_GET['One_3'] ?? '';
$Type = $_POST['Type'] ?? '';
$TDsearchValue = $_POST['TDsearchValue'] ?? '';
$YearOne = $_POST['YearOne'] ?? '';
$YearTwo = $_POST['YearTwo'] ?? '';

// 검색 파라미터 처리용 변수
$TDsearch = $_POST['TDsearch'] ?? '';
$OrderCate = $_POST['OrderCate'] ?? '';
$OrderStyleYU9OK = $_POST['OrderStyleYU9OK'] ?? '';
$PHP_SELF = $_SERVER['PHP_SELF'];  // 안전하게 다시 설정
...
if ($rows) {
    while ($row = mysqli_fetch_array($result)) {
        ...
        $one2Value = $row['One_2'] ?? '';
        if ($one2Value == "1") echo "신규업체";
        if ($one2Value == "2") echo "거래업체";
        if ($one2Value == "3") echo "하청";
        echo "</font></td>";
