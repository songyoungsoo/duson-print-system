...
$DesignMoney = isset($DesignMoney) ? $DesignMoney : '';
$no = isset($_GET['no']) ? (int)$_GET['no'] : 0;
$RadOne = isset($_POST['RadOne']) ? $_POST['RadOne'] : '';
...
<tr>
    ...
    <td>
        ...
        <?php } ?>
        <input type='button' value='디버그 정보' onclick="console.log(document.myForm); alert('RadOne: ' + document.myForm.RadOne.value + '\nmyList: ' + document.myForm.myList.value + '\nquantity: ' + document.myForm.quantity.value + '\nmoney: ' + document.myForm.money.value);">
    </td>
</tr>
</form>
</table>
...
<?php
 ...
 if ($mode == "form_ok") {
    ...
    // 디버그 정보 출력
    error_log("form_ok mode: RadOne=$RadOne, myList=$myList, quantity=$quantity, money=$money, TDesignMoney=$TDesignMoney, POtype=$POtype");

    // 준비된 명령문으로 SQL 인젝션 방지
    $stmt = $db->prepare("INSERT INTO $TABLE (style, Section, quantity, money, DesignMoney, POtype) VALUES (?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        error_log("Prepare failed: " . $db->error);
        echo "<script>alert('SQL 준비 오류: " . addslashes($db->error) . "');</script>";
        exit;
    }
    
    $stmt->bind_param("ssssss", $RadOne, $myList, $quantity, $money, $TDesignMoney, $POtype); // 모든 파라미터를 문자열로 처리
...
if ($mode == "Modify_ok") {
    ...
    // 디버그 정보 출력
    error_log("Modify_ok mode: RadOne=$RadOne, myList=$myList, quantity=$quantity, money=$money, TDesignMoney=$TDesignMoney, POtype=$POtype, no=$no");
    ...
    $stmt->bind_param("sssssss", $RadOne, $myList, $quantity, $money, $TDesignMoney, $POtype, $no); // 모든 파라미터를 문자열로 처리
