...
if ($mode == "SubmitOk") { ////////////////////////////////////////////////////////////////////////////
    ...
    $stmt = $db->prepare("INSERT INTO MlangOrder_PrintAuto 
        (no, Type, ImgFolder, TypeOne, money_1, money_2, money_3, money_4, money_5, name, email, zip, zip1, zip2, phone, Hendphone, bizname, bank, bankname, cont, date, status, ThingCate, another_phone, Gensu) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    $stmt->bind_param(
        ...
        $cont, $date, $status, $ThingCate, $phone, $Gensu
    );
...
if ($mode == "OrderView") {
    ...
    <br><br>
    <?php if ($no) { ?>
    <font style='font:bold; color:#336699;'>* 첨부 파일 *</font> 파일명을 클릭하시면 저장/보기를 하실 수 있습니다.
    <br>=============================<br>

    <table border="0" align="center" width="100%" cellpadding="5" cellspacing="0">
        <tr>
            <td height="20">
                <?php if (isset($row["ThingCate"])) { ?>
                    <a href='download.php?downfile=<?php echo  urlencode($row["ThingCate"]) ?>' class="file">
                        <?php echo  htmlspecialchars($row["ThingCate"]) ?>
                    </a>
                <?php } else { ?>
...
if ($mode == "SinFormModifyOk") {
    ...
    $ModifyCode = intval($no); // SQL 인젝션 방지
    ...
    // ✅ 파일 업로드 처리
    if (!empty($GF_upfile)) {
        if (!empty($_FILES['photofileModify']['name'])) {
            if (!empty($_FILES['photofile']['name'])) {
                $upload_dir = $dir;
                include "upload.php";

                // ✅ 기존 파일 삭제
                $old_file_path = "$dir/$GF_upfile";
