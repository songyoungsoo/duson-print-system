		...
		for(j=0;j<this.l[i].maxnum;j++){
			...
			if(i==0){n=this.l[0].names[j]; txt=this.m[n].text
				if(!bw.ns6){
					...
					str+=tempstr+' class="cl'+this.name+i+'">&nbsp;<img src="/admin/img/left_icon1234567.gif" width="4" height="20" align="absmiddle">&nbsp;'+txt+'</div>'; str+='</div>\n'
				}else this.oNS[j]=cm_NS6_createElement(this.ns6styleb[i],tempstr+' class="cl'+this.name+i+'">&nbsp;<img src="/admin/img/left_icon1234567.gif" width="4" height="20" align="absmiddle">&nbsp;'+txt+'</div>')
			}else frstr+=tempstr+'"></div>\n'
		}if(i!=0){frstr+='</div>\n'}
	}if(this.usebar){
		if(this.barinheritborder) str2='<div id="div'+this.name+'Barb" class="coolBar"></div>'; 
		if(bw.ns6) this.oNS[this.l[0].maxnum]=cm_NS6_createElement(this.ns6styleb[this.ns6styleb.length-1],str2)
		else{str+='<div id="div'+this.name+'Bar" class="coolBar">';str+=str2+'</div>\n'}
...
function cm_go(name){
	...
	if(url){
	    ...
	    else if(target=="Mlang") window.open(url,"mlANGwINKW","width=200,height=200,top=0,left=0,menubar=no,resizable=yes,statusbar=no,scrollbars=yes,toolbar=no")
