...
if ($mode == "view") {
    ...
    $stmt = $db->prepare("SELECT * FROM WebFreeMember WHERE no = ?");
    ...
    <td bgcolor='#FFFFFF'>
    <?php echo  htmlspecialchars($row['jumin1']); ?> - <?php echo  htmlspecialchars($row['jumin2']); ?>
    </td>
    </tr>
    ...
    <td bgcolor='#FFFFFF'>
    <?php echo  htmlspecialchars($row['headphone1']); ?> - <?php echo  htmlspecialchars($row['headphone2']); ?> - <?php echo  htmlspecialchars($row['headphone3']); ?>
    </td>
    </tr>
...
<td align=right>
<?php $InputStyle = "style='font-size:10pt; background-color:#429EB2; color:#FFFFFF;'"; ?>
<select onChange="MM_jumpMenu('parent',this,0)">
<option value="0" selected>선택하세요</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?job=PHP프로그램' <?php if ($job == "PHP프로그램") echo "$InputStyle selected"; ?>>PHP프로그램</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?job=홈페이지 기획/디자인' <?php if ($job == "홈페이지 기획/디자인") echo "$InputStyle selected"; ?>>홈페이지 기획/디자인</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?job=플래시 제작' <?php if ($job == "플래시 제작") echo "$InputStyle selected"; ?>>플래시 제작</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?job=드림위버 HTML 코딩작업' <?php if ($job == "드림위버 HTML 코딩작업") echo "$InputStyle selected"; ?>>드림위버 HTML 코딩작업</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?job=포토샵 작업' <?php if ($job == "포토샵 작업") echo "$InputStyle selected"; ?>>포토샵 작업</option>
<option value='<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>'>← 전체페이지</option>
</select>
</td>
...
$listcut = 12;  // 한 페이지당 보여줄 목록 게시물 수
if (!$offset) $offset = 0; 

$Mlang_query .= " ORDER BY no DESC LIMIT ?, ?";
$stmt = $db->prepare($Mlang_query);
if (isset($search_value)) {
    $stmt->bind_param("sii", $search_value, $offset, $listcut);
} else {
...
if ($rows) {
    while ($row = $result->fetch_assoc()) { 
        ...
        <td><font color=white><?php echo  htmlspecialchars($row['job']) ?></font></a></td>
        ...
        <td align=center><font color=white><?php echo  substr($row['jumin1'], 0, 2); ?></font></td>
        <td align=center><font color=white><?php echo  $row['school'] ?></font></td>
        <td align=center><font color=white><?php echo  htmlspecialchars($row['date']); ?></font></td>
        <td align=center>
        <input type='button' onClick="javascript:popup=window.open('<?php echo  htmlspecialchars($_SERVER['PHP_SELF']) ?>?mode=view&no=<?php echo  $row['no'] ?>', 'MemberModify','width=650,height=600,top=0,left=0,menubar=no,resizable=no,statusbar=no,scrollbars=yes,toolbar=no'); popup.focus();" value='회원정보보기'>
        </td>
        <tr>
