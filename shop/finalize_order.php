...
try {
    ...
    foreach ($cart_items as $item) {
        ...
        if (!empty($upload_folder) && is_dir($upload_folder)) {
            // ✅ 구버전 호환: mlangorder_printauto/upload/{주문번호}/ 경로 사용
            $new_folder = $_SERVER['DOCUMENT_ROOT'] . '/mlangorder_printauto/upload/' . $order_no . '/';
            
            if (rename($upload_folder, $new_folder)) {
                $img_folder = 'mlangorder_printauto/upload/' . $order_no . '/';
                
                // 첫 번째 파일을 ThingCate로 설정
                $files = array_diff(scandir($new_folder), ['.', '..']);
                if (!empty($files)) {
                    $thing_cate = reset($files);
                }
                
                error_log("✅ 폴더 이동 성공: $upload_folder -> $new_folder");
            } else {
                error_log("❌ 폴더 이동 실패: $upload_folder -> $new_folder");
            }
        }
