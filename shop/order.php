...
function generateEmailBody($order_id, $items, $customer_info, $is_admin = false) {
    ...
    $body .= "<tr><th>상품명</th><th>상세내역</th><th>추가옵션</th><th>수량</th><th>가격</th></tr>";
    
    $total = 0;
    foreach ($items as $item) {
        $body .= "<tr>";
        $body .= "<td>" . htmlspecialchars($item['product_name'] ?? $item['name'] ?? '상품') . "</td>";
        
        // 상세내역 표시
        $body .= "<td>";
        if (isset($item['options']) && is_array($item['options'])) {
            foreach ($item['options'] as $key => $value) {
                $body .= htmlspecialchars($key . ": " . $value) . "<br>";
            }
        }
        $body .= "</td>";
        ...
        // 추가옵션 표시
        $body .= "<td>";
        if (isset($item['additional_options']) && $item['additional_options']) {
            if (isset($item['additional_options']['options'])) {
                foreach ($item['additional_options']['options'] as $option) {
                    $body .= htmlspecialchars($option['category'] . "(" . $option['name'] . "): " . $option['formatted_price']) . "<br>";
                }
            }
        } else {
            $body .= "옵션 없음";
        ...
        $body .= "<td>" . htmlspecialchars($item['quantity'] ?? '1') . "</td>";
        $body .= "<td>" . number_format($item['st_price_vat'] ?? $item['price'] ?? 0) . "원</td>";
        $body .= "</tr>";
        $total += ($item['st_price_vat'] ?? $item['price'] ?? 0);
    }
    
    $body .= "</table>";
    $body .= "<p><strong>총 결제금액:</strong> " . number_format($total) . "원 (부가세 포함)</p>";
...
while ($data = mysqli_fetch_array($result)) {
    ...
    if ($product_type === 'leaflet') {
        ...
        $items[] = [
            ...
            'additional_options' => $has_additional_options ? $optionsDisplay->getOrderDetails($data) : null,
            'st_price' => $final_price,
            'st_price_vat' => $final_price_vat
        ];
    } else {
        // 스티커 데이터 처리
        $items[] = [
...
function getLeafletOrderInfo($connect, $data) {
    ...
    // 추가 옵션 정보 추가
    $optionsDisplay = getAdditionalOptionsDisplay($connect);
    $optionsDetails = $optionsDisplay->getOrderDetails($data);
    
    if ($optionsDetails['has_options']) {
        $info['additional_options'] = $optionsDetails;
    }
...
function generateCustomerEmailContent($order_id, $items, $customer_info)
{
    ...
    foreach ($items as $item) {
        ...
        if ($item['product_type'] === 'leaflet') {
            ...
            // 추가 옵션 정보 표시
            if (!empty($item['options']['additional_options'])) {
                $content .= '
                    <div style="margin: 10px 0; padding: 10px; background: #f0f8ff; border-radius: 5px; border-left: 3px solid #3498db;">
                        <p style="margin: 0 0 8px 0; font-weight: bold; color: #2c3e50;">📎 추가 옵션:</p>';
                
                foreach ($item['options']['additional_options']['options'] as $option) {
                    $content .= '
                        <p style="margin: 3px 0; color: #495057; font-size: 13px;">• ' . 
                        htmlspecialchars($option['category'] . '(' . $option['name'] . ')') . ': ' . 
                        htmlspecialchars($option['formatted_price']) . '</p>';
                }
                
                $content .= '
                        <p style="margin: 8px 0 0 0; padding-top: 5px; border-top: 1px solid #dee2e6; font-weight: bold; color: #e74c3c;">
                            추가옵션 소계: ' . number_format($item['options']['additional_options']['total_price']) . '원
                        </p>
                    </div>';
            }
        } else {
                ...
                <div class="order-item">
                    ...
                    <div class="item-options">
                        <?php if ($item['product_type'] === 'leaflet'): ?>
                            ...
                            <?php if (!empty($item['options']['additional_options'])): ?>
                            <div class="additional-options-section" style="grid-column: 1 / -1; margin-top: 15px; padding: 15px; background: #f0f8ff; border-radius: 10px; border-left: 4px solid #3498db;">
                                <h4 style="margin: 0 0 10px 0; color: #2c3e50; font-size: 14px;">📎 선택된 추가 옵션</h4>
                                <div style="display: grid; gap: 8px;">
                                    <?php foreach ($item['options']['additional_options']['options'] as $option): ?>
                                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 5px 0; border-bottom: 1px solid #e9ecef;">
                                        <span style="font-weight: 600; color: #495057;">• <?php echo $option['category'] . '(' . $option['name'] . ')'; ?></span>
                                        <span style="font-weight: 700; color: #28a745;"><?php echo $option['formatted_price']; ?></span>
                                    </div>
                                    <?php endforeach; ?>
                                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px 0; margin-top: 5px; border-top: 2px solid #3498db; font-weight: 700; color: #2c3e50;">
                                        <span>추가옵션 소계:</span>
                                        <span style="color: #e74c3c;"><?php echo number_format($item['options']['additional_options']['total_price']); ?>원</span>
                                    </div>
                                </div>
                            </div>
                            <?php endif; ?>
