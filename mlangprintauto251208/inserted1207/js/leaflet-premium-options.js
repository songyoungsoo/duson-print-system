 ...
 * @version 1.0
 * @date 2025-10-09
 * @author SuperClaude - Based on NameCard Premium Options System
 */
...
class LeafletPremiumOptionsManager {
    ...
    updatePriceDisplay(total = 0) {
        // 두 ID 모두 지원 (optionPriceTotal 또는 premiumPriceTotal)
        const priceElement = document.getElementById('optionPriceTotal') || document.getElementById('premiumPriceTotal');
        if (priceElement) {
            if (total > 0) {
...
document.addEventListener('DOMContentLoaded', () => {
    // additionalOptionsSection 또는 premiumOptionsSection이 존재하면 초기화
    if (document.getElementById('additionalOptionsSection') || document.getElementById('premiumOptionsSection')) {
        window.leafletPremiumOptions = new LeafletPremiumOptionsManager();
        console.log('✅ 전단지 추가 옵션 시스템 준비 완료');
    }
});
