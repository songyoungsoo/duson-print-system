...
<body class="merchandisebond-page<?php echo $body_class; ?>">
...
<?php endif; ?>
    ...
    <script>
        ...
        window.sendToQuotation = function() {
            ...
            if (!priceData || !priceData.PriceForm) {
            ...
            try {
                // Calculate supply price from PriceForm + DS_PriceForm
                const printCost = Math.round(priceData.PriceForm);
                const designCost = Math.round(priceData.DS_PriceForm);
                const supplyPrice = printCost + designCost;
                const vatPrice = Math.round(supplyPrice * 1.1);

                // 폼에서 제품 스펙 정보 수집 (상품권 전용)
