...
// 📱 모달 모드 감지 (견적서 시스템에서 iframe으로 호출될 때) - 최우선 설정
...
<body class="namecard-page<?php echo $body_class; ?>">
            ...
            <aside class="product-calculator">
                ...
                <form id="namecardForm">
                    ...
                    <div class="upload-order-button" id="uploadOrderButton">
                        <?php if ($is_quotation_mode): ?>
                            <!-- 견적서 모달 모드: 2단계 버튼 -->
                            <button type="button" class="btn-upload-order" id="calculateBtn" onclick="calculatePrice()" style="background: #0066cc;">
                                💰 1단계: 견적 계산
                            </button>
    ...
    <!-- 통합 갤러리 JavaScript (common-gallery-popup.js 사용) -->
    ...
    <script>
            ...
            fetch(`/mlangprintauto/namecard/get_paper_types.php?style=${typeValue}`)
                ...
                .then(data => {
                    if (data.success && data.data) {
                        ...
                        // 기본값이 있으면 자동 선택 (페이지 로드 시에만)
                        const defaultValue = sectionSelect.getAttribute('data-default-value');
                        if (defaultValue && data.data.some(opt => opt.no == defaultValue)) {
                            sectionSelect.value = defaultValue;
                            handleSectionChange(defaultValue);
                            console.log('✅ 기본 재질 자동 선택:', defaultValue);
                        }
                    } else {
                ...
                .then(data => {
                    ...
                    if (data.success && data.data) {
                        ...
                        if (data.data.length > 0) {
                            ...
                            console.log('✅ 수량 자동 선택:', data.data[0].value);
                            
                            // 모든 필드 상태 확인
                            const ordertypeValue = document.getElementById('ordertype').value;
                            console.log('🔍 가격 계산 전 필드 상태:', {
                                MY_type: typeValue,
                                Section: sectionValue,
                                POtype: potypeValue,
                                MY_amount: data.data[0].value,
                                ordertype: ordertypeValue
                            });
                            
                            // Section 값을 명시적으로 설정하여 유지
                            const sectionSelect = document.getElementById('Section');
                            if (sectionSelect) {
                                // 옵션이 존재하는지 확인
                                const hasOption = Array.from(sectionSelect.options).some(opt => opt.value == sectionValue);
                                if (hasOption) {
                                    sectionSelect.value = sectionValue;
                                    console.log('🔧 Section 값 설정:', sectionValue);
                                } else {
                                    console.warn('⚠️ Section 옵션 없음:', sectionValue);
                                }
                            }
                            
                            // DOM 업데이트 후 가격 계산 (타이밍 문제 해결)
                            setTimeout(() => {
                                // Section 값 다시 확인 및 복원
                                const sectionSelectCheck = document.getElementById('Section');
                                if (sectionSelectCheck) {
                                    const hasOption = Array.from(sectionSelectCheck.options).some(opt => opt.value == sectionValue);
                                    if (hasOption && !sectionSelectCheck.value) {
                                        sectionSelectCheck.value = sectionValue;
                                        console.log('🔧 Section 값 재설정:', sectionValue);
                                    } else if (!hasOption) {
                                        console.error('❌ Section 옵션 여전히 없음:', sectionValue);
                                    }
                                }
                                calculatePrice();
                            }, 150);
        ...
        function calculatePrice() {
            console.log('🔵 calculatePrice() 호출됨');
            ...
            if (!typeValue || !sectionValue || !potypeValue || !amountValue || !ordertypeValue) {
                console.warn('⚠️ 필수 필드 누락 - 가격 계산 중단');
                console.log('🔍 누락된 필드:', {
                    typeValue: typeValue || '❌',
                    sectionValue: sectionValue || '❌',
                    potypeValue: potypeValue || '❌',
            ...
            console.log('✅ 모든 필드 선택됨 - 가격 계산 진행');
        ...
        function showUploadButton() {
            ...
            // 견적서 모드일 때만 "2단계: 견적서에 적용" 버튼 표시
            const applyBtn = document.getElementById('applyBtn');
            if (applyBtn) {
                // 1단계 버튼 숨기고 2단계 버튼 표시
                const calculateBtn = document.getElementById('calculateBtn');
                if (calculateBtn) {
                    calculateBtn.style.display = 'none';
                }
                applyBtn.style.display = 'block';
                console.log('✅ 견적서 모드: 1단계 버튼 숨김 → 2단계 버튼 표시');
            }
        }
        ...
        // ⚠️ 파라미터 순서: uploadedFiles, onSuccess, onError (upload_modal.js와 일치해야 함)
        ...
        // 페이지 로드 시 초기화 - handleTypeChange로 대체됨
        // document.addEventListener('DOMContentLoaded', function() {
        //     initializeCalculator();
        //     // initializePremiumOptions(); // 프리미엄 옵션은 namecard-premium-options.js에서 자동 초기화됨
        // });

        // initializeCalculator - handleTypeChange로 대체됨
        ...
        // 용지 타입 로드 - handleTypeChange로 대체됨
        ...
        // 수량 로드 (기존 함수 - handleSectionChange로 대체됨)
        ...
        // Section 변경 이벤트 - onchange 속성으로 처리하므로 주석 처리
        // document.addEventListener('DOMContentLoaded', function() {
        //     const sectionSelect = document.getElementById('Section');
        //     if (sectionSelect) {
        ...
        window.sendToQuotation = function() {
            ...
            // 가격 계산 확인
            if (!priceData || !priceData.total_supply_price) {
            ...
            try {
                ...
                // ✅ 추가옵션 (박, 넘버링, 미싱, 귀돌이, 오시) 수집
                const additionalOptions = [];

                if (document.getElementById('foil_enabled')?.checked) {
                    additionalOptions.push('박');
                }
                if (document.getElementById('numbering_enabled')?.checked) {
                    additionalOptions.push('넘버링');
                }
                if (document.getElementById('perforation_enabled')?.checked) {
                    additionalOptions.push('미싱');
                }
                if (document.getElementById('rounding_enabled')?.checked) {
                    additionalOptions.push('귀돌이');
                }
                if (document.getElementById('creasing_enabled')?.checked) {
                    additionalOptions.push('오시');
                }

                // 규격 문자열 생성 (추가옵션 포함)
                let specification = `${typeText} / ${sectionText} / ${potypeText} / ${quantityText} / ${ordertypeText}`.trim();
                if (additionalOptions.length > 0) {
                    specification += ' + ' + additionalOptions.join(', ');
                }

                // 수량 값 추출
                const quantityValue = parseInt(myAmountSelect.value) || 500;
                ...
                const quotationData = {
                    ...
                    unit: '개',
                    supply_price: parseInt(priceData.total_supply_price) || 0,
                    vat_price: parseInt(priceData.final_total_with_vat) || 0,

                    // 원본 계산 데이터도 포함 (디버깅용)
                    _debug: {
                        MY_type: myTypeSelect ? myTypeSelect.value : '',
                        Section: sectionSelect ? sectionSelect.value : '',
                        POtype: potypeSelect ? potypeSelect.value : '',
