...
$qty_query = "SELECT quantityTwo FROM mlangprintauto_inserted
              ...
              LIMIT 1";
$qty_stmt = mysqli_prepare($connect, $qty_query);
if ($qty_stmt) {
...
error_log("리플렛 매수 조회: MY_amount=$MY_amount, mesu=$mesu");
...
// 장바구니에 추가 (파일 업로드 정보 및 추가 옵션 + quantityTwo/mesu 포함)
$insert_query = "INSERT INTO shop_temp (session_id, product_type, MY_type, PN_type, MY_Fsd, MY_amount, quantityTwo, mesu, POtype, ordertype, st_price, st_price_vat, work_memo, upload_method, uploaded_files_info, upload_folder,
...
if ($stmt) {
    ...
    // 총 18개: session_id(s), product_type(s), MY_type(s), PN_type(s), MY_Fsd(s), MY_amount(s), quantityTwo(s), mesu(s), POtype(s), ordertype(s),
    ...
    mysqli_stmt_bind_param($stmt, "ssssssssssddsssssi",
