...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* 숫자 필드는 영문 유지 */
        input[type="number"], input.price-input, input.qty-input, input.supply-input {
            ime-mode: disabled;
        }
...
<body>
                    ...
                    <table class="excel-table" id="itemsTable">
                        ...
                        <tbody id="itemsBody">
                            ...
                            function calcUnitPrice($supply, $qty) {
                                if ($qty <= 0 || $supply <= 0) return ['price' => 0, 'valid' => false, 'display' => ''];

                                $unitPrice = $supply / $qty;
                                $rounded = round($unitPrice, 2); // 소수점 2자리

                                // 정수면 정수로, 소수면 불필요한 0 제거하여 표시
                                if ($rounded == floor($rounded)) {
                                    $display = intval($rounded);
                                } else {
                                    $formatted = number_format($rounded, 2, '.', '');
                                    $display = rtrim(rtrim($formatted, '0'), '.');
                                }
                            ...
                            if ($fromCart && count($cartItems) > 0):
                                foreach ($cartItems as $item):
                                    ...
                                    // ★ 전단지/리플렛: 매수(quantityTwo) 기반으로 수량/단가 계산
                                    $mesu = 0;
                                    if (in_array($productType, ['inserted', 'leaflet'])) {
                                        $mesu = intval($item['quantityTwo'] ?? $item['mesu'] ?? 0);
                                        if ($mesu > 0) {
                                            $qty = $mesu;
                                            $unit = '매';
                                        } else {
                                    ...
                                    // 단가 계산 (소수점 1자리, 역계산 검증)
                                    $unitPriceResult = calcUnitPrice($supply, $qty);
                                    $unitPrice = $unitPriceResult['price'] ?? 0;
                                    $unitPriceDisplay = $unitPriceResult['display'];
                                    $unitPriceValid = $unitPriceResult['valid'];
                                ...
                                <tr class="item-row" data-source="cart" data-source-id="<?php echo $item['no']; ?>">
                                    ...
                                    <td class="col-qty">
                                        <input type="number" step="1" name="items[<?php echo $index; ?>][quantity]" value="<?php echo $qtyDisplay; ?>" class="qty-input" min="1">
                                    </td>
                                    <td class="col-unit">
                                        <input type="text" name="items[<?php echo $index; ?>][unit]" value="<?php echo $unit; ?>">
                                        <?php if ($mesu > 0 && $unit === '매' && $unitPriceValid): ?>
                                            <div style="color:#28a745; font-size:10px; margin-top:2px;">장당 <?php echo $unitPriceDisplay; ?>원</div>
                                        <?php endif; ?>
                                    </td>
                                    <td class="col-price"><input type="number" step="0.01" name="items[<?php echo $index; ?>][unit_price]" value="<?php echo $unitPriceDisplay; ?>" class="price-input" min="0"></td>
                ...
                <div class="section-body">
                    <div class="total-section">
                        ...
                        <div class="total-row">
                            ...
                            <input type="hidden" name="delivery_vat" id="deliveryVatInput" value="0">
                        </div>
                        <div class="total-row">
                            <span>할인</span>
    ...
    function calculateRow(row) {
        ...
        const unitPrice = parseFloat(row.querySelector('.price-input').value) || 0;  // 소수점 지원
        const supply = Math.round(qty * unitPrice);  // 결과는 정수로 반올림
     ...
     * - 소수점은 있을 때만 표시
     */
    function calcUnitPriceJS(supply, qty) {
        if (qty <= 0 || supply <= 0) return { price: 0, valid: false, display: '' };

        const unitPrice = supply / qty;
        const rounded = Math.round(unitPrice * 100) / 100;  // 소수점 2자리
        ...
        if (rounded === Math.floor(rounded)) {
            ...
            display = parseFloat(rounded.toFixed(2)).toString();
        }
        return { price: rounded, valid: true, display: display };
    }
    ...
    function calculateUnitPrice(row) {
        ...
        const supplyPrice = parseInt(supplyInput.value) || 0;

        // 단가 계산 (소수점 1자리, 역계산 검증)
        const result = calcUnitPriceJS(supplyPrice, quantity);
        priceInput.value = result.display;
        if (!result.valid) {
    ...
    function calculateTotals() {
        ...
        const deliveryVat = Math.round(delivery * 0.1);  // 배송비의 10% VAT
        const discount = parseInt(document.getElementById('discountAmount').value) || 0;

        // 합계 = 품목 공급가 + 품목 VAT + 배송비 공급가 + 배송비 VAT - 할인
        const grandTotal = supplyTotal + vatTotal + delivery + deliveryVat - discount;
        ...
        document.getElementById('deliveryVatInput').value = deliveryVat;
