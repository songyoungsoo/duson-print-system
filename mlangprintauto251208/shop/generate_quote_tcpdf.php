...
if (!empty($customer_info['memo'])) {
    $pdf->Ln(5);
    $pdf->SetFont('notosanskr', 'B', 10);
    $pdf->Cell(0, 6, '📝 요청사항:', 0, 1, 'L');
    $pdf->SetFont('notosanskr', '', 9);
...
foreach ($cart_items as $item) {
    ...
    if ($product_type === 'inserted' && !empty($item['mesu']) && intval($item['mesu']) > 0) {
        $unit_price = round(intval($item['st_price'] ?? 0) / intval($item['mesu']), 1);
    } else {
        $unit_price = intval($item['st_price'] ?? 0);
    }
    $total_item_price = intval($item['st_price'] ?? 0);
    ...
    // 전단지는 소수점 1자리 표시
    $unit_price_display = ($product_type === 'inserted') ? number_format($unit_price, 1) : number_format($unit_price);
    $pdf->Cell(25, $row_height, $unit_price_display . '원', 1, 0, 'R', $fill);
...
$pdf->SetFont('notosanskr', 'B', 11);
$pdf->SetTextColor(0, 0, 0);
$pdf->Cell(0, 6, '예금주: ' . COMPANY_ACCOUNT_HOLDER, 0, 1, 'L');

$pdf->SetFont('notosanskr', '', 11);
...
// PDF 출력 - 파일명에 담당자명+년월일 포함
$safe_customer_name = preg_replace('/[^가-힣a-zA-Z0-9]/u', '', $customer_info['name']);
if (mb_strlen($safe_customer_name) > 10) {
    $safe_customer_name = mb_substr($safe_customer_name, 0, 10);
}
$filename = '견적서_' . $safe_customer_name . '_' . date('Ymd') . '.pdf';
...
function getQuantity($item) {
    ...
    switch ($product_type) {
        case 'inserted':
            // 전단지: "0.5연 (2,000매)" 형식
            $amount = $item['MY_amount'] ?? '';
            $mesu = $item['mesu'] ?? '';
            if (!empty($amount) && !empty($mesu)) {
                return $amount . "연\n(" . number_format($mesu) . '매)';
            } elseif (!empty($amount)) {
                return $amount . '연';
            } elseif (!empty($mesu)) {
                return number_format($mesu) . '매';
            }
            return '1연';
        case 'sticker':
