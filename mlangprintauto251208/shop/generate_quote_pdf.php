    ...
    switch ($product_type) {
        case 'inserted':
            ...
            if (!empty($amount) && !empty($mesu)) {
                return $amount . '연<br>(' . number_format($mesu) . '매)';
            } elseif (!empty($amount)) {
                return $amount . '연';
            } elseif (!empty($mesu)) {
...
function generatePDFWithWkhtmltopdf($html, $quote_number, $customer_info) {
    ...
    if (file_exists($pdf_file)) {
        ...
        // 브라우저로 다운로드 - 파일명에 담당자명+년월일 포함
        $safe_name = preg_replace('/[^가-힣a-zA-Z0-9]/u', '', $customer_info['name']);
        if (mb_strlen($safe_name) > 10) {
            $safe_name = mb_substr($safe_name, 0, 10);
        }
        $download_filename = '견적서_' . $safe_name . '_' . date('Ymd') . '.pdf';
