...
// 라우팅: quotation_view 요청 처리
if (isset($_GET['action']) && $_GET['action'] === 'quotation_view') {
    include __DIR__ . '/quotation_view.php';
    exit;
}

// 라우팅: quotation_respond 요청 처리
if (isset($_GET['action']) && $_GET['action'] === 'quotation_respond') {
    include __DIR__ . '/quotation_respond.php';
    exit;
}

// 라우팅: migrate 요청 처리
if (isset($_GET['action']) && $_GET['action'] === 'migrate' && isset($_GET['key']) && $_GET['key'] === 'migrate2025') {
    include __DIR__ . '/migrate_quotations.php';
    exit;
}
...
include_once "../../includes/theme_loader.php"; // 테마 로더
                            ...
                            <tr style="background: <?php echo $row_bg; ?>; border-bottom: 1px solid #e8eaed; transition: background-color 0.2s ease;" onmouseover="this.style.background='#f0f4ff'" onmouseout="this.style.background='<?php echo $row_bg; ?>'">
                                ...
                                <td style="padding: 12px; border-right: 1px solid #e8eaed; vertical-align: top; text-align: center;">
                                    <div style="font-size: 13px; line-height: 1.4;">
                                        <?php
                                        // 스티커 감지: product_type='sticker' 또는 레거시(jong,garo,sero 필드가 있고 product_type이 비어있는 경우)
                                        $is_sticker = ($item['product_type'] == 'sticker') ||
                                                      (empty($item['product_type']) && !empty($item['jong']) && !empty($item['garo']) && !empty($item['sero']));
                                        if ($is_sticker): ?>
                                ...
                                <td style="padding: 12px; border-right: 1px solid #e8eaed; vertical-align: middle; text-align: center;">
                                    <div style="font-weight: 600; color: #2d3748; font-size: 15px;">
                                        ...
                                        error_log("cart.php 수량 표시 - no: {$item['no']}, product_type: {$item['product_type']}, MY_amount: " . ($item['MY_amount'] ?? 'NULL') . ", mesu: " . ($item['mesu'] ?? 'NULL'));

                                        // 양식지(ncrflambeau)는 "권" 단위 사용
                                        $unit = ($item['product_type'] == 'ncrflambeau') ? '권' : '매';

                                        // ★ 전단지(inserted) / 리플렛(leaflet)은 매수 기반 표시
                                        // quantityTwo 또는 mesu 값을 사용하여 "X연 (Y매)" 형식으로 표시
                                        if (in_array($item['product_type'], ['inserted', 'leaflet'])) {
                                            // quantityTwo 또는 mesu 값 확인
                                            $sheetCount = intval($item['quantityTwo'] ?? $item['mesu'] ?? 0);
                                            $reams = $item['MY_amount'] ?? '';

                                            if (!empty($reams) && $sheetCount > 0) {
                                                // 연 수와 매수 모두 있는 경우: "X연 (Y매)" + 장당 단가
                                                $displayValue = htmlspecialchars($reams);
                                                echo $displayValue . '연';
                                                echo '<br><span style="font-size: 11px; color: #718096; font-weight: 400;">(' . number_format($sheetCount) . '매)</span>';

                                                // 장당 단가 계산 및 표시
                                                $supplyPrice = intval($item['st_price'] ?? 0);
                                                if ($supplyPrice > 0 && $sheetCount > 0) {
                                                    $pricePerSheet = round($supplyPrice / $sheetCount, 2);
                                                    echo '<br><span style="font-size: 10px; color: #28a745; font-weight: 500;">장당 ' . number_format($pricePerSheet, 2) . '원</span>';
                                                }
                                            } elseif (!empty($reams)) {
                                                // 연 수만 있는 경우
                                                echo htmlspecialchars($reams) . '연';
                                            } elseif ($sheetCount > 0) {
                                                // 매수만 있는 경우
                                                echo number_format($sheetCount) . '매';
                                            } else {
                                                echo '1연';
                                            }
                                        } else {
...
<div id="quotationSection" style="display: none; font-family: 'Malgun Gothic', '맑은 고딕', sans-serif; line-height: 1.4; box-sizing: border-box; max-width: 210mm; margin: 0 auto; background: white; padding: 10mm; box-shadow: 0 4px 20px rgba(0,0,0,0.1); margin-bottom: 1rem;">
    ...
    <?php if (!empty($cart_items)): ?>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 8px; font-size: 13px;">
            ...
            <tr>
                <td style="border: 1px solid #000; padding: 6px; font-weight: bold; background: #f0f0f0;">고객명</td>
                <td style="border: 1px solid #000; padding: 6px;">
                    <input type="text" id="customerNameInput" placeholder="고객명" oninput="updatePrintValues()"
                           style="border: 1px solid #ccc; padding: 2px 5px; width: 80px; font-size: 13px;"><span class="print-value" id="customerNamePrint"></span> 귀하
                </td>
            </tr>
            <tr>
                <td style="border: 1px solid #000; padding: 6px; font-weight: bold; background: #f0f0f0;">고객 이메일</td>
                <td style="border: 1px solid #000; padding: 6px;">
                    <input type="email" id="customerEmailInput" placeholder="이메일 (선택사항)" oninput="updatePrintValues()"
                           style="border: 1px solid #ccc; padding: 2px 5px; width: 180px; font-size: 13px;"><span class="print-value" id="customerEmailPrint"></span>
                </td>
            </tr>
            <tr>
            ...
            <tr>
                <td colspan="2" rowspan="2" style="border: 1px solid #000; padding: 6px; text-align: center; font-weight: bold; font-size: 14px; vertical-align: middle; background: #f8f8f8;">
                    합계금액(부가세포함)
                </td>
            </tr>
            <tr></tr>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
            ...
            <tbody>
                ...
                foreach ($cart_items as $index => $item):
                    // 가격 계산 (장바구니와 동일한 로직)
                    $base_price = intval($item['st_price']);
                    $has_additional_options = !empty($item['coating_price']) || !empty($item['folding_price']) || !empty($item['creasing_price']);
                ...
                ?>
                    <tr>
                        ...
                        <td style="border: 1px solid #000; padding: 4px; font-size: 10px;">
                            <?php
                            // 스티커 감지: product_type='sticker' 또는 레거시(jong,garo,sero 필드가 있고 product_type이 비어있는 경우)
                            $is_sticker_quote = ($item['product_type'] == 'sticker') ||
                                                (empty($item['product_type']) && !empty($item['jong']) && !empty($item['garo']) && !empty($item['sero']));
                            ?>
                            ...
                            <!-- 추가 옵션 정보 표시 (일반 제품용 - 스티커 제외) -->
                            <?php if ($has_additional_options && $item['product_type'] != 'ncrflambeau' && !$is_sticker_quote): ?>
                                <div style="margin-top: 8px; padding-top: 8px; border-top: 1px solid #e2e8f0;">
                        ...
                        <td style="border: 1px solid #000; padding: 4px; text-align: center;">
                            <?php
                            // 전단지(inserted)는 "연 (매수)" 형식으로 표시
                            if ($item['product_type'] == 'inserted') {
                                if (!empty($item['MY_amount'])) {
                                    echo htmlspecialchars($item['MY_amount']);
                                    if (!empty($item['mesu'])) {
                                        echo '<br><span style="font-size: 10px; color: #666;">(' . number_format($item['mesu']) . '매)</span>';
                                    }
                        ...
                        <td style="border: 1px solid #000; padding: 4px; text-align: center;">
                            <?php
                            // 전단지(inserted)는 "연" 단위, 양식지(ncrflambeau)는 "권" 단위, 나머지는 "매" 단위
                            if ($item['product_type'] == 'inserted') {
                                echo '연';
    ...
    <div style="margin-top: 12px; font-size: 13px; line-height: 1.8;">
        ...
        <p style="margin: 4px 0;"><strong>▶ 고객명 :</strong></p>
...
<!-- 뒤로가기 캐시(bfcache) 방지 - 삭제 후 즉시 반영 -->
<script>
// 페이지가 bfcache에서 복원될 때 강제 새로고침
...
<?php
// 테마 스위처 UI 렌더링
ThemeLoader::renderSwitcher('bottom-left');
ThemeLoader::renderSwitcherJS();
?>
</body>
