        ...
        <div class="file-section" style="padding: 12px; margin: 10px 0;">
            ...
            <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border: 1px solid #e9ecef;">
                    ...
                    if ($row) {
                        ...
                        // ✅ ImgFolder 경로 추가 (구버전 호환)
                        if (!empty($row['ImgFolder'])) {
                            // 구버전 경로: ImgFolder/_MlangPrintAuto_xxx_index.php/...
                            $img_folder_path = "../../ImgFolder/" . $row['ImgFolder'];
                            
                            if (is_dir($img_folder_path)) {
                                $sticker_dirs[] = $img_folder_path;
                                error_log("✅ ImgFolder 경로 발견: " . $img_folder_path);
                            } else {
                                // 경로가 이미 ImgFolder를 포함하는 경우
                                $alt_path = "../../" . $row['ImgFolder'];
                                if (is_dir($alt_path)) {
                                    $sticker_dirs[] = $alt_path;
                                    error_log("✅ 대체 경로 발견: " . $alt_path);
                                } else {
                                    error_log("⚠️ 레거시 경로: " . $row['ImgFolder'] . " 폴더가 존재하지 않습니다.");
                                }
                            }
                        }
                        ...
                        // ✅ 중복 제거 및 배열 병합
                        $all_dirs = array_unique(array_merge([$upload_dir], $sticker_dirs));
                        $total_file_count = 0;
                        ...
                        foreach ($all_dirs as $dir) {
                            if (is_dir($dir)) {
                                ...
                                // 파일이 있는 경우에만 디렉토리 헤더 표시
                                if ($has_files) {
                                    $display_path = str_replace('../../', '', $dir);
                                    
                                    // 디렉토리 유형별 아이콘 및 색상
                                    if (strpos($dir, 'sticker_new') !== false) {
                                        echo "<div style='margin-top: 15px; padding: 8px; background: #d4edda; border-left: 4px solid #28a745; border-radius: 4px;'>";
                                        echo "<strong style='color: #155724;'>🏷️ 스티커 파일</strong>";
                                    } elseif (strpos($dir, 'orders') !== false) {
                                        echo "<div style='margin-top: 15px; padding: 8px; background: #d1ecf1; border-left: 4px solid #007bff; border-radius: 4px;'>";
                                        echo "<strong style='color: #004085;'>📦 주문 파일</strong>";
                                    } elseif (preg_match('/uploads\/\d{4}\/\d{2}\/\d{2}/', $dir)) {
                                        echo "<div style='margin-top: 15px; padding: 8px; background: #e2d9f3; border-left: 4px solid #9b59b6; border-radius: 4px;'>";
                                        echo "<strong style='color: #5a3a7a;'>📤 업로드 파일</strong>";
                                    } else {
                                        echo "<div style='margin-top: 15px; padding: 8px; background: #f8f9fa; border-left: 4px solid #6c757d; border-radius: 4px;'>";
                                        echo "<strong style='color: #495057;'>📁 기타 파일</strong>";
                                    }
                                    echo "</div>";
                                    ...
                                    foreach ($files as $file) {
                                        if ($file != "." && $file != ".." && is_file("$dir/$file")) {
                                            // 중복 파일 체크
                                            if (in_array($file, $displayed_files)) {
                                                continue; // 이미 표시된 파일은 건너뛰기
                                            }
                                            ...
                                            echo htmlspecialchars($file) . "</a> <span style='color: #6c757d; font-size: 0.85em;'>({$file_size_mb}MB)</span>";
