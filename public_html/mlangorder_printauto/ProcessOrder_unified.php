...
try {
    ...
    foreach ($cart_items as $item) {
        ...
        // 🆕 Phase 4: shop_temp에서 ImgFolder와 ThingCate 가져오기
        $img_folder_from_cart = $item['ImgFolder'] ?? '';
        $thing_cate_from_cart = $item['ThingCate'] ?? '';

        // 레거시 경로 검증 (레거시 형식인지 확인)
        $is_legacy_path = !empty($img_folder_from_cart) &&
                         strpos($img_folder_from_cart, '_MlangPrintAuto_') === 0;

        // ImgFolder와 ThingCate 설정
        if ($is_legacy_path) {
            // 레거시 경로 형식이면 shop_temp 값 사용
            $img_folder_path = $img_folder_from_cart;
            $thing_cate = $thing_cate_from_cart;

            // 경로 검증
            try {
                $validation_result = validateUploadPathInfo([
                    'img_folder' => $img_folder_path,
                    'physical_path' => "/var/www/html/ImgFolder/{$img_folder_path}",
                    'web_path' => "/ImgFolder/{$img_folder_path}",
                    'legacy_prefix' => '_MlangPrintAuto_',
                    'timestamp' => date('His')
                ]);

                if (!$validation_result['valid']) {
                    error_log("ImgFolder 경로 검증 실패: " . implode(', ', $validation_result['errors']));
                    // 검증 실패 시 기본 경로로 fallback
                    $img_folder_path = "uploads/orders/" . $new_no . "/";
                }
            } catch (Exception $e) {
                error_log("ImgFolder 검증 중 오류: " . $e->getMessage());
                $img_folder_path = "uploads/orders/" . $new_no . "/";
            }

            error_log("레거시 경로 사용 - ImgFolder: {$img_folder_path}, ThingCate: {$thing_cate}");
        } else {
            // 레거시 경로가 아니면 기존 방식 사용 (uploaded_files 테이블 조회)
            $thing_cate = '';
        ...
        mysqli_stmt_bind_param($stmt, 'isssiiissssssssssisissiiiiisiiiii',
