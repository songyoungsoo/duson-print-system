...
function getDeliveryRuleKey($type, $type_1) {
    ...
    // 전단지 - 90g아트지 A4
    if (preg_match('/a4/i', $type_1) && preg_match('/90g|아트지/i', $type_1)) {
        return 'inserted_90g_a4';
    }

    // 전단지 - B5(16절)
    if (preg_match('/16절|b5/i', $type_1)) {
        return 'inserted_b5_16';
    }

    // 스티커
    if (preg_match('/sticker|스티커|스티카/i', $type)) {
        return 'sticker';
    }

    // 봉투
    if (preg_match('/envelope|봉투/i', $type)) {
        return 'envelope';
    }
 ...
 * @return array ['box' => int, 'price' => int, 'label' => string]
 */
...
function extractQuantityFromType($type_1) {
    // "0.5연" → 500
    if (preg_match('/0\.5\s*연/u', $type_1)) {
        return 500;
    }

    // "1연" → 1000
    if (preg_match('/1\s*연/u', $type_1)) {
        return 1000;
    }

    // "2연" → 2000
    if (preg_match('/(\d+)\s*연/u', $type_1, $matches)) {
        return intval($matches[1]) * 1000;
    }

    // "5000매" → 5000
    if (preg_match('/(\d+)\s*매/u', $type_1, $matches)) {
 ...
 * @return array ['box' => int, 'price' => int, 'label' => string, 'rule_key' => string]
 */
